...
remotes::install_github("USEPA/TADA", ref="pseudo-develop")
...
dim(dataset) # 32217 rows, 136 columns
```
...
**Question 3: How many TADA.ActivityMediaNames exist in your dataset?**
...
wrong_datatype = dataset%>%filter(!TADA.ResultMeasureValueDataTypes.Flag%in%c("Numeric","Less Than","Greater Than","Approximate Value","Percentage","Comma-Separated Numeric","Numeric Range - Averaged"))

dataset = dataset%>%filter(TADA.ResultMeasureValueDataTypes.Flag%in%c("Numeric","Less Than","Greater Than","Approximate Value","Percentage","Comma-Separated Numeric","Numeric Range - Averaged"))
...
```{r}
dataset%>%dplyr::group_by(TADA.CharacteristicName)%>%dplyr::summarise(Result_Count = length(ResultIdentifier), Site_Count = length(unique(MonitoringLocationIdentifier)), Org_Count = length(unique(OrganizationIdentifier)))%>%dplyr::arrange(desc(Result_Count))
```
...
**Question 5: Do you notice that any totals change between the two tables? Has the number of units in the dataset increased, decreased, or stayed the same?**
...
```{r}
problem_flagged = dataset_flags%>%filter(TADA.SampleFraction.Flag=="Invalid"|TADA.MethodSpeciation.Flag=="Invalid"|TADA.ResultUnit.Flag=="Invalid"|!TADA.ActivityType.Flag=="Non-QC")

dataset_flagged = dataset_flagged%>%dplyr::filter(!ResultIdentifier%in%problem_flagged$ResultIdentifier)
...
**Question 9: What do you notice is different between the first and second tables?**
...
How are you feeling about your test dataset? Does it seem ready for the next step(s) in your analyses? One of the next big steps is data harmonization: translating and aggregating synonyms, combining multiple forms/species of certain characteristics, etc. We won't get to that in this demo, but we can start looking at how single characteristics are represented throughout the dataset. How many different units exist? Fractions? Species? We can use some plotting functions to see characteristic data ranges across related data groups.
