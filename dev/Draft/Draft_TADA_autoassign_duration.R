...
req <- request(pdf_url) |>
  req_user_agent("R/httr2 pdf fetch") |>
  req_perform()
...
split_sentences_distinct <- function(text, dedup = TRUE) {
  ...
  s <- gsub("(\\b[Ee])\\.(\\s*)coli\\b", paste0("\\1", dot_ph, "\\2coli"), s, perl = TRUE)
  ...
  if (dedup) parts <- unique(parts)
  return(parts)
}

# function to autoassign duration if possible
TADA_autoassign_duration <- function(param = "E. coli", magnitude = 700, text = pdf_text)
  ...
  magnitude_words <- function(x, value = magnitude) {
    ...
    sapply(x, function(s) {
      ...
      nums <- vapply(seq_along(loc), function(i) {
  ...
  param_index <- as.numeric(which.max(rowSums(sapply(param_words, grepl, x = list_of_strings, ignore.case = TRUE))))
  param_index
  ...
  if (str_detect(list_of_strings[final_index], "geometric\\s*mean|geo\\s*mean|30\\s*-?\\s*day")){
  ...
  WQS_PATTERNS <- list(
    ...
    FreqMethod = list(
      ...
      "Percent of samples not meeting" =
        "(?i)\\b(?:no\\s+more\\s+than\\s+)?\\d+%\\s+of\\s+samples\\s+(?:may\\s+)?(?:exceed|violate|fail|not\\s+meet)\\b|\\bpercent\\s+of\\s+samples\\s+(?:exceed|violate|fail|not\\s+meet)\\b",
      
      # e.g., “95th percentile”, “90th percentile”
      "percentile" =
        "(?i)\\b\\d{1,3}(?:st|nd|rd|th)?\\s*percentile\\b|\\bpercentile\\b",
      
      # Counts over a 3-year period; allow “samples/exceedances/excursions” and “in/per/over”
      "n-samples in 3 years" =
        "(?i)\\b(?:no\\s+more\\s+than\\s+)?\\d+\\s+(?:samples|excursions|exceedances)\\s+(?:in|per|over|within)\\s+3\\s*(?:years?|yrs?)\\b",
      
      "n-samples in 4 years" =
        "(?i)\\b(?:no\\s+more\\s+than\\s+)?\\d+\\s+(?:samples|excursions|exceedances)\\s+(?:in|per|over|within)\\s+4\\s*(?:years?|yrs?)\\b",
      
      "n-samples in 5 years" =
        "(?i)\\b(?:no\\s+more\\s+than\\s+)?\\d+\\s+(?:samples|excursions|exceedances)\\s+(?:in|per|over|within)\\s+5\\s*(?:years?|yrs?)\\b",
      
      # Binomial test references
      "binomial test" =
        "(?i)\\bbinomial\\s+test\\b|\\bexact\\s+binomial\\b|\\bbinomial\\s+method\\b",
      
      # Generic count-based frequency statements
      "NumberNotMeeting" =
        "(?i)\\b(?:number|count)\\s+of\\s+(?:samples|observations)\\s+(?:not\\s+meeting|failing|exceeding)\\b|\\bnumber\\s+not\\s+meeting\\b|\\bexceedances?\\b"
    )
  )
