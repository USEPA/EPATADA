...
- auto_assign:
  ...
  A boolean value. If TRUE, this will assign all unique ATTAINS.UseName
  to an ATTAINS.ParameterName if that parameter has not been included in
  prior ATTAINS assessment cycles for that
  ATTAINS.OrganizationIdentifier. If FALSE, the value for
  ATTAINS.UseName will be left blank for that ATTAINS.ParameterName and
  you will need to manually assign the use names as needed.
