...
create the crosswalk that defines the ATTAINS.ParameterName(s) needing
validation. All unique ATTAINS.UseNames from prior ATTAINS assessment
cycles are pulled in using ATTAINS Expert Query in this function. If a
user has defined multiple TADA.ComparableDataIdentifier matches to an
...
select a blank value for the ATTAINS.UseName to properly capture this
logic.
...
ATTAINS, they should consider using the useAURef argument input or the
useParamRef argument input which would specify that the use names should
come from a user supplied list rather than from prior ATTAINS assessment
cycles. If a list of use names come from the useAURef, this function
...
defined in your useAURef to the ATTAINS.WaterType found from ATTAINS
Expert Query.
