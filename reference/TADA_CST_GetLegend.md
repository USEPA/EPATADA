...
or "Search Tool Criteria Data" without numeric suffix), normalizes the
table, and caches the result for the session. Falls back to the
installed workbook under inst/extdata/cst-workbook.xlsx on download
failure.

## Usage

``` r
TADA_CST_GetLegend(download_only = FALSE, refresh = FALSE)
```
