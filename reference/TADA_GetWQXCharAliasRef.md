...
``` r
TADA_GetWQXCharAliasRef(download_only = FALSE, refresh = FALSE)
```
...
- download_only:
  ...
  to download the latest Characteristic Alias reference table directly
  from WQX, returning it without updating the cache. Errors if the
  download fails. If FALSE (default), uses a cached copy when available
  and updates the cache; on download failure, falls back to the
  package’s internal file.
...
data.frame with "Domain", "Unique.Identifier", "Alias.Name",
"Description", "Characteristic.Name", "Alias.Type.Name",
"Last.Change.Date"
