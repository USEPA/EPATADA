...
deprecated (i.e. retired and/or suspect) Characteristic Names with the
new name in the TADA.CharacteristicName column.
TADA_SubstituteDeprecatedChars is run within TADA_AutoClean, which runs
within TADA_DataRetrieval and (if autoclean = TRUE) in
TADA_BigDataRetrieval. Therefore, deprecated characteristic names are
harmonized to the new name automatically upon data retrieval.
TADA_SubstituteDeprecatedChars can also be used by itself on a user
supplied dataset that is in the WQX/WQP format, if desired. This
...
used as a crosswalk between the deprecated names (CharacteristicName)
and their new names (Comparable.Name).
