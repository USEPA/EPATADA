...
testthat::test_that(desc = "The CRS in the test data frame is the same as the input",
                    code = {
...
testthat::test_that(desc = "The geometry types in the data frame are points",
                    code = {
                      test_df <- TADA_MakeSpatial(data = TADA_dataframe) 
...
testthat::test_that(desc = "The number of rows are the same between TADA_dataframe and the output data frame of TADA_MakeSpatial",
                    code = {
                      test_df <- TADA_MakeSpatial(data = TADA_dataframe)
                      expect_true(nrow(TADA_dataframe) == nrow(test_df))
                    })

testthat::test_that(desc = "The columns between TADA_dataframe and the output data frame of TADA_MakeSpatial are identical (sans 'geometry')",
                    code = {
                      test_df <- TADA_MakeSpatial(data = TADA_dataframe)
                      expect_true(all(names(TADA_dataframe) == names(test_df)[names(test_df) != 'geometry']))
                    }) 

testthat::test_that(desc = "The structures of the TADA_dataframe and the output data frame of TADA_MakeSpatial are identical (sans 'geometry')",
                    code = {
                      test_df <- TADA_MakeSpatial(data = TADA_dataframe)
...
testthat::test_that(desc = "An empty df gets returned if there are no observations in the input df, ",
                    code = {
...
testthat::test_that(desc = "An input with no observations gets rejected",
                    code = {
