...
make_clean_group <- function(testdat, target_n = 20, nd_frac = 0.4, od_frac = 0.2) {
  ...
  sub$TADA.MonitoringLocationIdentifier[is.na(sub$TADA.MonitoringLocationIdentifier)] <- "LocX"
  sub$DetectionQuantitationLimitTypeName[is.na(sub$DetectionQuantitationLimitTypeName)] <- "DLX"
  ...
  if (length(nd_idx)) sub$TADA.CensoredData.Flag[nd_idx] <- "Non-Detect"
  if (length(od_idx)) sub$TADA.CensoredData.Flag[od_idx] <- "Over-Detect"
...
test_that("TADA_Stats computes stats correctly for a single random group", {
  ...
  expected_location_count <- length(unique(subf$TADA.MonitoringLocationIdentifier))
  ...
  expect_equal(out$Min, signif(min(subf$TADA.ResultMeasureValue, na.rm = TRUE), 3))
  expect_equal(out$Mean, signif(mean(subf$TADA.ResultMeasureValue, na.rm = TRUE), 3))
  expect_equal(out$Max, signif(max(subf$TADA.ResultMeasureValue, na.rm = TRUE), 3))
  
  # percentiles
  p <- function(prob) stats::quantile(subf$TADA.ResultMeasureValue, prob, na.rm = TRUE)
...
test_that("TADA_Stats respects additional grouping columns on random data", {
  ...
  out <- TADA_Stats(filtered, group_cols = c("TADA.MonitoringLocationIdentifier"))
  ...
  expected_nrows <- nrow(unique(filtered[, c("TADA.ComparableDataIdentifier", "TADA.MonitoringLocationIdentifier")]))
...
test_that("TADA_Stats rounding parameters modify outputs on random data", {
  ...
  expected_nd_count <- sum(subf$TADA.CensoredData.Flag == "Non-Detect")
  total_flags <- length(subf$TADA.CensoredData.Flag)
  expect_equal(out$Non_Detect_Pct, round((expected_nd_count / total_flags) * 100, 0))
})
...
test_that("TADA_Stats calls TADA_IDCensoredData when TADA.CensoredData.Flag is missing", {
  ...
  testthat::with_mocked_bindings(
    TADA_IDCensoredData = function(.data) {
      ...
      .data$TADA.CensoredData.Flag <- ifelse(.data$TADA.ResultMeasureValue < med, "Non-Detect", "Not-Censored")
      .data$DetectionQuantitationLimitTypeName[.data$TADA.CensoredData.Flag == "Non-Detect"] <- "DL_MOCK"
      .data
    },
    {
      out <- TADA_Stats(sub)
      ...
      nd_count <- sum(sub$TADA.ResultMeasureValue < stats::median(sub$TADA.ResultMeasureValue, na.rm = TRUE))
      total <- nrow(sub)
...
test_that("TADA_Stats ND_Estimation_Method logic across boundary scenarios (derived from random data)", {
  ...
  finalize_df <- function(df) {
    ...
    df$TADA.MonitoringLocationIdentifier[is.na(df$TADA.MonitoringLocationIdentifier)] <- "LocX"
    df$DetectionQuantitationLimitTypeName[is.na(df$DetectionQuantitationLimitTypeName)] <- "DLX"
    df
  }
  ...
  expect_equal(out_high$ND_Estimation_Method, "Percent censored too high for estimation methods")
  
  # Case B: 50% <= ND_Pct <= 80% and Measurement_Count >= 50 => MLE
  df_mle <- finalize_df(base[seq_len(60), , drop = FALSE])
  df_mle$TADA.CensoredData.Flag <- c(rep("Non-Detect", 45), rep("Not-Censored", 15)) # 75% ND
  df_mle$DetectionQuantitationLimitTypeName[df_mle$TADA.CensoredData.Flag == "Non-Detect"] <- "DL1"
  ...
  df_ros$TADA.CensoredData.Flag <- c(rep("Non-Detect", 30), rep("Not-Censored", 10)) # 75% ND
  df_ros$DetectionQuantitationLimitTypeName[df_ros$TADA.CensoredData.Flag == "Non-Detect"] <- "DL1"
  out_ros <- TADA_Stats(df_ros)
  expect_equal(out_ros$Measurement_Count, 40L)
  expect_equal(out_ros$Non_Detect_Pct, round(75, 1))
  expect_equal(out_ros$ND_Estimation_Method, "Robust Regression Order Statistics")
  ...
  df_km$TADA.CensoredData.Flag <- c(rep("Non-Detect", 9), rep("Not-Censored", 11)) # 45% ND
  dl_vals <- c("DL_A", "DL_B")
  df_km$DetectionQuantitationLimitTypeName[df_km$TADA.CensoredData.Flag == "Non-Detect"] <- rep(dl_vals, length.out = 9)
