...
test_that("TADA_IDCensoredData copies limits for BPQL/BDL/ND when present (dummy)", {
  ...
  df <- data.frame(
    ...
    TADA.DetectionQuantitationLimitMeasure.MeasureValue = c(0.11, 0.22, 0.33, 0.44),
    TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode = c("mg/L", "mg/L", "mg/L", NA_character_), # b4 unit missing
  ...
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "b1"], 0.11)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "b1"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "b1"],
                   "Result Value/Unit Copied from Detection Limit")
  
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "b2"], 0.22)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "b2"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "b2"],
                   "Result Value/Unit Copied from Detection Limit")
  
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "b3"], 0.33)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "b3"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "b3"],
                   "Result Value/Unit Copied from Detection Limit")
  ...
  expect_true(is.na(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "b4"]))
})
...
test_that("TADA_IDCensoredData does not introduce NAs in TADA.ResultMeasureValueDataTypes.Flag", {
  ...
  testdat <- TADA_ConvertSpecialChars(testdat, col = "TADA.ResultMeasureValue", clean = TRUE)
...
test_that("TADA_IDCensoredData handles NA/blanks and classifies censored data correctly", {
  ...
  test_df <- data.frame(
    ...
    ResultDetectionConditionText = c(
      cond_nd, cond_nd, cond_nd, cond_od, cond_nd, "   ", "FooBar", NA, cond_nd
    ),
    DetectionQuantitationLimitTypeName = c(
      lim_nd, NA, lim_nd, lim_od, lim_od, lim_nd, lim_od, NA, lim_nd
    ),
    TADA.ResultMeasureValueDataTypes.Flag = c(
      NA, NA, NA, NA, NA, NA, NA, NA, "NA - Not Available"
    ),
    TADA.ResultMeasureValue = c(NA, NA, NA, 10, NA, NA, NA, 2, NA),
    TADA.ResultMeasure.MeasureUnitCode = c(NA, NA, NA, "mg/L", NA, NA, NA, "mg/L", NA),
    TADA.DetectionQuantitationLimitMeasure.MeasureValue = c(0.5, NA, 0.2, NA, NA, 0.3, 1.1, NA, 0.1),
    TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode = c("mg/L", NA, "   ", NA, NA, "mg/L", "mg/L", NA, "mg/L"),
    TADA.MeasureQualifierCode.Flag = c(NA, "Non-Detect", "Non-Detect", "Over-Detect", "Non-Detect", "Non-Detect", "Non-Detect", NA, NA),
    stringsAsFactors = FALSE
  )
  ...
  expect_identical(get("r1", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Copied from Detection Limit")
  ...
  expect_identical(get("r2", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Cannot Be Estimated From Detection Limit")
  expect_identical(get("r2", "TADA.CensoredData.Flag"),
  ...
  expect_identical(get("r3", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Copied from Detection Limit")
  ...
  expect_identical(get("r5", "TADA.CensoredData.Flag"), "Conflict between Condition and Limit")
  expect_true(is.na(get("r5", "TADA.ResultMeasureValue")))
  expect_true(is.na(get("r5", "TADA.ResultMeasure.MeasureUnitCode")))
  expect_identical(get("r5", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Cannot Be Estimated From Detection Limit")
  ...
  expect_identical(get("r7", "TADA.CensoredData.Flag"), "Conflict between Condition and Limit")
  expect_true(is.na(get("r7", "TADA.ResultMeasureValue")))
  expect_true(is.na(get("r7", "TADA.ResultMeasure.MeasureUnitCode")))
  expect_identical(get("r7", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Cannot Be Estimated From Detection Limit")
  ...
  expect_identical(get("r9", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Copied from Detection Limit")
  ...
  expect_true(no_blanks(res$TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode))
...
test_that("ND override works with domain ND codes not in {ND,BDL,BPQL}", {
  ...
  res <- TADA_IDCensoredData(test_df)
  expect_identical(res$TADA.CensoredData.Flag, "Conflict between Condition and Limit")
  expect_true(is.na(res$TADA.ResultMeasureValue))
  expect_true(is.na(res$TADA.ResultMeasure.MeasureUnitCode))
  expect_identical(res$TADA.ResultMeasureValueDataTypes.Flag,
                   "Result Value/Unit Cannot Be Estimated From Detection Limit")
})
...
test_that("Early ND copy requires both limit pieces; NA-flag copy fills value when unit missing", {
  ...
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "c1"], 0.7)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "c1"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "c1"],
                   "Result Value/Unit Copied from Detection Limit")
  
  # c2: unit missing -> early copy does not fire, but NA-flag copy fills value; unit stays NA; flag set to Copied
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "c2"], 1.2)
  expect_true(is.na(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "c2"]))
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "c2"],
                   "Result Value/Unit Copied from Detection Limit")
})
...
test_that("NA-flag copy populates value and unit independently and sets flag", {
  df <- data.frame(
    ...
    TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode = c("mg/L", "mg/L", "mg/L"),
  ...
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "v_only"], 1)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "v_only"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "v_only"],
                   "Result Value/Unit Copied from Detection Limit")
  
  # u_only: only unit copied; value retained (5)
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "u_only"], 5)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "u_only"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "u_only"],
                   "Result Value/Unit Copied from Detection Limit")
  
  # both: both copied
  expect_identical(out$TADA.ResultMeasureValue[out$ResultIdentifier == "both"], 2)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode[out$ResultIdentifier == "both"], "mg/L")
  expect_identical(out$TADA.ResultMeasureValueDataTypes.Flag[out$ResultIdentifier == "both"],
                   "Result Value/Unit Copied from Detection Limit")
})
...
test_that("TADA_SimpleCensoredMethods fills ND/OD correctly and handles problematic records", {
  ...
  skip_if(length(cond_nd) == 0 || length(lim_nd) == 0 ||
            length(cond_od) == 0 || length(lim_od) == 0,
          "Reference tables incomplete for this test")
  
  cond_nd <- cond_nd[[1]]; lim_nd <- lim_nd[[1]]
  cond_od <- cond_od[[1]]; lim_od <- lim_od[[1]]
  ...
  df <- data.frame(
    ...
    ResultDetectionConditionText = c(cond_nd, cond_od, "   ", "FooBar", cond_nd, cond_nd),
    DetectionQuantitationLimitTypeName = c(lim_nd, lim_od, lim_nd, "FooLimit", lim_nd, lim_nd),
    TADA.ResultMeasureValueDataTypes.Flag = NA_character_,
    TADA.ResultMeasureValue = NA_real_,
    TADA.ResultMeasure.MeasureUnitCode = NA_character_,
    TADA.DetectionQuantitationLimitMeasure.MeasureValue = c(2.0, 10.0, 0.5, NA, 1.5, 4.0),
    TADA.DetectionQuantitationLimitMeasure.MeasureUnitCode = c("mg/L", "mg/L", "mg/L", NA, "mg/L", "mg/L"),
    TADA.MeasureQualifierCode.Flag = c("Non-Detect", "Over-Detect", "Non-Detect", "Non-Detect", "Non-Detect", "Non-Detect"),
    stringsAsFactors = FALSE
  )
  ...
  expect_identical(pick("t1", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Estimated from Detection Limit")
  expect_identical(pick("t1", "TADA.CensoredMethod"),
  ...
  expect_identical(pick("t2", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Estimated from Detection Limit")
  expect_identical(pick("t2", "TADA.CensoredMethod"),
  ...
  expect_identical(pick("t3", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Cannot Be Estimated From Detection Limit")
  expect_identical(pick("t3", "TADA.CensoredMethod"),
  ...
  expect_identical(pick("t4", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Cannot Be Estimated From Detection Limit")
  expect_identical(pick("t4", "TADA.CensoredMethod"),
  ...
  expect_identical(out_as_is$TADA.ResultMeasureValueDataTypes.Flag,
                   "Result Value/Unit Estimated from Detection Limit")
  expect_identical(out_as_is$TADA.CensoredMethod,
                   "Detection Limit Value Unchanged")
  ...
  expect_identical(out_rand$TADA.ResultMeasureValueDataTypes.Flag,
                   "Result Value/Unit Estimated from Detection Limit")
  expect_true(grepl("^Random Value Between 0 and Detection Limit Using this Multiplier:",
...
test_that("TADA_SimpleCensoredMethods validates parameters", {
  ...
  cond_od <- cond_ref$Name[cond_ref$TADA.Detection_Type == "Over-Detect"]
  lim_od  <- lim_ref$Name[lim_ref$TADA.Limit_Type == "Over-Detect"]
  skip_if(length(cond_nd) * length(lim_nd) * length(cond_od) * length(lim_od) == 0,
  ...
  expect_error(TADA_SimpleCensoredMethods(df, nd_method = "multiplier", nd_multiplier = "null"),
               "Please provide a multiplier")
  expect_error(TADA_SimpleCensoredMethods(df, nd_method = "multiplier", nd_multiplier = c(0.5, 0.2)),
               "single numeric value")
  
  # od_method = multiplier requires a numeric scalar
  expect_error(TADA_SimpleCensoredMethods(df, od_method = "multiplier", od_multiplier = "null"),
               "Please provide a multiplier")
  expect_error(TADA_SimpleCensoredMethods(df, od_method = "multiplier", od_multiplier = c(0.8, 0.9)),
               "single numeric value")
})
...
test_that("TADA_SimpleCensoredMethods applies OD multiplier correctly", {
  ...
  out <- TADA_SimpleCensoredMethods(
    df,
    nd_method = "multiplier", nd_multiplier = 0.5,  # irrelevant here
    od_method = "multiplier", od_multiplier = 0.8
  )
  ...
  expect_identical(pick("od1", "TADA.ResultMeasureValueDataTypes.Flag"),
                   "Result Value/Unit Estimated from Detection Limit")
  expect_identical(pick("od1", "TADA.CensoredMethod"),
...
test_that("TADA_SimpleCensoredMethods ND random is per-row and within bounds", {
  ...
  set.seed(123)
  out <- TADA_SimpleCensoredMethods(
    df, nd_method = "randombelowlimit",
    nd_multiplier = "null", od_method = "as-is", od_multiplier = "null"
  )
  ...
  expect_identical(unique(out$TADA.ResultMeasureValueDataTypes.Flag),
                   "Result Value/Unit Estimated from Detection Limit")
  expect_true(all(grepl("^Random Value Between 0 and Detection Limit Using this Multiplier:", 
...
test_that("TADA_SimpleCensoredMethods returns input unchanged when no censored data", {
  ...
  expect_message(
    ...
    "no censored data results", fixed = TRUE
  )
  expect_equal(nrow(out), nrow(df))
  expect_identical(out$TADA.ResultMeasureValue, df$TADA.ResultMeasureValue)
  expect_identical(out$TADA.ResultMeasure.MeasureUnitCode, df$TADA.ResultMeasure.MeasureUnitCode)
...
test_that("TADA_SimpleCensoredMethods respects existing TADA.CensoredData.Flag categories", {
  ...
  df <- data.frame(
    ...
    TADA.ResultMeasureValueDataTypes.Flag = c("Result Value/Unit Copied from Detection Limit",
                                              "Result Value/Unit Copied from Detection Limit"),
  ...
  expect_equal(out$TADA.ResultMeasureValue[out$ResultIdentifier == "n1"], 1.0, tolerance = 1e-12)
  expect_equal(out$TADA.ResultMeasureValue[out$ResultIdentifier == "o1"], 8.0, tolerance = 1e-12)
  expect_identical(unique(out$TADA.ResultMeasureValueDataTypes.Flag),
                   "Result Value/Unit Estimated from Detection Limit")
})
...
test_that("TADA_IDCensoredData flags missing detection condition when ND override does not apply", {
  ...
  expect_identical(res$TADA.CensoredData.Flag,
...
test_that("TADA_IDCensoredData flags 'Other Condition/Limit Populated' when both are Other", {
  ...
  skip_if(length(cond_other) == 0 || length(lim_other) == 0,
  ...
  res <- TADA_IDCensoredData(df)
  expect_identical(res$TADA.CensoredData.Flag, "Other Condition/Limit Populated")
})
