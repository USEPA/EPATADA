...
to_mdy <- function(x) {
  d <- try(as.Date(sub("\\s+.*$", "", as.character(x)), format = "%m/%d/%Y"), silent = TRUE)
  suppressWarnings(as.Date(d))
}

# Minimal data.frame with named columns
make_df <- function(cols) {
  as.data.frame(setNames(as.list(rep_len("", length(cols))), cols), stringsAsFactors = FALSE)
}
...
make_cst_xlsx <- function(
  ...
  if (!is.null(sheet_names)) names(x) <- sheet_names
...
testthat::test_that("Is TADA_GetWQXCharAliasRef up to date?", {
  skip_on_cran()
  skip_if_offline()
  file_path <- system.file("extdata", "WQXCharAliasRef.rda", package = "EPATADA")
  e <- new.env(parent = emptyenv())
  load(file_path, envir = e)
...
testthat::test_that("DetCondRef fallback path respects quiet", {
  ...
  testthat::local_mocked_bindings(
    .tada_read_csv_url = function(...) NULL,
    .tada_load_extdata_rda = function(...) make_df(c("Name","Last.Change.Date")),
    .env = ns
  )
  # quiet = FALSE -> message
  ...
  testthat::expect_silent(
    EPATADA::TADA_GetDetCondRef(refresh = TRUE, quiet = TRUE)
  )
})
...
testthat::test_that("CharacteristicRef normalizes expected columns and trims", {
  ...
  out <- EPATADA::TADA_GetCharacteristicRef(download_only = FALSE, refresh = TRUE)
  testthat::expect_true(all(c("CharacteristicName","Char_Flag","Comparable.Name","CAS.Number") %in% names(out)))
...
testthat::test_that("CharacteristicRef falls back when structure unexpected", {
  ...
  fb  <- data.frame(CharacteristicName = "X", Char_Flag = "A", stringsAsFactors = FALSE)
  testthat::local_mocked_bindings(
...
testthat::test_that("WQP Organization: column selection + fallback", {
  ...
  testthat::expect_identical(names(out), c("OrganizationIdentifier","OrganizationFormalName","ProviderName"))
  ...
  testthat::local_mocked_bindings(
    .tada_read_csv_url = function(...) NULL,
    .tada_load_extdata_rda = function(...) live[, c("OrganizationIdentifier","OrganizationFormalName","ProviderName")],
    .env = ns
  )
...
testthat::test_that("Detection Condition classifier assigns expected flags + legacy rows", {
  ...
  df <- data.frame(
    Name = c("Not Detected", "Present Above Quantification Limit", "Value Decensored", NA),
    Description = "x",
    stringsAsFactors = FALSE
  )
  out <- f(df)
  ...
  testthat::expect_true(all(c("*Non-detect","*Present <QL","*Present") %in% out$Name))
})

testthat::test_that("Detection Limit Type classifier assigns expected flags", {
  f <- get(".TADA_flag_DetLimitRef", envir = ns)
  df <- data.frame(
    Name = c("Method Detection Level", "Upper Reporting Limit", "Measurement Uncertainty", NA),
    Description = "x",
    stringsAsFactors = FALSE
  )
  out <- f(df)
...
testthat::test_that("ActivityType classifier by Code handles QC bins and Non_QC", {
  ...
  testthat::expect_equal(m[["Quality Control Sample-Inter-lab Split"]], "QC_duplicate")
  testthat::expect_equal(m[["Quality Control Field Calibration Check"]], "QC_calibration")
...
testthat::test_that("Monitoring Location Type flag assigns Surface Water / Groundwater", {
  ...
  df <- data.frame(Name = c("Lake","Well","Other-Surface Water","Other-X"), stringsAsFactors = FALSE)
  out <- f(df)
...
testthat::test_that("Measure Qualifier Code classifier flags Pass/Suspect/Non-Detect/Over-Detect and NA -> Pass", {
  ...
  out <- f(df)
  m <- setNames(out$TADA.MeasureQualifierCode.Flag, ifelse(is.na(out$Code), "<NA>", out$Code))
  ...
  testthat::expect_true(any(out$TADA.MeasureQualifierCode.Flag == "Pass" & is.na(out$Code)))
})
...
testthat::test_that("Getters read, normalize, and cache results (classic and fuzzy names)", {
  ...
  legend_in <- data.frame(A = c(" a ", "a "), B = c(" x ", "x "), stringsAsFactors = FALSE)
  sources_in <- data.frame(Src = c(" s1 ", "s1 "), More = c(" y  ", "y"), stringsAsFactors = FALSE)
  criteria_in <- data.frame(C1 = c(" val ", "val "), C2 = c(" z ", "z "), stringsAsFactors = FALSE)
  
  expected_legend <- unique(data.frame(A = "a", B = "x", stringsAsFactors = FALSE))
  expected_sources <- unique(data.frame(Src = "s1", More = "y", stringsAsFactors = FALSE))
  expected_criteria <- unique(data.frame(C1 = "val", C2 = "z", stringsAsFactors = FALSE))
  
  wb_path <- make_cst_xlsx(
    legend_in, sources_in, criteria_in,
...
testthat::test_that("download_only = TRUE returns data but does not populate cache", {
  ...
  wb_path <- make_cst_xlsx(
    legend_df = data.frame(A = "  foo "), sources_df = data.frame(Src = " bar "),
    criteria_df = data.frame(C1 = " baz "), sheet_names = c("Legend", "Sources", "Criteria")
  )
  mock_resolver_to_path(wb_path)
...
testthat::test_that("refresh = TRUE bypasses cached data and updates it", {
  ...
  current_path <- wb1
  testthat::local_mocked_bindings(
    .tada_cst_get_workbook_path = function(download_only = FALSE, refresh = FALSE, ...) {
      if (!download_only) get(".tada_cache_set", envir = ns)("CST_workbook_path", current_path)
      current_path
    },
    .env = ns
  )
  ...
  testthat::expect_equal(get_cache_value("CST_legend_df"), data.frame(A = "new", stringsAsFactors = FALSE))
})
...
testthat::test_that("Resolves new CST naming: base=(legend), (2)=(sources), (3)=(criteria)", {
  ...
  testthat::expect_equal(l, data.frame(Lcol = "L_new", stringsAsFactors = FALSE))
  testthat::expect_equal(s, data.frame(Scol = "S_new", stringsAsFactors = FALSE))
  testthat::expect_equal(c, data.frame(Ccol = "C_new", stringsAsFactors = FALSE))
})
...
testthat::test_that("Errors when sheet names are unrecognized (no index fallback)", {
  ...
  testthat::expect_error(EPATADA::TADA_CST_GetLegend(refresh = TRUE), "Failed to read Legend sheet")
  testthat::expect_error(EPATADA::TADA_CST_GetSources(refresh = TRUE), "Failed to read Sources sheet")
  testthat::expect_error(EPATADA::TADA_CST_GetCriteria(refresh = TRUE), "Failed to read Criteria sheet")
})
...
testthat::test_that("Falls back to internal workbook when download fails", {
  ...
  testthat::expect_equal(out, data.frame(C1 = "from_fallback", stringsAsFactors = FALSE))
})
...
testthat::test_that(".TADA_CST_UpdateWorkbook delegates to write helper", {
  ...
  testthat::local_mocked_bindings(
    .tada_cst_get_workbook_path = function(download_only = TRUE, refresh = TRUE, ...) fake_src,
    .tada_cst_write_ext_workbook_if_changed = function(
    src_path, pkg = "EPATADA", filename = "cst-workbook.xlsx", normalize_tabs = TRUE
    ) {
      called <<- TRUE
...
testthat::test_that("ReportDateTime is extracted from Legend", {
  ...
  wb <- make_cst_xlsx(legend_df = legend, sources_df = data.frame(S=1), criteria_df = data.frame(C=1))
...
testthat::test_that("Normalized copy renames new CST naming to classic", {
  ...
  wb <- make_cst_xlsx(
    ...
    sheet_names = c("Search Tool Criteria Data", "Search Tool Criteria Data (2)", "Search Tool Criteria Data (3)")
  )
...
testthat::test_that("Workbook path is cached and reused when refresh = FALSE", {
  testthat::skip_if_not_installed("openxlsx")
  reset_cst_cache()
  wb <- make_cst_xlsx(legend_df = data.frame(L=1), sources_df = data.frame(S=1), criteria_df = data.frame(C=1))
...
testthat::test_that("CST getters error clearly when workbook path cannot be retrieved", {
  ...
  testthat::expect_error(EPATADA::TADA_CST_GetLegend(refresh = TRUE), "Failed to retrieve CST workbook")
  testthat::expect_error(EPATADA::TADA_CST_GetSources(refresh = TRUE), "Failed to retrieve CST workbook")
  testthat::expect_error(EPATADA::TADA_CST_GetCriteria(refresh = TRUE), "Failed to retrieve CST workbook")
})

testthat::test_that("RDA writer saves and skips when unchanged", {
  tmp_pkg <- withr::local_tempdir()
  dir.create(file.path(tmp_pkg, "inst", "extdata"), recursive = TRUE, showWarnings = FALSE)
  ...
  testthat::expect_message(f(df1, obj_name = "OBJ", pkg = "EPATADA", filename = "obj.rda"), "No changes detected")
  
  df2 <- transform(df1, a = 2)
  testthat::expect_message(f(df2, obj_name = "OBJ", pkg = "EPATADA", filename = "obj.rda"), "saved to")
})
...
testthat::test_that("Is the saved ATTAINSOrgIDsRef up to date (live domain subset in fallback)?", {
  ...
  file_path <- system.file("extdata", "ATTAINSOrgIDsRef.rda", package = "EPATADA")
  e <- new.env(parent = emptyenv())
  load(file_path, envir = e)
...
testthat::test_that("WQXcharValRef has unique characteristic/media/unit/max/min rows for threshold functions", {
  ...
  unit.ref <- dplyr::filter(e$WQXcharValRef, Type == "CharacteristicUnit", Status == "Accepted")
  find.dups <- unit.ref |>
