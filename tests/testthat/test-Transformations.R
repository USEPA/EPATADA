...
make_dummy_nutrient_df <- function() {
  ...
  data.frame(
    ActivityStartDate = as.Date(c("2025-01-01","2025-01-01","2025-01-01","2025-01-02")),
    TADA.MonitoringLocationIdentifier = c("SITE-001","SITE-001","SITE-001","SITE-001"),
    ...
    TADA.ResultSampleFractionText = c("UNFILTERED","UNFILTERED","UNFILTERED","UNFILTERED"),
    TADA.ResultMeasure.MeasureUnitCode = c("MG/L","MG/L","UG/L","UG/L"),
    TADA.ResultMeasureValue = c(0.2, 0.5, 10, 30),
    ActivityTypeCode = c("Field Msr/Obs","Field Msr/Obs","Field Msr/Obs","Field Msr/Obs"),
    ...
    MonitoringLocationTypeName = c("River/Stream","River/Stream","River/Stream","River/Stream"),
    MonitoringLocationDescriptionText = c("Upstream reach","Upstream reach","Upstream reach","Upstream reach"),
    ...
    TADA.ResultMeasureValueDataTypes.Flag = c("Numeric","Numeric","Numeric","Numeric"),
...
make_dummy_non_np_df <- function() {
  ...
  data.frame(
    ...
    TADA.CharacteristicName = c("Dissolved oxygen (DO)","Dissolved oxygen (DO)"),
    ...
    MonitoringLocationDescriptionText = c("Downstream reach","Downstream reach"),
...
test_that("TADA package functions maintain ResultIdentifier integrity (dummy)", {
  ...
  expect_equal(length(missing_identifiers), 0L, info = paste(
    "Missing identifiers:", paste(missing_identifiers, collapse = ", ")
  ))
...
test_that("daily_agg blank (NULL) treated as 'max' (dummy, full path)", {
  ...
  out_null_ord <- dplyr::arrange(out_null, dplyr::across(dplyr::all_of(keycols)))
