test_that("No NA's in dependent flag columns", {
  ...
  testdat = suppressWarnings(TADA_InvalidFraction(testdat, clean = FALSE, errorsonly=FALSE))
  expect_false(any(is.na(testdat$TADA.SampleFraction.Flag)))
  
  testdat = suppressWarnings(TADA_InvalidSpeciation(testdat, clean = "none", errorsonly=FALSE))
  expect_false(any(is.na(testdat$TADA.MethodSpeciation.Flag)))
  
  testdat = suppressWarnings(TADA_InvalidResultUnit(testdat, clean = "none", errorsonly=FALSE))
