...
test_that("TADA_UpdateTribalLayers writes shapefiles, caches signature, and skips when unchanged", {
  ...
  withr::local_options(
    TADA.tribal.meta_dir = file.path("inst/extdata", ".meta")
  )
  ...
  make_sf <- function(n = 2, offset = 0L) {
    ...
    xy <- data.frame(
      id = seq_len(n),
      DATE_MO = as.numeric(as.POSIXct("2020-01-01 00:00:00", tz = "UTC")) *
        1000 +
        offset +
        (seq_len(n) - 1) * 86400000,
      CURRENT = as.numeric(as.POSIXct("2020-06-01 00:00:00", tz = "UTC")) *
        1000 +
        offset,
