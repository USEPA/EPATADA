# tests/testthat/test-update-tribal-layers.R

test_that("TADA_UpdateTribalLayers writes shapefiles, caches signature, and skips when unchanged", {
  skip_if_not_installed("sf")
  # Ensure EPATADA namespace exists and the internal URL symbols are present
  ...
  url_syms <- c("AKAllotmentsUrl", "AKVillagesUrl", "AmericanIndianUrl",
                "OffReservationUrl", "OKTribeUrl", "VATribeUrl")
  for (sym in url_syms) {
  ...
  # Helper: build a small sf layer with epoch-ms date fields
  make_sf <- function(n = 2, offset = 0L) {
    # Two points with attributes; DATE_MO in epoch ms around 2020
    xy <- data.frame(
      id = seq_len(n),
      DATE_MO = as.numeric(as.POSIXct("2020-01-01 00:00:00", tz = "UTC")) * 1000 + offset + (seq_len(n) - 1) * 86400000,
      CURRENT = as.numeric(as.POSIXct("2020-06-01 00:00:00", tz = "UTC")) * 1000 + offset,
      name = sprintf("feat_%d", seq_len(n))
    )
    sf::st_as_sf(xy, coords = c("id", "DATE_MO"), crs = 4326, remove = FALSE) # bogus geometry just for testing
  }
  
  # Use proper geometry instead of mixing columns: safer small points
  make_points_sf <- function(n = 2, offset = 0L) {
    pts <- sf::st_sfc(
      lapply(seq_len(n), function(i) sf::st_point(c(-120 + i * 0.01, 38 + i * 0.01))),
      crs = 4326
    )
    df <- data.frame(
      id = seq_len(n),
  ...
  for (sym in url_syms) {
    # unlock binding if locked
    if (bindingIsLocked(sym, ns)) unlockBinding(sym, ns)
    assignInNamespace(sym, src1, ns = "EPATADA")
  }
  
  # 1) First run: should write all shapefiles and create meta sidecars
  expect_message(TADA_UpdateTribalLayers(), "updated\\.", fixed = FALSE)
  ...
  for (d in dests) {
    ...
    expect_true(file.exists(meta_file), info = paste("meta sidecar missing:", meta_file))
    
    # meta signature should exist and date fields be Date class after conversion
    meta <- readRDS(meta_file)
    expect_true(is.list(meta))
    expect_true(!is.null(meta$sig))
    sig <- meta$sig
    expect_s3_class(sig, "data.frame")
    if ("DATE_MO" %in% names(sig)) {
      expect_s3_class(sig$DATE_MO, "Date")
    }
    if ("CURRENT" %in% names(sig)) {
      expect_s3_class(sig$CURRENT, "Date")
    }
    ...
    if ("DATE_MO" %in% names(x)) {
      expect_s3_class(x$DATE_MO, "Date")
    }
    if ("CURRENT" %in% names(x)) {
      expect_s3_class(x$CURRENT, "Date")
    }
  }
  
  # 2) Second run unchanged: should skip write based on canonical signature
  expect_message(TADA_UpdateTribalLayers(), "unchanged — skipping write\\.", fixed = FALSE)
  ...
  expect_message(TADA_UpdateTribalLayers(), "AKAllotments.shp updated\\.", fixed = FALSE)
  ...
  ak <- suppressWarnings(sf::st_read("inst/extdata/AKAllotments.shp", quiet = TRUE))
