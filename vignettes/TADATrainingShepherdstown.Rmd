...
list.of.packages <- c("tidyverse", "writexl")
...
If you are interested in reviewing the column headers and formats required to run TADA, use the function below, which saves an example spreadsheet to the user's working directory. You can also take a look at an example dataset, like `TADA::Nutrients_Utah` to get an idea of the data structure and format.
...
# Nutrients_Utah <- TADA::Nutrients_Utah 
```

`TADAdataRetrieval` is built upon USGS's `readWQPdata` function within the dataRetrieval package, which uses web service calls to bring WQP data into the R environment. Additionally, `TADAdataRetrieval` performs some basic quality control checks on the data using new TADA-specific columns to preserve the original dataset:
...
Let's give it a try. `TADAdataRetrieval` follows similar parameterization to `readWQPdata`, but check out the [help page](https://usepa.github.io/TADA/reference/TADAdataRetrieval.html) or enter ?TADA::TADAdataRetrieval into the console for more information about input parameters and to see several examples.
...
Next, we can use the TADA `fieldCounts()` function to see how many unique values are contained within each column of the dataset. The function can either return all column counts, most, or just the key columns. We'll try the input with `display = "key"` and `display = "all"`. Enter ?TADA::fieldCounts() into the console for more information on this function.
...
Two additional helper functions one can use at any step in the process are `fieldValuesTable()` and `fieldValuesPie()`. These functions create a summary table and pie chart (respectively) of all the unique values in a given column. Let's give it a try on ActivityTypeCode, which is a WQP column describing the type of sample collected for each result.

```{r}
TADA::fieldValuesTable(dataset,field="OrganizationFormalName")
TADA::fieldValuesPie(dataset,field="OrganizationFormalName")
```
...
incompatible_datatype = dataset%>%filter(!TADA.ResultMeasureValueDataTypes.Flag%in%c("Numeric","Less Than","Greater Than","Approximate Value","Percentage","Comma-Separated Numeric","Numeric Range - Averaged","Result Value/Unit Copied from Detection Limit"))%>%dplyr::mutate(TADA.RemovalReason = "Result value type cannot be converted to numeric or no detection limit values provided.")
...
dataset_cens%>%dplyr::group_by(TADA.CharacteristicName)%>%dplyr::summarise(Result_Count = length(ResultIdentifier), Site_Count = length(unique(MonitoringLocationIdentifier)), Org_Count = length(unique(OrganizationIdentifier)))%>%dplyr::arrange(desc(Result_Count))%>%print(n = nrow)
```
...
TADA::fieldValuesPie(dataset_cens, field = "TADA.ResultSampleFractionText", characteristicName = "NITROGEN, MIXED FORMS (NH3), (NH4), ORGANIC, (NO2) AND (NO3)")
```
...
```{r}
TADA::fieldValuesPie(dataset_cens, field = "MonitoringLocationTypeName")
```
...
View(TADA::fieldValuesTable(dataset_cens, field = "TADA.ComparableDataIdentifier"))
...
```{r}
TADA::TADA_Histogram(plot_data, id_col = "TADA.ComparableDataIdentifier")
```
...
Finally, we can download our PASS and FAIL datasets together into an Excel spreadsheet to see all of the TADA-related columns together. TADA-generated columns will occur at the far end of the dataset.
