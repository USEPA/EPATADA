...
## Subset and generate statistics for results of interest
```{r}
...
```{r}
TADA_TwoCharacteristicScatterplot(GreenBay_FoxRiver_Subset, id_cols = "TADA.ComparableDataIdentifier", groups = c("TOTAL PHOSPHORUS, MIXED FORMS_UNFILTERED_AS P_UG/L", 
      "TOTAL NITROGEN, MIXED FORMS_UNFILTERED_AS N_MG/L", 
      "DISSOLVED OXYGEN (DO)_NA_NA_MG/L"))
```
...
## Creating new AUs to assess additional areas (leveraging USGS's nhdplusTools)
...
-   Placeholder: Do we want to include anything else here from StreamCatTools or other packages to help with new AU delineation?
