...
footer: EPATADA, dataRetrieval, nhdplusTools, StreamCatTools, hydroloom, spsurvey,
  spmodel, SSN2, sf, prism, terra, leaflet and tmap
---
...
Install and load the [EPATADA R Package](https://github.com/USEPA/EPATADA) and example data for this workshop from GitHub.
...
# remotes::install_github("mhweber/NMC-Workshop-Data",force = TRUE)
...
We create a unique identifier based on shared lat lon values and filter to just the 26 unique locations
...
We use `StreamCatTools` function `sc_get_comid` (which uses `nhdplusTools`) to get the comid for each location

```{r}
library(StreamCatTools)
GreenBay_FoxRiver_sf_locs$COMID <- sc_get_comid(GreenBay_FoxRiver_sf_locs)
```

## Placeholder for dataRetrieval/NLDI, nhdplusTools, hydroloom (Dave)
