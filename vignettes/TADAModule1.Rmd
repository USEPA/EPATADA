...
The TADA **TADA_InvalidCoordinates** function identifies and flags
...
```{r TADA_InvalidCoordinates}
# flag only
TADAProfileClean1 <- TADA_InvalidCoordinates(TADAProfile, clean_outsideUSA = "no", clean_imprecise = FALSE, errorsonly = FALSE)
...
# TADAProfileClean1 <- TADA_InvalidCoordinates(TADAProfile, clean_outsideUSA = "change sign", clean_imprecise = FALSE, errorsonly = FALSE)
```
...
The **TADA_AggregatedContinuousData** function checks for and removes
...
the following code in the console: ?TADA_AggregatedContinuousData

```{r AggregatedContinuousData}
TADAProfileClean1 <- TADA_AggregatedContinuousData(TADAProfileClean1, clean = FALSE)

# uncomment below to create a dataframe of only the aggregated continuous data

# TADAProfile_aggcont <- TADA_AggregatedContinuousData(TADAProfileClean3, clean = FALSE, errorsonly = TRUE)
```
...
TADAProfileClean2 <- TADA_InvalidMethod(TADAProfileClean1, clean = TRUE)

TADAProfileClean2 <- TADA_InvalidFraction(TADAProfileClean2, clean = TRUE)

TADAProfileClean2 <- TADA_InvalidSpeciation(TADAProfileClean2, clean = "invalid_only")

TADAProfileClean2 <- TADA_InvalidResultUnit(TADAProfileClean2, clean = "invalid_only")
```
...
-   ?**TADA_AboveNationalWQXUpperThreshold**
...
-   ?**TADA_BelowNationalWQXLowerThreshold**
...
```{r WQX_Thresholds}
TADAProfileClean3 <- TADA_AboveNationalWQXUpperThreshold(TADAProfileClean2, clean = TRUE)

TADAProfileClean3 <- TADA_BelowNationalWQXLowerThreshold(TADAProfileClean3, clean = TRUE)
```
...
The **TADA_QAPPapproved** function checks data for an approved QAPP.
...
```{r QAPPapproved}
TADAProfileClean3 <- TADA_QAPPapproved(TADAProfileClean3, clean = FALSE, cleanNA = FALSE)
```

The **TADA_QAPPDocAvailable** function checks to see if a QAPP Doc is
Available
...
```{r TADA_QAPPDocAvailable}
TADAProfileClean3 <- TADA_QAPPDocAvailable(TADAProfileClean3,
                                       clean = FALSE)
```
...
TADA_QualityControlActivity function.

```{r TADA_QualityControlActivity}
# Remove all QC samples using the TADA_QualityControlActivity function:
# enter ?TADA_QualityControlActivity into the console for more information
TADAProfileClean4 <- TADA_QualityControlActivity(TADAProfileClean3, clean = TRUE)
...
The **TADA_HarmonizeData** function then compares the input dataframe to the
...
Here are some examples of how the TADA_HarmonizeData function can be used:
...
2.  Some variables have different names but represent the same
    ...
    "Kjeldahl nitrogen"). The TADA_HarmonizeData function gives a consistent
    name (and identifier) to synonyms.

```{r TADA_HarmonizeData}
UniqueHarmonizationRef <- TADA_HarmonizationRefTable(TADAProfileClean5,
                                                download = FALSE)

TADAProfileClean5 <- TADA_HarmonizeData(TADAProfileClean5,
