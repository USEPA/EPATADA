...
You can also use the **TADA_AutoFilter()** function to remove any
columns not required for the TADA workflow that contain only NAs.

```{r remove_cols}
TADAProfileClean5 <- TADA_AutoFilter(TADAProfileClean5)
```
...
Filter dataframe and generate scatterplots for each selected TADA.ComparableDataIdentifier: 
```{r}
TADAProfileClean6_filtered <- TADAProfileClean6[
  TADAProfileClean6$TADA.ComparableDataIdentifier %in% 
    c("ARSENIC_TOTAL_NA_UG/L",
...
TADAProfileCleanTP <- dplyr::filter(TADAProfileClean6, 
...
TP_Histogram <- TADA_Histogram(TADAProfileCleanTP, id_cols = 
...
TP_Boxplot <- TADA_Boxplot(TADAProfileCleanTP, id_cols = 
                             "TADA.ComparableDataIdentifier")
