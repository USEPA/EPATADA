...
description: An overview of TADA Module 1 Data Discovery and Cleaning functions and a recommended workflow.
editor_options:
...
TADAProfile_raw <- TADA_DataRetrieval(organization = c("REDLAKE_WQX", "SFNOES_WQX", "PUEBLO_POJOAQUE", "FONDULAC_WQX", "PUEBLOOFTESUQUE", "CNENVSER"), startDate = "2018-01-01", endDate = "2019-01-01", applyautoclean = FALSE, ask = FALSE)
```
...
```{r TADA_DataRetrieval_single, , eval = F}
...
```{r TADA_DataRetrieval_tribal, eval = F}
...
```{r TADA_DataRetrieval_spatial, eval = F}
...
```{r TADA_DataRetrieval_tribal_2, eval = F}
...
```{r, include = F, eval = F}
rm(TADAProfile_single_2)
rm(TADAProfile_tribal_2)
```
...
as groundwater. This function identifies surface water, groundwater, and 
sediment results.

```{r AnalysisDataFilter}
# Retain only surface water results
TADAProfile <- TADA_MediaFilter(TADAProfile_raw,
  clean = TRUE,
  surface_water = FALSE,
  ground_water = TRUE,
  sediment = TRUE,
...
```{r, include = F}
rm(Unique_SuspectCoordinateFlags)
```
...
```{r FindPotentialDuplicates_single}
...
```{r FindPotentialDuplicates_multiple, eval = F}
...
Users could alternatively remove both Ns and NAs using the inputs
...
it becomes impossible to have a low RPD. In this scenario, high RPDs
