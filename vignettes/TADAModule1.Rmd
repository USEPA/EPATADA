...
remotes::install_github("USEPA/TADA",
  ref = "336-original-vs-tada-columns-copies",
  dependencies = TRUE
)
```
...
Review all column names in the TADA Profile. Remove results that you do not need for analysis. TADA was primarily designed to work with water data. You could remove all non-water results through filtering by TADA.ActivityMediaName. Uncomment below if you would like to try this option.
...
Many TADA users are interested in using WQP data for Clean Water Act Assesssments. The **TADA_AssessmentDataFilter** function may assist in identifying results that should be included in the assessment process. 

This function identifies surface water, groundwater, and sediment results. Users can  specify whether all results should be returned with a new column, *TADA.UseForAssessment.Flag*,identifying if the result should be included in assessments or if only results that should be in included in assessments are returned. 

The defaults are to include surface water, exclude groundwater and sediment, and to return only the results that should be used for assessments (clean = TRUE). This is shown in the active example below.

If you would like to see all results with the *TADA.UseForAssessment.Flag* column, you can uncomment the example where clean = FALSE.

```{r TADA_AssessmentDataFilter}

# Filter to retain only results for use in assessments
TADAProfile <- TADA_AssessmentDataFilter(TADAProfile)

# Add TADA.UseForAssessment.Flag column to identify which results should be used for assessments
# TADAProfile <- TADA_AssessmentDataFilter(TADAProfile)
...
Now **TADA_AutoClean** can be run on smaller dataset after unnecessary results have been removed. It performs the following functions on the data retrieved from the WQP:
...
[here](https://cdx.epa.gov/wqx/download/DomainValues/ResultMeasureQualifier.CSV){style="font-size: 12pt;"}. TADA_FlagMeasureQualifierCode will automatically add applicable definitions to the input dataframe in a new column
called "TADA.MeasureQualifierCode.Def". 


In this example, we show how to use the function TADA_FlagMeasureQualifierCode to add definitiond and flag and/or remove rows with specific codes under MeasureQualifierCode that are categorized as "SUSPECT". 
...
```{r filter_out_NAs}
# Removes rows where the result value is not numeric. Specifically, removes rows with "Text" or "NA - Not Available" in the TADA.ResultMeasureValueDataTypes.Flag column, or NA in the TADA.ResultMeasureValue column.
