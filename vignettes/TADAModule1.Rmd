...
The defaults are to include surface water, exclude groundwater and sediment, and to return only the results that should be used for assessments (clean = TRUE). This is shown in the active example below. If you would like to see all results with the *TADA.UseForAssessment.Flag* column, you can uncomment the example where clean = FALSE.

If you are not interested in using **TADA_AssessmentDataFilter**, but would like to filter by activity media, uncomment the example to filter for water data only by using dplyr::filter() with TADA.ActivityMediaName.
...
# Remove data for non-water media types, alternate workflow without using TADA_AssessmentDataFilter()
