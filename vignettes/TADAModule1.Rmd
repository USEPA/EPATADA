...
This vignette will walk through how to discover and clean (i.e.,
wrangle, Quality Assure and Quality Control (QAQC), and harmonize)
...
This example includes monitoring data collected from Jan 2018 to Jan 2019
by six organizations: 1) Red Lake Band of Chippewa Indians, 2) Sac & Fox
Nation, 3) Pueblo of Pojoaque, 4) Minnesota Chippewa Tribe (Fond du Lac
Band), 5) Pueblo of Tesuque, and 6) The Chickasaw Nation
...
Enter ?TADAdataRetrieval into the console to review more example queries
...
and the TADAdataRetrieval function is not working due to WQP timeout
...
See ?TADABigdataRetrieval for more details. WARNING, some of the
...
**TADAOverviewMap** function. **TADAOverviewMap** counts the number of
...
# TADAProfileClean1 <- InvalidCoordinates(TADAProfile, clean_outsideUSA = "change sign", clean_imprecise = FALSE, errorsonly = FALSE)
```
...
TADAProfileClean1 <- ConvertDepthUnits(TADAProfileClean1,
                                       unit = "m",
                                       transform = TRUE)
```
...
The **TADA_ConvertResultUnits** function converts all results to WQX target
...
See additional function documentation for additional function options by
entering the following code in the console: ?TADA_ConvertResultUnits

```{r TADA_ConvertResultUnits}
#Converts all results to WQX target units
TADAProfileClean1 <- TADA_ConvertResultUnits(TADAProfileClean1, transform = TRUE)
```
...
TADAProfileClean1 <- AggregatedContinuousData(TADAProfileClean1, clean = FALSE)
...
TADAProfileClean2 <- InvalidMethod(TADAProfileClean1, clean = TRUE)

TADAProfileClean2 <- InvalidFraction(TADAProfileClean2, clean = TRUE)

TADAProfileClean2 <- InvalidSpeciation(TADAProfileClean2, clean = "invalid_only")

TADAProfileClean2 <- InvalidResultUnit(TADAProfileClean2, clean = "invalid_only")
```
...
```{r WQX_Thresholds}
TADAProfileClean3 <- AboveNationalWQXUpperThreshold(TADAProfileClean2, clean = TRUE)

TADAProfileClean3 <- BelowNationalWQXLowerThreshold(TADAProfileClean3, clean = TRUE)
```
...
TADAProfileClean3 <- PotentialDuplicateRowID(TADAProfileClean3, clean = TRUE, errorsonly = FALSE)
```
...
```{r QAPPapproved}
TADAProfileClean3 <- QAPPapproved(TADAProfileClean3, clean = FALSE, cleanNA = FALSE)
```
...
```{r QAPPDocAvailable}
TADAProfileClean3 <- QAPPDocAvailable(TADAProfileClean3,
                                       clean = FALSE)
```
...
number of unique values in each field using the **fieldCounts**
function.
...
fieldCounts(TADAProfileClean3)

# create object of table
fieldCounts_Table <- fieldCounts(TADAProfileClean3)

```

Next, choose a field from the list generated above to view a summary table or pie chart of the counts of unique values in that field using **fieldValuesTable** or **fieldValuesPie**. We'll start with ActivityTypeCode.

```{r fieldValues_all, fig.width=6, fig.height=2, fig.fullwidth=TRUE}
fieldValuesTable(TADAProfileClean3, field = "ActivityTypeCode")
fieldValuesPie(TADAProfileClean3, field = "ActivityTypeCode")
```

The ActivityTypeCode field has multiple unique values. In this example
we remove Quality Control (QC) values using the TADA
QualityControlActivity function.

```{r QualityControlActivity}
...
TADAProfileClean4 <- QualityControlActivity(TADAProfileClean3, clean = TRUE)
...
fieldValuesTable(TADAProfileClean4, "ActivityTypeCode")
fieldValuesPie(TADAProfileClean4, "ActivityTypeCode")
...
fieldValuesPie(TADAProfileClean4, "MeasureQualifierCode")
```
...
# regenerate table and pie chart
fieldValuesPie(TADAProfileClean4, field = "MeasureQualifierCode")
```
...
*ResultDetectionConditionText* and *DetectionQuantitationLimitTypeName*.
This utility function is automatically run  within the TADAdataRetrieval function and produces the *TADA.CensoredData.Flag* column. All records receive one of the following
...
the other suggests non-detect. - Detection condition or detection limit is not documented in TADA reference tables. -
detection condition or limit type is not characterized in the TADA
reference tables, which are based on WQX domain tables. - Detection condition is missing and required for censored data ID. - Result needs more information before being categorized.
...
TADAProfileClean_censoredSummary <- summarizeCensoredData(TADAProfileClean4, spec_cols = c("TADA.CharacteristicName", "TADA.ResultMeasure.MeasureUnitCode", "TADA.ResultSampleFractionText", "TADA.MethodSpecificationName"))

TADAProfileClean_censoredSummary

TADAProfileClean4 = simpleCensoredMethods(TADAProfileClean4,
...
```{r HarmonizeData}
UniqueHarmonizationRef <- HarmonizationRefTable(TADAProfileClean5,
                                                download = FALSE)

TADAProfileClean5 <- HarmonizeData(TADAProfileClean5,
...
**fieldValuesTable** function.

Enter ?fieldValuesTable into the console for more information.

```{r fieldValuesTable_chars}
fieldValuesTable(TADAProfileClean5, field = "TADA.CharacteristicName")
```

Next, we can revisit the **fieldCounts** function at the characteristic level to review how many unique allowable values are included within each of the following fields:
...
```{r fieldCounts_char}
fieldCounts(TADAProfileClean5, display = "most", characteristicName = "TOTAL PHOSPHORUS, MIXED FORMS")
```
...
Now, we'll use **fieldValuesTable** and **fieldValuesPie** at the characteristic-level to review a column of interest.

```{r fieldValuesTable_Pie_char, fig.width=6, fig.height=2, fig.fullwidth=TRUE}
# In this example we review values from the SampleCollectionMethod.MethodName field
fieldValuesTable(TADAProfileClean5, field = "SampleCollectionMethod.MethodName",  characteristicName = "TOTAL PHOSPHORUS, MIXED FORMS")
fieldValuesPie(TADAProfileClean5, field = "SampleCollectionMethod.MethodName",  characteristicName = "TOTAL PHOSPHORUS, MIXED FORMS")
