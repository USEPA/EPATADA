...
Filter down to only include data that is numeric in the "TADA.ResultMeasureValue" column. This removes data where the
...
You can also use the **TADA_AutoFilter()** function to remove any columns not required for the TADA workflow that
contain only NAs. 
...
TADAProfileClean6 <- TADAProfileClean5
  # TADAProfileClean6 <- TADA_CalculateTotalNP(TADAProfileClean5, daily_agg = "max")

# this is generating a new invalid comparable data ID
# test = as.data.frame(unique(TADAProfileClean6$TADA.ComparableDataIdentifier))
```
