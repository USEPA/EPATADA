...
Let's filter our MT_criteria table and select "Warm water, other life stages present",
...
fig <- TADA_Scatterplot(
  ...
  plotly::add_lines(
    ...
    #hoverinfo = "none",
    name = paste(
      MT_criteria_filter$ATTAINS.UseName[i],
      "for",
      MT_criteria_filter$UniqueSpatialCriteria[i],
      MT_criteria_filter$MagnitudeValueLower[i],
      MT_criteria_filter$MagnitudeUnit[i],
      "as a",  MT_criteria_filter$DurationValue[i],
      MT_criteria_filter$DurationUnit[i],
      MT_criteria_filter$DurationMethod[i]
      )

# join use and criteria to tada df (many to many relationships may occur)
tada.MT.clean.DO <- tada.MT.clean.DO |>
  dplyr::left_join(aquatic.life, relationship = 'many-to-many',
                   dplyr::join_by(ATTAINS.AssessmentUnitIdentifier,
ATTAINS.OrganizationIdentifier, ATTAINS.WaterType)) |>
 
  
   dplyr::left_join(MT_criteria_test, relationship = 'many-to-many',
                    dplyr::join_by(TADA.CharacteristicName, ATTAINS.UseName))
...
tada.MT.do.subsets <- tada.MT.clean.DO |>
  dplyr::group_by(DurationValue, DurationMethod, DurationUnit,
                  UniqueSpatialCriteria) |>
  dplyr::group_split()
```

Note that the scatterpoints are only individual result measurements. FutureTADA development in the package is being made for handling of the full criteria (magnitude, duration and frequency), seasonal considerations and other data sufficiency needs!
...
desc <- paste0(
  "Lower bound for ",
  test3[[4]]$ATTAINS.UseName[1],
  " (",
  test3[[4]]$DurationValue[1],
  " ",
  test3[[4]]$DurationUnit[1],
  " ",
  test3[[4]]$DurationMethod[1],
  " for ",
  test3[[4]]$UniqueSpatialCriteria[1],
  ")"
)


TADA_Scatterplot(tada.MT.do.subsets[[4]]) |>
   plotly::add_lines(
      x = c(
    min(test3[[4]]$ActivityStartDate, na.rm = TRUE),
    max(test3[[4]]$ActivityStartDate, na.rm = TRUE)
  ),
   y = (test3[[4]]$MagnitudeValueLower[1]),
  inherit = FALSE,
  name = paste(strwrap(desc, width = 40), collapse = "<br>"),
