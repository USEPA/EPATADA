...
In this example, we will get bacteria and pH data from Missoula County, Montana. 
...
Create an overview map. For each site, we can view the measurement count, visit
count and characteristic counts along with the site ID, site name, and organization name.
...
Let's filter our TADA data frame to only include data from the top 4 OrganizationFormalName.
The orgs data frame is sorted in descending order in which 1:4 will select the top 4 OrganizationFormalName values.
...
organizations, if you would like to prioritize results from one organization 
over another, this can be done using the org_hierarchy argument in
...
In this example, no censored data results were found. The input tada.MT.clean 
data is returned.
...
Scroll through the table and check to see if there any synonyms. 
Are there any TADA.ResultSampleFractionText,
TADA.MethodSpeciationName, and TADA.ResultMeasure.MeasureUnitCode
that may need to be harmonized for consistency? Let's check using
...
example, there were only three TADA.ComparableDataIdentifier and we still have three,
however PH has now been harmonized from "PH_NA_NA_NONE" to "PH_NONE_NONE_NONE".
This is the recommended consistent format for PH harmonization across all
states and tribes.
...
For demonstration purposes, let's assume we know for sure this MonitoringLocationIdentifier,
"MTWTRSHD_WQX-COMBITR02" should be associated with the AssessmentUnitIdentifier "MT76M001_020".
We are however uncertain if there are other sites that we want to associate with this 
AssessmentUnitIdentifier. We will use the result from TADA_with_ATTAINS above and filter
our TADA data frame to AssessmentUnitIdentifier "MT76M001_020" and also keep the  
...
filter our cleaned data frame as specified. We will also filter our data frame
to just DISSOLVED OXYGEN (DO)
...
Make the AU assignments for the remaining unassigned MLs not defined from our
user.supplied.cw using TADA_CreateATTAINSAUMLCrosswalk.
...
```{r}
AU_uses <- TADA_AssignUsesToAU(tada.MT.clean, org_id = "MTDEQ", AUMLRef = MT.AUMLRef$ATTAINS_crosswalk)
```
...
View scatterplots for all parameter and use criteria defined in your table. 
Note that the scatterpoints are individual result measure. Future development
is being made in the package for handling of the full criteria (magnitude, duration and frequency)!
...
fig <- TADA_Scatterplot(tada.MT.clean, id_cols = "TADA.ComparableDataIdentifier") |>
  plotly::add_lines(
...
View individual scatterplots with defined criteria for different combination of uses for DO.
```{r}
