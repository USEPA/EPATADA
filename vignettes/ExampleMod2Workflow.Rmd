...
-   To make this function run more efficiently, the default is to not
    ...
    they can set add_catch = TRUE. This will return the catchments, but
    increases function run time significantly. ATTAINS catchments are
    always returned for `TADA_CreateATTAINSAUMLCrosswalk` assessed WQP
    monitoring locations.

**Current Limitations**

-   Currently, the nhd_catch argument applies only to monitoring
