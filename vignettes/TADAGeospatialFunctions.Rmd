...
`return = TRUE`, the function returns a list containing the dataframe
...
If we set `return = TRUE` as done to create the `TADA_with_ATTAINS_list`
object above, we also now have all the raw ATTAINS features associated
with these TADA Water Quality Portal observations stored in a list with
the TADA dataframe.
...
`TADA_GetATTAINS()` when `return = TRUE`. For the function to work
properly, the input dataframe must be the list produced from
`TADA_GetATTAINS()` with `return = TRUE`. The map also displays the
