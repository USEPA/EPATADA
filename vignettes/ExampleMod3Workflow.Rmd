...
-   Define and summarize your spatial level of analysis for uses and
    parameters to sites (in addition to to assessment units if desired).
...
functions can help generate these crosswalks and filling in some values,
users must review and modify the tables generated in each step of the
process to ensure accuracy. Additionally, the TADA team has also
...
This will allow users to analyze their data against the 304(a) criteria;
and to easily compare results when when using 304(a) criteria vs. their
own organization's criteria.

# Getting Started

First, we will load in some internal data sets created for TADA workflow
example purposes. These data sets are loaded in from the
ExampleMod2Workflow and contains a ML to AU crosswalk and a Uses to AU
crosswalk table.

```{r load-data}
# import example data set, example AUML crosswalk and example uses to AU crosswalk.

Final.MT.AUMLRef <- Data_MT_AUMLRef


MT.UseAURef.rda <- Data_MT_Use
```
...
functions, but we will go through some minimal cleaning with an example
...
between each WQP/TADA.ComparableDataIdentifier and
ATTAINS.ParameterName. TADA_CreateParamRef() can provide a pre-filled
crosswalk with the required column names
...
organization. For example, some organizations may consider any of Total
Nitrogen, Nitrate/Nitrite, Ammonia as "Nitrogen" as the
ATTAINS.ParameterName and may not consider the fraction or speciation.
...
We can see that MTDEQ has additonal rows for E. Coli that was unable to
