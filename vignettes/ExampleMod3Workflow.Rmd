...
(TADA_ParametersForAnalysis and TADA_UsesForAnalysis) that are needed before
...
crosswalk table (TADA_ParametersForAnalysis), let's review all the unique
...
specific organization in the TADA_ParametersForAnalysis() function, we need to
...
TADA_ParametersForAnalysis() includes an argument input 'auto_assign' to assist
...
Setting "excel = TRUE" in TADA_ParametersForAnalysis tells the function to
...
TADA_ParametersForAnalysis() may not return all parameters required by an
...
TADA_ParametersForAnalysis using the modified parameter reference table as the
...
paramRef argument input in TADA_ParametersForAnalysis(). When this reference
...
cycles for MTDEQ (see ATTAINS.FlagUseName column in NCTC_usesRef).
...
Users can also choose to supply their own usesRef as an argument
...
supplied usesRef table. We will assume MTDEQ has forgotten to
...
TADA_ParametersForAnalysis requires a crosswalk between ATTAINS parameter names
...
AU level of summary, this requires an AUML crosswalk and a AU_UsesRef, we
