...
autopopulate characteristic alias matches. The auto assignment will create exact
...
Another option is to set auto_assign = 'Org'. This limits the characteristic alias
matches to return only ATTAINS.ParameterNames that an
...
Alternatively, we can choose to assign all unique use names that was submitted
in prior ATTAINS assessment cycles, by your organization to those
...
### User has completed a Uses to AU crosswalk

Now, if you’ve assigned uses to AUs with ExampleMod2Workflow.Rmd, 
pass that table as AU_UsesRef so uses for your analysis needs come
from your defined uses table in AU_UsesRef, not prior ATTAINS cycles.

Existing ATTAINS uses (pulled from prior cycles) include a useGroup column.
Align them to your AUMLRef by joining on ATTAINS.OrganizationIdentifier,
ATTAINS.WaterType, and ATTAINS.UseGroup.

Note: In the MT.UsesRef.AutoAssign example, use names are unchanged,
so the tables should be identical to MT.UsesRef.with.AU_UsesRef.
...
MT.UsesRef.with.AU_UsesRef <- TADA_UsesForAnalysis(
  ...
  org_id = c("MTDEQ"),
  paramRef = MT.ParamRef.Final,
  usesRef = NULL, # no parameter to uses crosswalk was provided
...
extract the use and parameters from their crosswalk. TADA has created
the TADACommunityHub in which TADA users can submit, share and maintain
user-contributed criteria and methodologies templates to support
reproducible and efficient analyses.

We can see that MTDEQ has additional rows of uses associated with E. Coli
that was unable to be retrieved from ATTAINS. We also see PH was not found
in the user supplied UsesRef table. We will assume MTDEQ has forgotten to
include PH in their table and showcase the importance of considering all readily
...
MT.UsesRef.user.supplied <- dplyr::select(criteria_table, ATTAINS.OrganizationIdentifier, TADA.ComparableDataIdentifier, TADA.CharacteristicName, ATTAINS.ParameterName, ATTAINS.UseName)
...
MT.UsesRef.user.supplied.edit <- TADA_UsesForAnalysis(
  ...
  org_id = c("MTDEQ"),
  paramRef = MT.ParamRef.Final,
  usesRef = MT.UsesRef.user.supplied,
  auto_assign = TRUE,
  excel = FALSE
  # uncomment excel = TRUE, overwrite = TRUE to run the excel file
  # excel = TRUE, overwrite = TRUE
