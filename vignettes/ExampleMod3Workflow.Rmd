...
In the next section, we’ll also use the AUMLRef and AU_UsesRef crosswalk outputs from ExampleModule2Workflow.Rmd to show how to work with assessment unit–level analysis.

First, join the criteria table to your WQP data frame. The criteria table will automatically be joined by the best match of either TADA.ComparableDataIdentifier or by combinations of TADA.CharacterisiticName, TADA.ResultSampleFractionText, and TADA.MethodSpeciationName.

```{r}
MT_data_w_criteria <- TADA_Analysis_Join_WQP_Criteria(Data_MT_MissoulaCounty, MT.CriteriaMethods.Final)
```

How many columns do you see?

```{r}
ncol(Data_MT_MissoulaCounty)
ncol(MT.CriteriaMethods.Final)
ncol(MT_data_w_criteria)
length(union(names(Data_MT_MissoulaCounty),names(MT.CriteriaMethods.Final)))
```
