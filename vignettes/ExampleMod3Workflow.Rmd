...
-   Creating a crosswalk (reference table) between ATTAINS parameter
    names and WQP/TADA characteristic names/TADA comparable data
    identifier.
...
Users will have four options in retrieving use names associated with
parameter names which are prioritized as follows:
...
1.  **User supplied usesRef** An optional crosswalk. This will contain
    ...
    with ATTAINS.ParameterName(s). If your user supplied usesRef is
    missing any ATTAINS.ParameterName(s) that are not found in your
    paramRef (required input) then it will get assigned accordingly from
    one of the next two options.

2. **User supplied AU_UsesRef** An optional crosswalk. This user
    supplied table will contain the most up-to-date use names by assessment
    unit for your analysis needs. For any use names already present in
    ATTAINS, only those prior parameter and use combinations will be
    returned. If AU_UsesRef includes modified or new use names (i.e., not
    currently found in ATTAINS), each unique use name will be assigned to
    each parameter.

3. **User supplied AUMLRef** An optional crosswalk. Users should only
    provide the AUMLRef if they have not Created an AU_UsesRef. TADA will
    run **`TADA_AssignUsesToAU()`** by default and assign uses to those AU.
    In this scenario all uses will come from prior ATTAINS assessments,
    filtered by the use names associated with these assessment unit's water
    types.

4. **From Prior ATTAINS assessment cycles**: Pulls in all prior ATTAINS
    uses (ATTAINS.UseNames) that have been associated with any parameter
    names (ATTAINS.ParameterName) found in your paramRef crosswalk for the
    specified ATTAINS organization (provided via the org_id argument to
    TADA_UsesForAnalysis). In this example, org_id = "MTDEQ" (Montana). Data
    is retrieved from prior ATTAINS assessment cycles via ATTAINS
    ExpertQuery web services.
...
We will start off with the use case in which TADA users may have not gone through the assignment of assessment units to uses or are only interested in summarizing results by monitoring locations.

### Option 4: From Prior ATTAINS assessment cycles
...
```{r usesRef-auto}
MT.UsesRef.auto.assign <- TADA_UsesForAnalysis(
  tada.MT.clean,
  org_id = c("MTDEQ"),
  paramRef = MT.ParamRef.Final,
  usesRef = NULL,
...
TADA_TableExport(MT.UsesRef.auto.assign)
```

### Option 1: User supplied usesRef

Users can choose to supply their own UsesRef as an argument input.
...
Note: In the MT.UsesRef.auto.assign example, all use names are found in
...
MT.UsesRef.with.AU_UsesRef <- TADA_UsesForAnalysis(
  ...
  paramRef = MT.ParamRef.Final,
  usesRef = NULL,
  AU_UsesRef = Data_MT_AU_UsesRef, # uses will be sourced from a user supplied uses to AU crosswalk that has been completed.
  AUMLRef = NULL,
  auto_assign = TRUE,
  excel = FALSE
  # excel = TRUE, overwrite = TRUE
...
TADA_TableExport(MT.UsesRef.with.AU_UsesRef)
```

### Option 3: User has completed a AU to ML crosswalk but not AU to Uses

While it is recommended for users who have created an AU to ML crosswalk to run **`TADA_AssignUsesToAU`** to create the AU_UsesRef crosswalk, it is not a requirement to do so. In this scenario users can provide the AUMLRef crosswalk to filter only use names that may be associated with those assessment units in your data frame of interest.

Let's go ahead and pull in our AUMLRef crosswalk created in TADAModule2.Rmd vignette. We will compare the output of **`TADA_UsesForAnalysis`** for assessment units for MTDEQ for the case in which water type is 'LAKE, FRESHWATER' versus 'RIVER'.
...
Now, let's compare the assignments of uses and parameters in these two scenarios. 

Question: What differences do we see between the two outputs for 'LAKE, FRESHWATER' versus 'RIVER'?

```{r usesRef-with-AUMLRef-lake}
MT.UsesRef.MT76F007_010 <- TADA_UsesForAnalysis(
  tada.MT.clean,
  org_id = c("MTDEQ"),
  paramRef = MT.ParamRef.Final,
  usesRef = NULL,
...
TADA_TableExport(MT.UsesRef.MT76F007_010)
```

```{r usesRef-with-AUMLRef-river}
MT.UsesRef.MT76M004_070 <- TADA_UsesForAnalysis(
  tada.MT.clean,
  org_id = c("MTDEQ"),
  paramRef = MT.ParamRef.Final,
  usesRef = NULL,
...
TADA_TableExport(MT.UsesRef.MT76M004_070)
```
...
rm(MT.UsesRef.auto.assign, user.supplied.MT.data, MT.UsesRef.Manual, MT.UsesRef.Manual.Update)
```
