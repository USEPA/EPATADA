...
Now, if you’ve assigned uses to AUs with ExampleMod2Workflow.Rmd, pass
that table as AU_UsesRef so use names for your analysis needs come from
your defined uses table in AU_UsesRef, not prior ATTAINS cycles.

Existing ATTAINS uses (pulled from prior cycles) include a useGroup
column. Align them to your AUMLRef by joining on
ATTAINS.OrganizationIdentifier, ATTAINS.WaterType, and ATTAINS.UseGroup.

Note: In the MT.UsesRef.AutoAssign example, use names are unchanged, so
the tables should be identical to MT.UsesRef.with.AU_UsesRef.
