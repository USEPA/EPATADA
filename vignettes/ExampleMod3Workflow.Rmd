...
vignette: >
  ...
  %\VignetteIndexEntry{Example Mod 3 Workflow: Generate the Criteria and Methodology File using the step-by-step Crosswalk Reference File}
...
analysis criteria and methodologies. This is a continuation of an example workflow with ExampleMod2Workflow.Rmd. 
...
-   Define and summarize your spatial level of analysis (ML versus AU) and capture any unique site-specfic WQS criteria needs for your organization.
...
This example will focus only on Montana. The
remainder of this vignette will walk through how to fill out two
...
Before creating the TADA/WQP CharacteristicName and ATTAINS parameter crosswalk table
(TADA_CreateParamRef), let's review all the unique
...
ParamRef <- MT.ParamRef_None %>%
  ...
  grepl("PH_NA_NA_NONE", TADA.ComparableDataIdentifier) ~ "PH",
  grepl("ESCHERICHIA COLI", TADA.ComparableDataIdentifier) ~ "ESCHERICHIA COLI (E. COLI)")) %>% 
    dplyr::bind_rows(data.frame(TADA.ComparableDataIdentifier = "PH_NA_NA_NONE", ATTAINS.ParameterName = "PH, HIGH"))
...
We can now save and reuse MT.ParamRef_Manual as the user-supplied paramRef argument
input in TADA_CreateParamRef(). When this reference file is used for the next assessment or analysis cycle, this process of reusing your prefilled paramRef table will assist in reviewing new WQX characteristic names, if any are added, and determine the appropriate crosswalk for these new entries if needed.
...
Your user supplied 'paramRef' is prioritized over the 'auto_assign' argument inputs. The auto_assign option will allow you to fill in any remaining blanks that you have not filled in. For example, this user supplied 'paramRef' argument input that was provided
chose to match only the TADA.ComparableDataIdentifier 'PH_NA_NA_NONE' to the ATTAINS.ParameterName 'PH' and will keep this as the crosswalk leaving 'PH_TOTAL_NA_NONE' as blank for the ATTAINS.ParameterName. However, by specifying auto_assign = 'All' for any values that have not been crosswalk,TADA_CreateParamRef() will fill those if there is an exact match. In the example below, using auto_assign = "All" and supplying the user supplied crosswalk for paramRef = MT.ParamRef_Manual, we will see 'PH_TOTAL_NA_NONE' will now be crosswalked to 'PH' as the ATTAINS.ParameterName.
...
remove intermediate variable, keep only the crosswalk table that we will continue to use in the workflow.
...
names associated with them. In this case, users will be responsible for manually assigning the appropriate ATTAINS.UseName applicable to their org under the column 'ATTAINS.UseName', and for inputting additional rows as needed if the parameter applies to multiples uses. Users can also choose to 'auto_assign' all unique ATTAINS.UseName by
ATTAINS.OrganizationName to any ATTAINS.ParameterName missing a use name associated with it.

In the example below, we can see "PH" and "ESCHERICHIA COLI (E. COLI)" as the ATTAINS.ParameterNames were assessed in prior assessment cycles for MTDEQ (see ATTAINS.FlagUseName column in NCTC_UseParamRef). "PH, HIGH" was
not listed in ATTAINS for MTDEQ in the prior assessment cycle, but
was included as a parameter name MTDEQ would like to assess for in this current assessment/analysis cycle. Thus, the ATTAINS.UseName is left blank for "PH, HIGH".
...
Now, if a user has completed assignments of uses to AUs in the recommended ExampleMod2Workflow.Rmd, they need to provide this as a useAURef argument input. This will help to ensure that the uses are applicable to your most recent assessment/analysis needs. These uses are extracted based on the uses to water type assignments in your useAURef and joined to an internal parameter and water type extraction from rExpertQuery to assign any new uses to parameters.
...
If desired, a user can manually assign "PH, HIGH" to any applicable
uses (disclaimer: this is for demonstration purposes only and does not reflect MTDEQ's criteria and assessment process).
...
Alternatively, We can choose to assign all unique use names found in your current useParamRef table, by your organization
to those ATTAINS.ParameterName without any associated ATTAINS.UseName. Users will need to review this assignment carefully and choose include or exclude appropriately or include additional rows as needed if there are more ATTAINS.UseName applicable for that ATTAINS.ParameterName that was not captured by the auto_assign method.
...
Users can also choose to supply their own UseParamRef as an argument input. This may be useful if your organization has made significant changes to use names in the current cycle that cannot be retrieved from the prior ATTAINS assessment cycle or if there are just additional use names that were unable to be retrieved from the prior assessment cycle. (Note: TADA functions leverages the ATTAINS assessment profiles which will not reflect any submitted use name changes to ATTAINS until the next assessment cycle is approved. In addition, it may not be possible to retrieve every use and parameter name combination from ATTAINS as it is dependent on what each organization has submitted to ATTAINS as only those uses that are impaired may have been submitted.) 

In this example, let's use a filled out criteria table for MTDEQ and extract the use and parameters from their crosswalk. We can see that MTDEQ has additonal rows for E. Coli that was unable to be retrieved from ATTAINS. We also see PH was not found in the user supplied useParamRef table. We will assume MTDEQ has forgotten to include PH in their table and showcase the importance of considering all readily available data from your WQP data retrieval that you may need to consider. We will change this table to "Include" for all PH values.
...
Now, we will proceed with generating a final summary of analyzing your WQP data and specifying the spatial level of summary. In this example, we will showcase an example of an AU and ML level of summary for MTDEQ analysis and compare the two outputs.

Let's start off with a ML summary of MTDEQ data frame.
```{r ML_Summary}
MT.MLSummaryRef_ML <- TADA_CreateMLSummaryRef(
  .data = tada.MT.clean, 
  useParamRef = MT.UseParamRef_User_Supplied,
  org_id = "MTDEQ"
  )
```

Now let's compare this to the AU level of summary. To provide the ML to AU level of summary, this requires an AUML crosswalk, we recommend you to review the ExampleMod2Workflow vignette for more in depth information on this crosswalk.
...
MT.CriteriaMethods_Blank <- TADA_DefineCriteriaMethodology(
  .data = tada.MT.clean,
  org_id = "MTDEQ",
  MLSummaryRef = MT.MLSummaryRef_AU,
  excel = FALSE
)
```

Now, if MTDEQ, already has a filled out/partially filled out criteria methods table, let's provide this as an argument input. 

Note: Whenever a criteriaMethods argument input value is provided, users will be warned if there are additional WQP characteristics (or TADA.ComparableDataIdentifier - see argument input uniqueDataId = TRUE/FALSE in the R documentation for more information) that are not captured in their user supplied table.
...
Note: When a user provides a MLSummaryRef, it is assumed their criteria tables parameter crosswalk is based on the paramRef and useParamRef crosswalk done. However, whenever a criteriaMethods table is provided, all unique WQP Characteristic (or TADA.ComprableDataIdentifier) will be shown in the table.
...
You can append epa304a recommended standards (if there is one found for a TADA.CharacteristicName)
