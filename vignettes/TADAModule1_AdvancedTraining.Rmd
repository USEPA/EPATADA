...
We can also use another function, `TADA_IDDepthProfile`s to identify
...
required to idenitfy a location/date/characteristic as a depth profile
is 2, but this can be changed by the user.

```{r id depth profile}
# 
depth_profile_id <- TADA::TADA_IDDepthProfiles(dataset_depth)
```

Now, we can use `TADA_DepthProfilePlot` to plot up to three
characteristics against depth. In this example, we will look at
dissolved copper, dissolved lead, and dissolved iron.

```{r id depth profile}
# 
TADA::TADA_DepthProfilePlot(dataset_0, groups = c("TOTAL PHOSPHORUS, MIXED FORMS_UNFILTERED_NA_UG/L", "TOTAL NITROGEN, MIXED FORMS_UNFILTERED_NA_MG/L"), location = "WVDEP_WQX-16393", activity_date = "2023-01-04", depthcat = FALSE)
```
