...
```{r overview map, warning=FALSE, message=FALSE}
...
We can look for groups, locations, and acitivty dates that we know have
depth profile compatiable data. Now, we can use `TADA_DepthProfilePlot`
to plot up to three characteristics against depth. In this example, we
will look at dissolved copper, dissolved lead, and dissolved iron.

```{r id depth profile}
# 
 TADA_DepthProfilePlot(Data_6Tribes_5y_Harmonized, 
                        groups = c('TEMPERATURE_NA_NA_DEG C', 'DEPTH, SECCHI DISK DEPTH_NA_NA_M', 'PH_NA_NA_NA'),
