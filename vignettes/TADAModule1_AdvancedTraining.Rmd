...
an example data frame, like `Data_Nutrients_UT` to get an idea of
...
Additionally, `TADA_DataRetrieval` performs some basic quality
control checks via `TADA_AutoClean` on the data using new
...
-   Unifies result and depth units to common units to improve ease of
    ...
    `?TADA_ConvertDepthUnits` for more information on these
...
enter `?TADA_DataRetrieval` into the console for more information
...
Currently, the `TADA_DataRetrieval` function combines three WQP
...
usage of the `TADA_DataRetrieval` function, but it will simplify
...
`display = "all"`. Enter `?TADA_FieldCounts()` into the console
...
are `TADA_FieldValuesTable()` and `TADA_FieldValuesPie()`.
...
`?TADA_ConvertSpecialChars` for more details on result value types
and handling and `?TADA_IDCensoredData` for details on censored
data preparation.

First, we can run `TADA_IDCensoredData` to fill in as many
NA/missing values as possible. We can use `TADA_FieldValuesPie` to
view the censored data flags identified in the data frame and their
relative frequency. `TADA_IDCensoredData` sorts result values into
...
`TADA_FindQCActivities()`, these flagging functions leverage WQX's
[QAQC Validation
...
potentially invalid data. `TADA_FindQCActivities()` uses a
TADA-specific domain table users can review with
`TADA_GetActivityTypeRef()`. All QAQC tables are frequently
