...
```{r}
list.of.packages <- c("tidyverse", "DT")
...
```{r table_temp, echo = FALSE}
# NOT REQUIRED FOR WORKSHOP WORKFLOW
# create table of template for display on packagedown site
DT::datatable(template, fillContainer = TRUE)
```
...
some basic quality control checks via `TADA_AutoClean` on the data using
new TADA-specific columns to preserve the original dataset:
...
`TADA_DataRetrieval` means that the basic quality control steps
described above are run. `TADA_DataRetrieval` follows similar
parameterization to `readWQPdata`, but check out the [help
page](#0){style="font-size: 11pt;"} or enter `?TADA::TADA_DataRetrieval`
...
```{r key}
key <- TADA::TADA_FieldCounts(dataset_0, display = "key")
```

```{r table_key, echo = FALSE}
# NOT REQUIRED FOR WORKSHOP WORKFLOW
# create table of template for display on packagedown site
DT::datatable(key, fillContainer = TRUE )
```

```{r all}
all <- TADA::TADA_FieldCounts(dataset_0, display = "all")
```

```{r table_all, echo = FALSE}
# NOT REQUIRED FOR WORKSHOP WORKFLOW
# create table of template for display on packagedown site
DT::datatable(all, fillContainer = TRUE)
```
...
orgs <- TADA::TADA_FieldValuesTable(dataset, field = "OrganizationFormalName")
```

```{r table_org, echo = FALSE}
# NOT REQUIRED FOR WORKSHOP WORKFLOW
# create table of template for display on packagedown site
...
DetectionQuantitationLimitType fields are populated. Use
...
censored data flags identified in the data set and their relative
...
DT::datatable(check, fillContainer = TRUE)
```

```{r table_check, echo = FALSE}
# NOT REQUIRED FOR WORKSHOP WORKFLOW
# create table of template for display on packagedown site
DT::datatable(check, fillContainer = TRUE)
```
...
DT::datatable(removed, fillContainer = TRUE)

# remove dfs no longer needed
...
DT::datatable(flag.datatypes, fillContainer = TRUE)
...
# create a data table for removed results
DT::datatable(removed, fillContainer = TRUE)
...
# create data table of removed results
DT::datatable(removed, fillContainer = TRUE)
...
DT::datatable(removal, fillContainer = TRUE)
```
...
to make non-detect values equal to half of the provided detection
limit?**
...
non-detect estimatiom methods based on the number of results, % of
dataset censored, and number of censoring levels (detection limits). The
decision tree used in the function was outlined in an [National Nonpoint
Source Tech
...
DT::datatable(dataset_cens_summary, fillContainer = TRUE)
```
...
DT::datatable(compid, fillContainer = TRUE)
```
...
DT::datatable(stats, fillContainer = TRUE)
```
