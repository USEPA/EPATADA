...
important functions with a sample data frame, and create basic
visualizations. The workflow is similar to a funnel: at each decision
point, data that fail QC checks are removed from the core data frame and
placed in a separate data frame, while data that pass are carried into
...
bring a data frame into the R environment. TADA is designed to work with
...
upload their own custom data frame into R for use with TADA by ensuring
...
an example data frame, like `Data_Nutrients_UT` to get an idea of the
data structure and format.

```{r template}
getwd() # find your working directory
template <- EPATADA::TADA_GetTemplate() # download template to working directory
...
enter `?TADA_DataRetrieval` into the console for more information about
input parameters and to see several examples.
...
dataset_0 <- EPATADA::Data_6Tribes_5y
...
number of rows in the original input data frame (`all_result_num`) equal
the the total number of rows added up between the passing (`pass_data`)
and removed (`fail_data`) data frames.
...
unique values are contained within each column of the data frame. The
...
```{r key}
key_counts <- EPATADA::TADA_FieldCounts(dataset_0, display = "key")

key_counts
```

```{r all}
all_counts <- EPATADA::TADA_FieldCounts(dataset_0, display = "all")

all_counts
```

**Question 3: How many unique 'TADA.ActivityMediaName' values exist in
your data frame? Are there any media types that are not water?**
...
```{r org}
EPATADA::TADA_FieldValuesPie(dataset, field = "OrganizationFormalName")

org_counts <- EPATADA::TADA_FieldValuesTable(dataset, field = "OrganizationFormalName")
...
censored data flags identified in the data frame and their relative
...
```{r id cens data}
dataset <- EPATADA::TADA_IDCensoredData(dataset)

EPATADA::TADA_FieldValuesPie(dataset, field = "TADA.CensoredData.Flag")
...
flag.datatypes <- EPATADA::TADA_FieldValuesTable(dataset, field = "TADA.ResultMeasureValueDataTypes.Flag")
...
We've taken a quick look at the raw data frame and split off some data
that are not compatible with TADA, now let's run through some quality
control checks. The most important ones to run to ensure your data frame
...
Guide](https://usepa.github.io/TADA/articles/WQXValidationService.html)
for more details on how TADA leverages the validation table to flag
potentially invalid data. `TADA_FindQCActivities()` uses a TADA-specific
...
```{r qaqc}
qaqc_ref <- EPATADA::TADA_GetWQXCharValRef()
...
```{r example flags}
dataset_flags <- EPATADA::TADA_FlagFraction(dataset, clean = FALSE, flaggedonly = FALSE)
dataset_flags <- EPATADA::TADA_FlagSpeciation(dataset_flags, clean = "none", flaggedonly = FALSE)
dataset_flags <- EPATADA::TADA_FlagResultUnit(dataset_flags, clean = "none", flaggedonly = FALSE)
dataset_flags <- EPATADA::TADA_FindQCActivities(dataset_flags, clean = FALSE, flaggedonly = FALSE)
...
```{r flag pies}
EPATADA::TADA_FieldValuesPie(dataset_flags, field = "TADA.SampleFraction.Flag")
EPATADA::TADA_FieldValuesPie(dataset_flags, field = "TADA.MethodSpeciation.Flag")
EPATADA::TADA_FieldValuesPie(dataset_flags, field = "TADA.ResultUnit.Flag")
EPATADA::TADA_FieldValuesPie(dataset_flags, field = "TADA.ActivityType.Flag")
```
...
correctly formatted. Let's add any invalid results to the removed data
frame for later review.
...
```{r removal reason}
removal <- EPATADA::TADA_FieldValuesTable(removed, field = "TADA.RemovalReason")
...
problematic detection limit data from our data frame, but that doesn't
...
```{r cens dataset}
dataset_cens <- EPATADA::TADA_SimpleCensoredMethods(dataset_flags, nd_method = "multiplier", nd_multiplier = 0.5, od_method = "as-is")
```
...
```{r data type flags}
# before
EPATADA::TADA_FieldValuesTable(dataset_flags, field = "TADA.ResultMeasureValueDataTypes.Flag")
```
...
# after
EPATADA::TADA_FieldValuesTable(dataset_cens, field = "TADA.ResultMeasureValueDataTypes.Flag")
```
...
How are you feeling about your test data frame? Does it seem ready for
...
Let's first check out characteristics in the data frame using `dplyr`
functions and pipes.
...
EPATADA::TADA_FieldValuesPie(dataset_cens, field = "OrganizationFormalName", characteristicName = "DISSOLVED OXYGEN (DO)")
```
...
```{r overview map}
EPATADA::TADA_OverviewMap(dataset_cens)
```
...
```{r location pie}
EPATADA::TADA_FieldValuesPie(dataset_cens, field = "MonitoringLocationTypeName")
```
...
and Cleaning](https://usepa.github.io/TADA/articles/TADAModule1.html) or
[TADA_HarmonizeSynonyms()](https://usepa.github.io/TADA/reference/TADA_HarmonizeSynonyms.html)),
...
compid <- EPATADA::TADA_FieldValuesTable(dataset_cens, field = "TADA.ComparableDataIdentifier")
```
...
EPATADA::TADA_Histogram(plot_data, id_cols = "TADA.ComparableDataIdentifier")

EPATADA::TADA_Boxplot(plot_data, id_cols = "TADA.ComparableDataIdentifier")

stats <- EPATADA::TADA_Stats(plot_data)
```
...
```{r depth category}
dataset_depth <- EPATADA::TADA_FlagDepthCategory(dataset_cens)
```
...
```{r id depth profile}
depth_profile_id <- EPATADA::TADA_IDDepthProfiles(dataset_depth, nvalue = 5)
```
...
```{r depth plot}
EPATADA::TADA_DepthProfilePlot(dataset_cens,
  groups = c(
    "TEMPERATURE,
                                       WATER_NA_NA_DEG C",
