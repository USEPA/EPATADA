...
rATTAINS::domain_values(domain_name = "parameters")
```
...
By specifying the name of the organization(s) used, if creating an excel
spreadsheet (using excel = TRUE) this will create a separate tab in the
file which contains the parameter names used in prior assessment cycles
filtered by those organization(s). This will allow users to decide
whether to continue to use the same parameter names that are org
specific or to use other parameter names found in the domain list in
ATTAINS or assist a user to determine if a new parameter name is needed.
