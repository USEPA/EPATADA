...
Disclaimer: The EPATADA Module 3 functions are under active development. They are being designed to: (1) assist
users with associating Water Quality Portal characteristics with
applicable ATTAINS and EPA 304(a) parameters and uses, and to (2) compare Water Quality Portal results with numeric water quality thresholds or criteria (magnitude component only). EPATADA functions do not constitute current EPA policy or regulatory requirements. While organizations may optionally choose to use EPATADA as a a tool in their assessment process, use of EPATADA is not required.

## Introduction

Criteria are defined as the magnitude, duration and frequency associated
with a parameter and designated use. At this time, this tool aims to assist
only with comparing results against the magnitude component of criteria.
...
This vignette provides an overview of two functions that assist users with:

-   Creating a crosswalk between ATTAINS parameter names and
    WQP/TADA characteristic names.

-   Creating a crosswalk of all unique combinations of assessment units, uses, and parameters.
    
Future vignettes aim to assist with next steps in the analysis process, including:
...
Module 3 functions are being designed with the flexibility for users from
...
will focus on assisting users with creating the crosswalk reference tables
that are needed prior to defining and capturing their organization's
...
Data cleaning, harmonization, and removal of suspect results,
...
has already been through this process.
...
of creating two TADA Module 3 reference tables (TADA_CreateParamRef and TADA_CreateParamUseRef) that are needed before we can start assigning applicable criteria and methodologies information for a specific ATTAINS organization.
...
# Three example use cases for TADA_CreateParamUseRef().
...
use_name is not applicable, users should choose "Exclude" for that
...
In the example below, we can see "PH" was not a prior ATTAINS parameter name for MDE_EASP (see ATTAINS.FlagUseName column in NCTC_ParamUseRef). If desired, a user can manually assign this parameter to any applicable uses
...
Users may also choose to include the EPA304a recommended water quality criteria within the function input "org_names". This option utilizes the EPA304a.PollutantName column created in TADA_CreateParamRef(). If a user includes additional organizations beyond "MDE_EASP" and "EPA304a", like in the example NCTC_ParamUseRef2 below, it will not generate rows for the parameter and use combinations for those additional orgs ("21VASWCB" (Virginia), "21PA"(Pennsylvania), WVDEP
(West Virginia)) because only MDE_EASP (Maryland) was defined in
TADA_CreateParamRef() for paramRef = ParamRef. If you would like to include other organizations here, you must first fill out the TADA_CreateParamRef() for all organizations of interest.
