...
# hopefully to help with knit and pass the checks, but we want to make sure this chunk does not show up in the final PR, just for development.
remotes::install_github("USEPA/EPATADA",
...
-   Define the Criteria and Methodologies used for assessments and
    ...
    sumarizing individual magnitude excursions or for the full Criteria
    (Magnitude, Duration and Frequeuncy) associated with each parameter
    and use.
...
Load the data and run TADA_GetATTAINS on example data frame. This has
already been done as it does take some time to run.
...
```{r}
Data_NCTC3 <- TADA_RunKeyFlagFunctions(Data_NCTC)
Data_NCTC4 <- TADA_HarmonizeSynonyms(Data_NCTC3)
```
...
```{r}
Data_NCTC2 <- dplyr::filter(Data_NCTC4, TADA.CharacteristicName %in% c("ZINC", "PH","NITRATE"))
```

Creates a blank template of ATTAINS.ParameterName which needs crosswalk.
In this example, the user has not specified any orgs and the output will
generate duplicate rows for each unique org names found in the TADA data
frame.
...
Users should specify the name(s) of their orgs that will be used for
assessment. This will filter the drop-down list for prior parameter
names and use names down the line in creating the reference tables.

(Note: EPA 304a standards will be specified in the next step, but
utilizes the CST.PollutantName column. Should we allow users to "modify"
the CST.PollutantName crosswalk that we have done?)
...
We also provide an example in which a user modifies this reference table
directly in the R environment only. If this occurs, user will need to
run "validate = TRUE" to accurately reflect the flagging of parameter
names.

```{r}
# only run the code if you make edits to the excel file
...
ParamRef2 <- dplyr::mutate(ParamRef, ATTAINS.ParameterName = case_when(TADA.CharacteristicName == "NITRATE" & !is.na(TADA.MethodSpeciationName)  ~ "NITROGEN, TOTAL"))

NCTC_ParamRef3.1 <- TADA_CreateParamRef(
  Data_NCTC2, 
  org_names = c("Maryland"),
  paramRef = ParamRef2,
  excel = FALSE,
...
Users may also include states outside of their spatial region if desired
depending on a user's interest. Users will be responsible for filling
out this crosswalk though for state's outside their own orgs. Although,
future efforts will focus on creating a repository of criteria and
methods along with crosswalks done from other TADA users (this will
depend on other org participation). However, the drop-down list will
include all prior ATTAINS domain value for all orgs that a user selects.
Users can also choose to only return a data frame in the R environment
by using excel = FALSE as a function input.

it does not autopopulate, but the goal is that if states share their
criteria and methods, we will now have a way to allow states to compare
...
First example does not specify which orgs to use, and will use the
filtered orgs in TADA_CreateParamRef to determine what orgs to filter
by. Since 'ParamRef' only contains the crosswalk information for
Maryland it only returns parameter and use combinations found in
Maryland.
...
\*Look at ParamRef for reference

should we change the 'org_names' argument to only 'epa304a' in this
case? Users should have determined what orgs and parameter names are
used in the first step in TADA_CreateParamRef() and can choose to
include EPA304a here.
...
NCTC_ParamUseRef4 <- TADA_CreateParamUseRef(
  ...
  paramRef = NCTC_ParamRef4, 
  excel = FALSE
  )
```

Users must review the matching done with TADA_GetATTAINS of monitoring
sites to AU. If there are site-specific criteria that only applies to
some monitoring locations within an AU, whether it's an ecoregion, for a
specific parameter and use, or if there are separate criteria for
"warm-waters" versus "cold-waters" or "trout-based" versus other species
based, this should be defined here. Users can further customize specific
exclusions that pertains to their water quality assessments.
