...
```{r}
Data_NCTC2 <- dplyr::filter(Data_NCTC4, TADA.CharacteristicName %in% c("ZINC","NICKEL", "PH","NITRATE"))
```
...
parameter reference data frame.
...
desired. However, the drop-down list will include all prior ATTAINS
domain value for all orgs that a user selects. Users can also choose to
only return a data frame in the R environment by using excel = FALSE as
a function input. Users will be responsible for filling out this
crosswalk though for state's outside their own orgs

# it does not autopopulate, but the goal is that if states share their criteria and methods, we will now have a way to allow states to compare against other states
...
parameter reference to determine what orgs to filter by. Since
NCTC_ParamRef2 only contains the crosswalk information for Maryland it
only returns parameter and use combinations found in Maryland.
...
# Look at NCTC_ParamRef for reference

# should we change the 'org_names' argument to only 'epa304a' in this case? Users should have determined what orgs and parameter names are used in the first step in TADA_CreateParamRef() and can choose to include EPA304a here.
