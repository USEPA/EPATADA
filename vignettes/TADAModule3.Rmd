...
# Three cases of TADA_CreateUseParamRef().
...
NCTC_AURef2 <- TADA_CreateAURef(Data_NCTC_GetATTAINS,
                               excel = FALSE)
# User has provided a final crosswalk customized as AURef. In this case, it does not matter whether Data_NCTC or Data_NCTC_GetATTAINS is used. AURef in this example, assumes
...
NCTC_AURef_final <- TADA_CreateAURef(Data_NCTC, AURef = AURef,
                               excel = TRUE, overwrite = TRUE)
```
...
desire in this step. Users should do this before running
TADA_CreateAURef().

Users can also choose to do this filter during TADA_CreateUseAURef()
where they can provide their org_id to filter this (should this be done
during TADA_CreateAURef()?)

```{r}
# Currently, this uses rATTAINS to pull in all prior use name associated with this org_id.
NCTC_UseAURef <- TADA_CreateUseAURef(Data_NCTC, org_id = "MDE_EASP", AUtoMLRef = NCTC_AURef_final, excel = TRUE, overwrite = TRUE)
```
