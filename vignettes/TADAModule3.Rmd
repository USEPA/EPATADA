...
criteria or methodologies. While TADA functions automate some of this
...
Using an example data set for HUC code '02070004', we will focus on a
sample workflow with Maryland in creating the TADA Module 3 reference
tables. We will facilitate the association of WQP monitoring locations
with assessment units/designated uses. These reference tables will be
necessary for creating and defining the comparison of WQP characteristic
results with the numeric criteria associated with the relevant
assessment unit/designated use.

The necessary data cleaning, harmonization, handling of censored data,
removal of suspect results, and other QAQC processes must be completed
by users before utilizing Module 3 functions. The example TADA data
frame used in this vignette has already been through this process. See
Module 1 Workflow.R script to review how this example data frame was
prepared.
...
TADA_CreateParamRef() using the modified parameter reference table as
the input for "paramRef". This will update the
...
"organization_identifier" column has listed this parameter as a cause in
prior assessment cycles. It also flags rows where the
...
# excel = TRUE, overwrite = TRUE)
TADA_CSVExport(ref = NCTC_ParamRef_Final)
```
...
TADA_CSVExport(ref = NCTC_UseParamRef)
```
...
"MDE_EASP" and "EPA304a", see example NCTC_UseParamRef2 below, it will
...
TADA_CSVExport(ref = NCTC_UseParamRef3)
```
...
```{r paged.print=TRUE}
add_data <- data.frame(
  "organization_identifier" = "MDE_EASP", "ATTAINS.ParameterName" = rep("PH", 3),
  "use_name" = c(
...
UseParamRef <- NCTC_UseParamRef3 %>%
  dplyr::left_join(
    add_data, 
    by = c("organization_identifier", "ATTAINS.ParameterName"), 
    keep = FALSE
    ) %>%
...
TADA_CSVExport(ref = UseParamRef)
...
TADA_CSVExport(ref = NCTC_UseParamRef_Final)
```

## TADA_CreateUseAURef()

Users should have ran TADA Module 2 functions to assist with matching
WQP Monitoring Location sites to ATTAINS Assessment Units. Users are
responsible for ensuring that their spatial region is properly capturing
only the Assessment Units of interest relevant to their organization.

**Users will only need to provide this AU and Use Name reference table,
which pulls in prior use names associated with an ATTAINS Assessment
Unit, if they are interested in summarizing criteria and methodology
summaries of a parameter and use by Assessment Unit as well.**

If a user is only interested in summarizing criteria and methodologies
of a parameter and use on a monitoring location, they are not required
to run TADA_CreateUseAURef() as there are no use name to AU to define in
this scenario.

In this example below, a user has created their final ML to AU crosswalk
(see Module 2b vignette for more information on user support in this
crosswalk). If there are unique site-specific criteria that only applies
to some monitoring locations within an AU, whether it's an ecoregion,
for a specific parameter and use, or if there are separate criteria for
"warm-waters" versus "cold-waters" or "trout-based" versus other species
based, this should be defined in NCTC_AURef_Final. Users will be
responsible for filling out this unique spatial criteria under
"ApplyUniqueSpatialCriteria" column.

```{r}
# The users .data dataframe has TADA_GetATTAINS crosswalk that will now be used.
NCTC_AURef_GetATTAINS <- TADA_CreateAURef(
  Data_NCTC_GetATTAINS,
  excel = FALSE
  )
```

Users should note that if their initial data retrieval pull covered a
larger spatial area than their spatial region of interest, they should
filter the data frame to only contain the Assessment units that they
desire in this step. In this example, Maryland has filtered their
spatial data to only represent their organization's region on interest,
as their initial data pull of a HUC code captured other regions.

```{r}
AURef_User_Supplied <- dplyr::filter(NCTC_AURef_GetATTAINS, grepl('MD', ATTAINS.assessmentunitidentifier))

NCTC_AURef_Final <- TADA_CreateAURef(
  Data_NCTC,
  AURef = AURef_User_Supplied,
  excel = FALSE
  )
```

<!-- Move to Mod 2 workflow for ML to AU matching process -->

<!-- ```{r} -->

<!-- # User will make the decision whether they would like to provide a ML to AU crosswalk. If monitoring location level is all that is desired, they can run the first function with no AURef. If they want to use TADA_GetATTAINS() output as the crosswalk, they can provide that as the .data dataframe. User can also provide their own custom crosswalk that has all the required column names. -->

<!-- NCTC_AURef <- TADA_CreateAURef(Data_NCTC, -->

<!--                                excel = FALSE) -->

<!-- # Load example dataframe that already has TADA_GetATTAINS() ran. -->

<!-- # Example dataframe contains larger spatial area, filter by desired organization for Maryland. -->

<!-- Data_NCTC_GetATTAINS <- Data_WV_Mod1_Output_GetATTAINS -->

<!-- # The users .data dataframe has TADA_GetATTAINS crosswalk that will now be used. -->

<!-- NCTC_AURef2 <- TADA_CreateAURef( -->
...
<!-- # User has provided their own crosswalk customized. In this case, it does not matter whether Data_NCTC or Data_NCTC_GetATTAINS is used.  -->

<!-- AURef_User_Supplied <- dplyr::filter(NCTC_AURef2, grepl('MD', ATTAINS.assessmentunitidentifier)) -->

<!-- ## We may need to consider handling new AU or new ML that has been included in the dataframe that users may need to review? Or should this all assume that users have reviewed this process prior? -->
...
Users can now pull in all prior use_names associated for an ATTAINS
Assessment Unit. **Note: CURRENTLY we are utilizing ATTAINS Assessment
webservices. This will NOT include any uses related to new or otherwise
previously unassessed AUs even if they exist in ATTAINS.**

```{r}
# Currently, this uses rATTAINS to pull in all prior use name associated with this org_id.
NCTC_UseAURef <- TADA_CreateUseAURef(
  Data_NCTC, 
  org_id = "MDE_EASP", 
  AUtoMLRef = NCTC_AURef_Final, 
  excel = TRUE, overwrite = TRUE
  )
```
