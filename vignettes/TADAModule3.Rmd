...
Disclaimer: While the EPATADA package aims to assist different
organizations (state, territories and tribal nations) with their Clean
Water Act assessment process, it does not constitute current EPA policy
or regulatory requirements. This tool is not to be solely used for an
organizations decision making process and use of EPATADA is not
required.
...
Users can reference the domain values of parameter names listed as a
...
Users that need to fill out the blank template can do so either: 1)
within the Excel file (downloaded using excel = TRUE), or 2) directly in
the R environment.

In the example below, we will modify the reference table called
'ParamRef' directly in the R environment and not in the excel file. We
can then re-run the function using the updated 'ParamRef' file to
reflect the ATTAINS validation (flagging) of parameter names. See
updated ATTAINS.FlagParameterName column in NCTC_ParamRef2.
...
TADA_CSVExport(ref = NCTC_ParamRef3)
```
