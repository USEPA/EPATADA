...
Next, install and load TADA. dataRetrieval and other TADA R Package dependencies will also be downloaded from CRAN with the TADA install, but the development version can be downloaded directly from GitHub if desired (un-comment). 
...
# example for Alaska
# TADAProfile2 <- TADAdataRetrieval(project = "Anchorage Bacteria 20-21")
...
Review all column names in the TADA Profile. Remove results that are not water samples or measurements (TADA is recommended for use with water data only). 
...
FALSE which flags rows of data with invalid or imprecise coordinates without removing them. Assigning clean_imprecise = TRUE will remove rows of data with invalid or imprecise coordinates.
...
-   ?**InvalidSpeciation**
    ...
    -   When clean = "none", this function adds the following column to
        ...
        each TADA.CharacteristicName and MethodSpecificationName                 combination in your dataframe as either "Nonstandardized",               "Invalid", or "Valid".
...
-   ?**InvalidFraction**
    ...
    -   When clean = FALSE, this function adds the following column to
        ...
        each TADA.CharacteristicName and TADA.ResultSampleFractionText           combination
...
-   ?**AboveNationalWQXUpperThreshold**
    ...
    -   When clean = FALSE, the following column is added to your
        dataframe: TADA.ResultValueAboveUpperThreshold.Flag. This column         flags rows with data that are above the upper WQX threshold.
...
-   ?**BelowNationalWQXLowerThreshold**
    ...
    -   When clean = FALSE, the following column is added to your
        dataframe: TADA.ResultValueBelowLowerThreshold.Flag. This column         flags rows with data that are below the lower WQX threshold.
...
TADAProfileClean10 <- BelowNationalWQXLowerThreshold(TADAProfileClean9, clean = FALSE)
```
...
-   ?**PotentialDuplicateRowID**
    ...
    -   When clean = FALSE, the following column will be added to you
        dataframe: TADA.PotentialDupRowIDs.Flag. This column flags               potential duplicate rows of data in your dataframe, and assigns          each potential duplicate combination a unique number linking the         two potential duplication rows.
...
The HydrologicEvent field has five unique values. In this example we
want to remove samples collected during "Flood" events. Therefore, we'll
specify that we want to remove rows where the CharacteristicName is
"NITROGEN" and the HydrologicEvent field is "Flood."

```{r}
TADAProfileClean16 <- dplyr::filter(TADAProfileClean15, !(TADA.CharacteristicName %in% "NITROGEN" & HydrologicEvent %in% "Flood"))
```
