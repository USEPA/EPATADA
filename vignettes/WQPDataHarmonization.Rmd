...
MonitoringLocationTypeName has multiple values. In this example, 
...
```{r}
FilterParFields(TADAProfileClean18, "TOTAL NITROGEN, MIXED FORMS")
```
...
FilterParFieldReview(field = "MeasureQualifierCode", TADAProfileClean18,  
...
TADAProfileClean19 <- dplyr::filter(TADAProfileClean18, !(TADA.CharacteristicName %in% "TOTAL NITROGEN, MIXED FORMS" & MeasureQualifierCode %in% "U"))

# regenerate table and pie chart
FilterParFieldReview(field = "MeasureQualifierCode", TADAProfileClean19,  
                     parameter = "TOTAL NITROGEN, MIXED FORMS")
...
unique(TADAProfileClean19$TADA.ComparableDataIdentifier)

# filter dataframe to only "TOTAL NITROGEN, MIXED FORMS"
TADAProfileClean20 <- dplyr::filter(TADAProfileClean19, TADA.CharacteristicName == "TOTAL NITROGEN, MIXED FORMS")

# generate stats table
TADAProfileClean20_stats <- TADA::TADA_stats(TADAProfileClean20)

TADAProfileClean20_stats

# generate a histogram
hist(TADAProfileClean20$TADA.ResultMeasureValue, 
     main = paste((unique(TADAProfileClean20$TADA.CharacteristicName))
                  ,"vs. Frequency"),
     breaks=200, xlab = paste(
       (unique(TADAProfileClean20$TADA.CharacteristicName)),
       "Values"))
...
TADA::TADA_boxplot(TADAProfileClean20)
...
# run chisq test
outliers::chisq.out.test(TADAProfileClean20$TADA.ResultMeasureValue)

# remove results greater than the upper fence (see stats table generated above)
# note: many outliers are valid results.
TADAProfileClean20_removeOutliers <-
  dplyr::filter(TADAProfileClean20, TADA.ResultMeasureValue <= 2.593)
...
hist(TADAProfileClean20_removeOutliers$TADA.ResultMeasureValue, 
     main = paste((unique(TADAProfileClean20_removeOutliers$TADA.CharacteristicName))
                  ,"vs. Frequency"), xlab = paste(
       (unique(TADAProfileClean20_removeOutliers$TADA.CharacteristicName)),
       "Values"))

# regenerate interactive box plot
TADA::TADA_boxplot(TADAProfileClean20_removeOutliers)
