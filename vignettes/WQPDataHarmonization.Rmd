...
list.of.packages <- c("plyr", "data.table", "dplyr", "magrittr", "stringr", "utils", "RColorBrewer", "stats", "gganimate", "gifski", "lubridate", "maps", "remotes", "dataRetrieval", "usmap", "ggplot2", "maptools")
...
repo. You must load the remotes library before installing TADA and other packages from GitHub
...
Uncomment the lines below to install latest version of dataRetrieval, ggplot2, usmap, and TADA from GitHub.
...
remotes::install_github("pdil/usmap", dependencies=TRUE)
...
#libraries loaded straight from GitHub (see last two code chunks)
library(ggplot2)
...
the TADA **autoclean** and **MeasureValueSpecialCharacters** functions
as well, which are both required for subsequent functions within the
TADA R package to run. These functions alter and/or add the following
WQP columns (enter *?MeasureValueSpecialCharacters* and *?autoclean*
...
-   Adds (All the data cleaning and transformations are done directly to
    ...
    "TADA.DetectionLimitMeasureValue.Flag" are created to track any
    ...
    if special characters are included in result values, and specifies
    what the special characters are.
...
```{r}
# dataRetrieval1 <- dataRetrieval::readWQPdata(statecode = "AK", characteristicName = c("Fecal Coliform", "Escherichia coli", "Enterococcus"), startDate = "05-01-2018", ignore_attributes = TRUE)

# another example
# dataRetrieval2 <- dataRetrieval::readWQPdata(statecode = "UT", characteristicName = c("Ammonia", "Nitrate", "Nitrogen"), startDate = "01-01-2021", ignore_attributes = TRUE)

```

Use the code below to download data from the WQP using TADAdataRetrieval. Edit the code chuck below to define your own WQP query inputs.

```{r}
# here is an example for Utah
# TADAProfile1 <- TADAdataRetrieval(statecode = "UT", characteristicName = c("Ammonia", "Nitrate", "Nitrogen"), startDate = "10-01-2020")
...
TADAProfile_CharSummary <- SummarizeCharacteristics(TADAProfile)
```
...
TADAProfileClean1$FullCountyCode <- paste(TADAProfileClean1$StateCode, sep = "", TADAProfileClean1$CountyCode)
Unique_FullCountyCodes <- unique(TADAProfileClean1$FullCountyCode)
...
usmap::plot_usmap("counties", include = c(Unique_FullCountyCodes), labels = FALSE) +
  geom_point(data = usmap_transform(TADAProfileClean1, 
                                    input_names = c("LongitudeMeasure", "LatitudeMeasure"), 
                                    output_names = c("x", "y")), 
             aes(x = x, y = y),
...
The ActivityDepthHeightMeasure.MeasureValue column provides depth
information. This is a crucial column for lake data but less often for
river data.

This function checks dataframe for depth profile data. Where depth
profile columns are populated, the function appends 'Conversion Factor'
columns and populates those columns based on the original unit
(MeasureUnitCode columns) and the target unit, which is defined in the
'unit' argument. A 'Depth Target Unit' column is also appended,
indicating the unit all selected depth data is converted to. When
...
includes converted depth data and the 'Depth Target Unit' column, which
acts as a flag indicating which rows have been converted. Default is
transform = TRUE.
...
1.  ResultSampleFractionText specifies forms of constituents. In some
    ...
    cases, each CharacteristicName and ResultSampleFractionText
...
Create animated national map for a single characteristic. This animation currently only works for Nitrogen. Updates are coming soon.

```{r}
#Animation currently only works for Total Nitrogen or Nitrogen. Updates coming soon.
