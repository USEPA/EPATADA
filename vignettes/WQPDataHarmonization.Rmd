...
```{r}
# here is an example for Utah. We will move forward with this example in the remainder of the vignette
...
# another example query for Alaska
...
The ActivityMediaSubdivisionName field has unique values. In this example, we show how a user could leverage this function to remove specific values from their dataframe.

```{r}
TADAProfileClean15 <- dplyr::filter(TADAProfileClean14, !(ActivityMediaSubdivisionName %in% c("Groundwater", "Bulk deposition")))

FilterFieldReview("ActivityMediaSubdivisionName", TADAProfileClean15)
```
...
TADAProfileClean18_TNonly <- dplyr::filter(TADAProfileClean17, TADA.CharacteristicName == "TOTAL NITROGEN, MIXED FORMS")

# review if any remaining result values are NA
sum(is.na(TADAProfileClean18_TNonly$TADA.ResultMeasureValueDataTypes.Flag))

# review unique values in TADA.ResultMeasureValueDataTypes.Flag
unique(TADAProfileClean18_TNonly$TADA.ResultMeasureValueDataTypes.Flag)

# if applicable, substitute a value for NA. In this example, 0.02 MG/L is used for NITROGEN.
TADAProfileClean18_TNonly$TADA.ResultMeasureValue <- ifelse(is.na(TADAProfileClean18_TNonly$TADA.ResultMeasureValue) == TRUE, 0.02, TADAProfileClean18_TNonly$TADA.ResultMeasureValue)

# generate summary stats
summary(TADAProfileClean18_TNonly$TADA.ResultMeasureValue)

# generate a histogram
hist(TADAProfileClean18_TNonly$TADA.ResultMeasureValue, 
     main = paste((unique(TADAProfileClean18_TNonly$TADA.CharacteristicName))
                  ,"vs. Frequency"),
     breaks=200, xlab = paste(
       (unique(TADAProfileClean18_TNonly$TADA.CharacteristicName)),
       "Values"))

# generate interactive box plot
TADA::TADA_boxplot(TADAProfileClean18_TNonly)

# calculate lower and upper thresholds
UpperBound=quantile(TADAProfileClean18_TNonly$TADA.ResultMeasureValue, 
                      c(.75))+(1.5*IQR(TADAProfileClean18_TNonly$TADA.ResultMeasureValue))
LowerBound=quantile(TADAProfileClean18_TNonly$TADA.ResultMeasureValue, 
                      c(.25))-(1.5*IQR(TADAProfileClean18_TNonly$TADA.ResultMeasureValue))

# test if highest value is a true outlier
outliers::chisq.out.test(TADAProfileClean18_TNonly$TADA.ResultMeasureValue)

# remove results greater than the upper bound
TADAProfileClean18_TNonly_removeOutliers <-
  dplyr::filter(TADAProfileClean18_TNonly, TADA.ResultMeasureValue <= 2.59)

# regenerate histogram
hist(TADAProfileClean18_TNonly_removeOutliers$TADA.ResultMeasureValue, 
     main = paste((unique(TADAProfileClean18_TNonly_removeOutliers$TADA.CharacteristicName))
                  ,"vs. Frequency"), xlab = paste(
       (unique(TADAProfileClean18_TNonly_removeOutliers$TADA.CharacteristicName)),
       "Values"))

# regenerate interactive box plot
TADA::TADA_boxplot(TADAProfileClean18_TNonly_removeOutliers)
