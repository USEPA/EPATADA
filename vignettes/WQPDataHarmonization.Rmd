...
```{r}
# here is an example for Sac and Fox Nation. We will move forward with this example in the remainder of the vignette. 
TADAProfile = TADAdataRetrieval(organization = "SACFOXMO")

# here is an example for UT. 
...
# remove all data for sites with invalid coordinates. 
# Change function inputs to keep this data, but be aware it may impact your mapping. 
...
we remove quality control (QC) values.
...
TADAProfileClean14 <- QualityControlActivity(TADAProfileClean13, clean = "all")
...
```{r}
# Remove non-surface water results
TADAProfileClean15 <- dplyr::filter(TADAProfileClean14, 
                                    !(ActivityMediaSubdivisionName %in%
                                        c("Bulk deposition", "Groundwater")))

# regenerate table and pie chart
FilterFieldReview("ActivityMediaSubdivisionName", TADAProfileClean15)

# Remove results associated with storm or tidal action
TADAProfileClean15 <- dplyr::filter(TADAProfileClean15, 
                                    !(OrganizationFormalName %in%
...
**FilterParList** function.

```{r}
FilterParList(TADAProfileClean16)
```

Next, using the **FilterParFields** function, review how many unique
allowable values are included within each of the following fields for a
specific parameter:
...
# review TADA.ResultMeasureValueDataTypes.Flag to see any
# NA's remain
unique(TADAProfileClean18$TADA.ResultMeasureValueDataTypes.Flag)

# review if any remaining result values are NA
sum(is.na(TADAProfileClean18$TADA.ResultMeasureValue))
...
Regenerate histogram and box plot.
```{r}
