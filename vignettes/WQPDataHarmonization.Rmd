...
version can be downloaded directly from GitHub (un-comment). The following code will also install
any packages you do not have, and load all packages required to run this vignette into 
your R session.
...
# note that you must load the remotes library before installing TADA or dataRetrieval from GitHub
library(remotes)
# If needed, uncomment the line below to install the "remotes" package specifying the repo
# install.packages("remotes", repos = "http://cran.us.r-project.org")

library(dataRetrieval)
# Uncomment the line below to optionally install latest version of dataRetrieval from github
...
2.  the datset with CharacteristicName, ResultSampleFractionText,
...
TADAProfileClean17 <- HarmonizeData(TADAProfileClean16, transform = TRUE, flag = FALSE)
```
