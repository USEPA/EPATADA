---
title: "TADA Module 2: Workflows to Support TADAShinyJoinToAU Use Cases"
...
description: An overview TADA Module 2 workflows to support TADAShinyJoinToAU development and EPATADA package users.
editor_options:
...
## TADA Module 2 Use Cases

This vignette focuses on the following TADA module 2 use cases:

1) New ATTAINS Tribe

2) State/Existing ATTAINS Tribe

3) State With Existing AU/ML/Use Crosswalks - formatting help only

## 1) New ATTAINS Tribe

1.	Load TADA File – loads Mod 1 output file containing all data for assessments .

```{r load.tribal}

mod1.tribal <- Data_TribalNations_Harmonized |>
  dplyr::filter(OrganizationFormalName == "Blackfeet Nation (Montana)")

```

2.	Flag and Review Sites
```{r}
...
3.	Join AUs
4.	Review AUs
5.	Create New AUs (optional)
6.	Join and Review Existing Uses
7.	Assign New Uses to Existing or New AUs
8.	Assign Unique Spatial Criteria
