...
```{r TADA Overview Map}
TADA_OverviewMap(WQP_Shepherdstown_clean)
```

Check coordinates

```{r}
WQP_Shepherdstown_clean = TADA_FlagCoordinates(WQP_Shepherdstown_clean, clean_outsideUSA = "change sign", clean_imprecise = FALSE)

WQP_Shepherdstown_clean <- WQP_Shepherdstown_clean[!is.na(WQP_Shepherdstown_clean$LongitudeMeasure),]
WQP_Shepherdstown_clean <- WQP_Shepherdstown_clean[!is.na(WQP_Shepherdstown_clean$LatitudeMeasure),]
```
...
# get TADA df with ATTAINS info added from list
...
```{r TADA_ViewATTAINS}
ATTAINS_map <- TADA_ViewATTAINS(WQP_Shepherdstown_subset_spatial)
...
Let's check if any IRON results are above 300 µg/l. That is the EPA 304A
recommended maximum criteria magnitude for IRON to support the
Organoleptic use. It is 1000 µg/l for Aquatic Life, Freshwater, Chronic
use.
...
Now, let's look at the specific conductance data.
...
Subset_SC <- Subset_TADAwithATTAINS %>%
  dplyr::filter(TADA.CharacteristicName == "SPECIFIC CONDUCTANCE")

TADA_AggregateMeasurements(Subset_SC,
  agg_fun = c("max"),
  clean = TRUE
)
```
