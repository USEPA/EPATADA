...
```{r install EPATADA, eval = F, results = 'hide'}
remotes::install_github("USEPA/EPATADA", ref = "develop", dependencies = TRUE)
```

```{r dev install EPATADA, include = F}
remotes::install_github("USEPA/EPATADA", ref = "596-tada_analysisdatafilter-removes-rows-when-clean-true-if-tadauseforanalysisflag-no---na", dependencies = TRUE)
```
...
# Keep only surface water and other (noncategorized) results, not working
