...
In this example, we will first get an ATTAINS Assessment Unit shapefile
(only works for polygons for now) from the ATTAINS geospatial services,
and then we will use that to query WQP using TADA_DataRetrieval and the
new aoi_sf function input. You can find ATTAINS Assessment Unit ID's
using EPA's How's My Waterway Application, see [example
...
WQP_flag_review = TADA_NearbySitesMap(WQP_clean)
...
compid_table <- TADA_FieldValuesTable(WQP_clean, field = "TADA.ComparableDataIdentifier")
...
```{r}
rm(compid_table, WQP_flag_review)
```
...
```{r filter}
WQP_clean <- dplyr::filter(WQP_flag, TADA.SingleOrgDup.Flag == "Unique")

WQP_clean <- dplyr::filter(
  WQP_flag,
...
```
In the remainder of this workshop, we will work with the clean dataset
only.

Remove intermediate variables in R by using 'rm()'

```{r}
rm(WQP_flag, WQP_flag_review, WQP_raw)
```
...
lower and upper thresholds. For these, we will set
clean = FALSE and flaggedonly = TRUE so that it returns only flagged results in the review dataframe returned. 
...
```{r}
rm(chars_before, chars_after, WQP_flag_reviewabove, WQP_flag_reviewbelow)
```
...
```{r stats}
WQP_clean_subset_stats <- TADA_Stats(WQP_clean_subset)
```
...
value (STV) for the bacteria samples. The waterbody GM should not be greater than the selected GM magnitude in any 30-day interval. The STV approximates the 90th percentile of the water quality distribution and is intended to be a
value that should not be exceeded by more than 10 percent of the samples in the same 30-day interval. The table summarizes the magnitude component of the
...
```{r}
# add column with comparison to criteria mag
...
```{r}
# scatterplot
criteria_mag_scatter <- TADA_Scatterplot(WQP_clean_subset, id_cols = "TADA.ComparableDataIdentifier") %>%
  plotly::add_lines(
    y = 320,
...
criteria_mag_scatter
```

```{r boxplot and histogram}
# generate a histogram
Histogram <- TADA_Histogram(WQP_clean_subset, id_cols = "TADA.CharacteristicName")
...
Boxplot <- TADA_Boxplot(WQP_clean_subset, id_cols = "TADA.CharacteristicName")
