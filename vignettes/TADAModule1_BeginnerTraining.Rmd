...
description: An introduction to TADA's major Module 1 functions for users unfamiliar with R and RStudio.
editor_options: 
...
```{r for-development-only, echo = F, results = 'hide'}
...
```{r Map, fig.width=8, fig.height=6, fig.fullwidth=TRUE, eval = F}
...
**TADA_FlagContinuousData** flags high frequency time series (i.e.,
continuous) data, if present.

Continuous data and daily statistics calculated from continuous data
(avg, max, or min value over daily or other intervals), may (or may not)
be usable for your analyses. Therefore, this function uses metadata
submitted by data providers to flag rows with continuous data.

When clean = TRUE, rows with continuous data are removed from the
dataframe and no column will be appended. The default is clean = FALSE.

In this example, we'll use the default clean = FALSE to flag any
continuous data.

An additional input called flaggedonly will allow the user to filter
data to show only rows of continuous data.

```{r ContinuousData}
R5ProfileClean1 <- TADA_FlagContinuousData(R5Profile)
```
...
\`\`\`{r, eval = F} \# download TADA Shiny repository
