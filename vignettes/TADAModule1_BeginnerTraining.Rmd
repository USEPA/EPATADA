---
title: 'TADA Module 1: Training for Beginner R Users'
format: html
editor: visual
author: "TADA Team"
date: "`r Sys.Date()`"
...
vignette: >
  ...
  %\VignetteIndexEntry{TADA Module 1: Training for Beginner R Users}
  %\VignetteEngine{knitr::rmarkdown}
description: An introduction to major TADA Module 1 functions for users unfamiliar with R and RStudio.
editor_options: 
...
users likely will not notice a difference in their usage of the
...
and the **TADA_DataRetrieval** function is not working due to WQP
timeout issues, then the **TADA_BigDataRetrieval** function may work
better. See **?TADA_BigDataRetrieval** for more details or the TADA
Module 1 vignette for more details.
...
ActMedName_Pie <- TADA::TADA_FieldValuesPie(R5Profile, field = "ActivityMediaName")

ActMedName_Pie

# Create table with count for each ActivityMediaName
FieldValues_ActMedia <- TADA::TADA_FieldValuesTable(R5Profile, field = "ActivityMediaName")
```
...
R5Profile <- TADA::TADA_AnalysisDataFilter(R5Profile, clean = FALSE)

# Create pie chart for TADA.UseForAnalysis.Flag
FieldVal_Pie <- TADA::TADA_FieldValuesPie(R5Profile, field = "TADA.UseForAnalysis.Flag")

FieldVal_Pie

FieldValues_AnalysisFlag <- TADA::TADA_FieldValuesTable(R5Profile, field = "TADA.UseForAnalysis.Flag")
```
...
-   **TADA_ConvertDepthUnits** - converts depth units to a consistent
    unit (meters). This function can also be run on its own, for example
    if you would like to convert depths from meters to feet later in the
    workflow.
...
R5Profile <- TADA::TADA_AutoClean(R5Profile)
```
...
MonLocTypNam_Pie <- TADA::TADA_FieldValuesPie(R5Profile, field = "MonitoringLocationTypeName")
...
```{r question4}
FieldValues_MLs_table <- TADA::TADA_FieldValuesTable(R5Profile, field = "MonitoringLocationTypeName")
...
```{r Map}
TADA::TADA_OverviewMap(R5Profile)
```
...
specified column and returns a data frame displaying the number of sites
...
R5Profile_CharSummary <- TADA::TADA_SummarizeColumn(R5Profile, "TADA.CharacteristicName")
```

**Question 5: How many unique values of TADA.CharacteristicName are
included in the data set? Which TADA.CharactersticName was collected at
the greatest number of sites? Which TADA.CharactersticName has the most
results?**
...
R5ProfileClean1 <- TADA::TADA_FlagMethod(R5Profile, clean = TRUE)

R5ProfileClean1 <- TADA::TADA_FlagFraction(R5ProfileClean1, clean = TRUE)

R5ProfileClean1 <- TADA::TADA_FlagSpeciation(R5ProfileClean1, clean = "invalid_only")

R5ProfileClean1 <- TADA::TADA_FlagResultUnit(R5ProfileClean1, clean = "invalid_only")
```
...
```{r WQX_Thresholds}
R5ProfileClean2 <- TADA::TADA_FlagAboveThreshold(R5ProfileClean1, clean = TRUE)

R5ProfileClean3 <- TADA::TADA_FlagBelowThreshold(R5ProfileClean2, clean = TRUE)
```
...
FieldValues_Orgs <- TADA::TADA_FieldValuesTable(R5ProfileClean3, field = "OrganizationFormalName")
```
...
R5ProfileClean3 <- TADA::TADA_FindPotentialDuplicatesMultipleOrgs(R5ProfileClean3,
  dist_buffer = 100,
  org_hierarchy = "none"
)

# Flag potential duplicates from a single organization
R5ProfileClean3 <- TADA::TADA_FindPotentialDuplicatesSingleOrg(R5ProfileClean3)
...
This function returns counts for you the entire data frame for each of
...
FieldCounts_Table_R5 <- TADA::TADA_FieldCounts(R5ProfileClean4)
```
...
R5ProfileClean5 <- TADA::TADA_FindQCActivities(R5ProfileClean4,
  clean = TRUE
)

# Generate pie chart for ActivityTypeCode
ActTypCod_Pie <- TADA::TADA_FieldValuesPie(R5ProfileClean5, "ActivityTypeCode")
...
most_common_activity <- TADA::TADA_FieldValuesTable(R5ProfileClean5, "ActivityTypeCode") %>%
  dplyr::slice_max(Count)
```
...
MQC_Pie <- TADA::TADA_FieldValuesPie(R5ProfileClean5, "MeasureQualifierCode")
...
# Flag only
Review_R5ProfileClean5 <- TADA::TADA_FlagMeasureQualifierCode(R5ProfileClean5,
  clean = FALSE,
  flaggedonly = TRUE,
  define = TRUE
)
...
R5ProfileClean5 <- TADA::TADA_FlagMeasureQualifierCode(R5ProfileClean5,
  clean = TRUE
)
```
...
R5ProfileClean5 <- TADA::TADA_SimpleCensoredMethods(R5ProfileClean5,
...
workflow that contain only NAs. The **TADA_AutoFilter** function removes
non-numeric data and optional columns containing only NAs.
...
R5ProfileClean6 <- TADA::TADA_AutoFilter(R5ProfileClean5)
```
...
UniqueHarmonizationRef <- TADA::TADA_GetSynonymRef(R5ProfileClean6)
...
# Harominze data set
R5ProfileClean7 <- TADA::TADA_HarmonizeSynonyms(R5ProfileClean6,
...
R5ProfileClean8 <- TADA::TADA_CalculateTotalNP(R5ProfileClean7, daily_agg = "max")
```
...
```{r TADA_FieldValuesTable_chars, results = 'hide'}
Char_Pie <- FieldValuesTable_Chars <- TADA::TADA_FieldValuesTable(R5ProfileClean8, field = "TADA.CharacteristicName")
...
R5_FieldCounts_Chars <- TADA::TADA_FieldCounts(R5ProfileClean8, display = "most", characteristicName = "DISSOLVED OXYGEN (DO)")
```
...
DO_SCM_Pie <- TADA::TADA_FieldValuesPie(R5ProfileClean8, field = "SampleCollectionMethod.MethodName", characteristicName = "DISSOLVED OXYGEN (DO)")

DO_SCM_Pie

# Create table for SampleCollectionMethod.MethodName for Dissolved Oxygen (DO results)
FieldValuesTable_DO_scm <- TADA::TADA_FieldValuesTable(R5ProfileClean8, field = "SampleCollectionMethod.MethodName", characteristicName = "DISSOLVED OXYGEN (DO)")
```
...
```{r TADA_TwoCharacteristicScatterplot}
# choose two and generate scatterplot
TADA::TADA_TwoCharacteristicScatterplot(R5ProfileClean8, id_cols = "TADA.ComparableDataIdentifier", groups = c("TEMPERATURE_NA_NA_DEG C", "DISSOLVED OXYGEN (DO)_NA_NA_MG/L"))
```
...
First, filter the data frame to include only "DISSOLVED OXYGEN
(DO)\_NA_NA_MG/L"
...
TADA::TADA_Histogram(R5ProfileCleanDO, id_cols = "TADA.ComparableDataIdentifier")
...
```{r boxplot}
TADA::TADA_Boxplot(R5ProfileCleanDO, id_cols = "TADA.ComparableDataIdentifier")
```

Generate a new data frame with only the daily max dissolved oxygen
values.

```{r DO_filtered}
R5ProfileCleanDO_dailymax <- TADA::TADA_AggregateMeasurements(R5ProfileCleanDO,
  agg_fun = c("max"),
  clean = TRUE
)
```
...
```{r scatterplot}
TADA::TADA_Scatterplot(R5ProfileCleanDO_dailymax, id_cols = "TADA.ComparableDataIdentifier")
```
...
remotes::install_github("USEPA/TADAShiny", ref = "develop", dependencies
= TRUE )
