...
To contribute a specific change or new code, outside contributors can fork this repo from the **develop** branch, or simply create a new branch. Contributors should work from their personal fork and/or branch, and on one specific "task" at a time. When complete, they should submit a pull request, to request their changes be merged from their branch into the TADA develop branch. Contributors should submit a separate pull request for each "task".
...
Complete the pull request by detailing all fixes and contributions, and tagging TADA repo admins who should review the work. For this package, please tag cristinamullin (Cristina Mullin) and mthawley (Shelly Thawley). Repository Administrators will review code contributions from both internal EPA and external collaborators and integrate code commits into source code. This is done to ensure code stability and consistency and prevent degradation of code performance. After review, the admin will either accept the submission, recommend specific improvements to the submission, or in some cases reject the submission. 

To avoid issues, developers contributing code should contact the repository admins (Cristina or Shelly) early in the development process and maintain contact throughout to help ensure the submission is compatible with the code base and is a robust addition.
