...
Users have the option of returning the ATTAINS geospatial sf objects
with their ATTAINS-linked Water Quality Portal dataframe. If
...
`TADA_GetATTAINS()` always returns a dataframe (or dataframes if
`fill_catchments = TRUE`) containing the original TADA WQP dataset, plus
new columns representing the ATTAINS assessment unit(s) that fall within
the same NHDPlus HR catchment as them. This means that it is possible
...
Now, let's select specific columns from the TADA_with_ATTAINS dataframe,
and create a new dataframe with ONLY the unique combinations of WQP
MonitoringLocationIdentifier's and ATTAINS Assessment Unit Identifiers.
...
associated ATTAINS assessment unit (see that in the returned dataframe,
some WQP observations have `NA`'s where there should be ATTAINS
...
into two dataframes: WQP observations with ATTAINS data, and WQP
observations without ATTAINS data. Instead of listing ATTAINS
information in `TADA_without_ATTAINS`, it links basic information about
the catchment including its unique identifier, catchment area, and the
resolution of the NHD used. As a default, TADA_GetATTAINS() will use the
NHD HiRes for filling in missing ATTAINS catchments. However, the user
can choose to change the resolution to the NHDPlus V2 by setting
`resolution = "NHDPlus V2".`

Moreover, when return_sf = TRUE as above, the function will additionally
return the raw catchment features associated with the observations in
`TADA_without_ATTAINS` in `without_ATTAINS_catchments`.

#### Arguments for `TADA_GetATTAINS()`

-   `.data`: Your input TADA-style Water Quality Portal data.

-   `fill_catchments`: If TRUE, it will use NHD data to fill in
    information for samples not covered by ATTAINS.

-   `resolution`: Specifies which version of the NHD to use if filling
    catchments: the "NHDPlus V2", or "NHDPlus HiRes". The default option
    is "NHDPlus HiRes".
