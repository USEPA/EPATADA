...
```{r install_remotes, eval = F, results = 'hide'}
...
```{r load TADA, eval = F, results = 'hide'}
library(EPATADA)
```
...
(or dataframes if `fill_catchments = TRUE`, see section ***Filling in
...
Enter `?TADA_CreateATTAINSAUMLCrosswalk` into the console to review
...
`TADA_CreateATTAINSAUMLCrosswalk()` when `return_sf = TRUE`. For the
function to work properly, the input dataframe must be the list produced
from `TADA_CreateATTAINSAUMLCrosswalk()` with `return_sf = TRUE`. The
map also displays the Water Quality Portal monitoring locations used in
`TADA_CreateATTAINSAUMLCrosswalk()`.
...
```{r ATTAINS catchments, fig.height=7}
