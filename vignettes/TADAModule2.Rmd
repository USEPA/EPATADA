...
By default, `TADA_CreateATTAINSAUMLCrosswalk()` returns a dataframe with ATTAINS-linked
...
objects, `TADA_CreateATTAINSAUMLCrosswalk()` always returns a dataframe (or dataframes
...
units linked to it. If the user would like to link all of these
features, set `return_nearest = FALSE`. In these instances of multiple
...
observations can be identified using the \`ResultIdentifier\`column
(i.e., multiple rows with the same \`ResultIdentifier\`value are the
same observation).
...
#### Using `TADA_CreateATTAINSAUMLCrosswalk()`
...
```{r}
TADA_with_ATTAINS <- TADA_CreateATTAINSAUMLCrosswalk(.data = TADA_dataframe, return_sf = FALSE, return_nearest = FALSE)

# Can also be performed on the spatial data:
# TADA_with_ATTAINS <- TADA_CreateATTAINSAUMLCrosswalk(.data = TADA_spatial, return_sf = FALSE, return_nearest = FALSE)
```
...
```{r}
TADA_with_ATTAINS_list <- TADA_CreateATTAINSAUMLCrosswalk(.data = TADA_dataframe, return_sf = TRUE, return_nearest = FALSE)
...
```{r}
TADA_with_ATTAINS_filled <- TADA_CreateATTAINSAUMLCrosswalk(TADA_dataframe, fill_catchments = TRUE, return_sf = TRUE, return_nearest = FALSE)
```
...
`TADA_CreateATTAINSAUMLCrosswalk()` will use the NHD HiRes (`resolution = "Hi"`) for
...
Enter `?TADA_CreateATTAINSAUMLCrosswalk` into the console to review another example of
...
`TADA_CreateATTAINSAUMLCrosswalk()` when `return_sf = TRUE`. For the function to work
properly, the input dataframe must be the list produced from
`TADA_CreateATTAINSAUMLCrosswalk()` with `return_sf = TRUE`. The map also displays the
Water Quality Portal monitoring locations used in `TADA_CreateATTAINSAUMLCrosswalk()`.
