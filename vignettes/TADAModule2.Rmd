...
Install arcgis and arcgislayers from GitHub. These are required to run
TADA geospatial functions such as TADA_GetATTAINS().

```{r install_arcgislayers, eval = F, results = 'hide'}
remotes::install_github("R-ArcGIS/arcgis")
remotes::install_github("R-ArcGIS/arcgislayers")
```

Use the **library()** function to load the packages into your R session.
