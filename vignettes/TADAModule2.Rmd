...
included in the EPATADA R package, and can be loaded into your R environment 
like this:

```{r load WQP data}
# Load the dataset
data("Data_MT_MissoulaCounty", package = "EPATADA")
...
-   To make this function run more efficiently, the default is to not
    ...
    they can set fill_ATTAINS_catch = TRUE. This will return the catchments, but
...
```{r useexampledata}
data("Data_MT_AUMLRef", package = "EPATADA")
...
contains more than one assessment unit.
...
```{r view.map}
TADA_ViewATTAINS(MT.AUMLRef, ref_icons = FALSE)
```

You may have noticed that because we set `fill_USGS_catch = TRUE`, `TADA_ViewATTAINS()` also mapped the `without_ATTAINS_catchments`. These are represented as clear polygons with black outlines.
