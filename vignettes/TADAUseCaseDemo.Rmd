...
Filtered_data <- Harmonized_data %>%
  dplyr::filter(TADA.ComparableDataIdentifier %in% (c(params$compid1, params$compid2, params$compid3)))
...
compid3standard_table <- compid2standard %>%
  dplyr::select(MonitoringLocationIdentifier, OrganizationFormalName, ActivityStartDate, TADA.ResultMeasureValue,
                MeetsStandard)

compid3scatter <- TADA::TADA_Scatterplot(compid2standard, id_cols = "TADA.ComparableDataIdentifier") %>%
  plotly::add_lines(
    y = params$wqstandard2lower,
    x = c(0, max(compid2standard$ActivityStartDate)),
    inherit = FALSE,
