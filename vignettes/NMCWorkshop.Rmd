...
vignette: >
  ...
  %\VignetteIndexEntry{Creating Efficient and Reproducible Water Quality Workflows Using R}
...
## DRAFT EPA-USGS Joint Workshop

Creating Efficient and Reproducible Water Quality Workflows Using R

## (5 mins) Introduction

This workshop showcases how to integrate several R Packages developed by
the U.S. Environmental Protection Agency (EPA) and the U.S. Geological
Survey (USGS) to create efficient and reproducible workflows that
support water quality programs and research. For example, performing
watershed or waterbody level analyses such as Clean Water Act (CWA)
analyses.

## Intended audience

Water Quality eXchange (WQX) and Water Quality Portal (WQP) community,
CWA assessment community (EPA, States and Tribal Nations), water quality
and geospatial data analysts/researchers, EPA/USGS and other federal
agencies.

## Agenda

This workshop demonstrates potential uses (beyond their original
...
statistical models. Finally, we will demonstrate an example workflow for
...
Install and load required packages including remotes,
[EPATADA](https://github.com/USEPA/EPATADA), StreamCatTools, and
NMCWorkshopData (example data) for this workshop from CRAN or GitHub.

```{r devsetup, include = F}
...
# Topic for discussion: Would users like results with NA units to be dealt with in TADA_AutoFilter or in the TADA_ConvertResultUnits()? These have not yet been addressed.
...
We create a unique identifier based on shared lat long values and filter
to just the 26 unique locations
...
For on-network waterbodies, it will also include the outlet flowling for
each waterbody.
...
We do not have time to cover the full process today. Let's focus on
geospatial aspects!

[Integrated Reporting Memoranda under CWA Sections 303(d), 305(b) and
...
CWA assessment determinations are made by assessment unit, meaning the
entire assessment unit is assessed as either meeting or not meeting
water quality standards (i.e., thresholds or criteria) for all
designated uses.
...
homogeneity and spatial clarity within the assessment unit.

-   Existing Assessment Units are available from ATTAINS geospatial
    services
...
-   Automates matching of WQP monitoring locations with ATTAINS
    assessment units that fall within (intersect) the same NHDPlus
    catchment
...
Finally, some waterbodies have data available in the WQP or from other
sources, but there are no existing Assessment Units in ATTAINS for them.
In the next section, we will share a way to create AU's using NHDPlus
high resolution catchments.
...
fill_catchments function input. This is included for exploratory
purposes only. In theory, states and tribal nations could use the high
res catchments as new assessment unit polygons to assess additional
areas where there is WQP data but no Assessment Unit yet in ATTAINS, but
...
-   Placeholder: Now that we have data assigned to watershed/waterbodies
    (Assessment Units). Let's showcase how to visualize water quality
    issues for a few characteristics of interest on a map and in figures
