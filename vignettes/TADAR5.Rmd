...
**Answer:**
There are `r n_char_name` unique values of TADA.CharacteristicName in the R5 data set. `r char_max_sites` was collected at the greatest number of sites (n = `r max_sites`). `r char_max_records` has the most results (n = `r max_records`).
...
## Depth unit conversions - REMOVE THIS?
...
R5ProfileClean1 <- TADA_ConvertDepthUnits(R5ProfileClean1,
  unit = "ft",
  transform = TRUE
)
```
...
R5ProfileClean4 <- TADA_FindQAPPApproval(R5ProfileClean4)

# organizations with an approved QAPP
approved_QAPP <- R5ProfileClean4 %>%
  dplyr::filter(QAPPApprovedIndicator == "Y") %>%
  dplyr::select(OrganizationFormalName) %>%
  unique()

approved_QAPP_orgs <- paste(unique(approved_QAPP$OrganizationFormalName), collapse = ", ") %>%
  stringi::stri_replace_last(fixed = ", ", " and ")
  
approved_QAPP_org_n <- length(unique(approved_QAPP$OrganizationFormalName))

```

**Question 9: How many organizations submitted results with an approved QAPP? What are the names of those organizations?**

**Answer:**
`r approved_QAPP_org_n` (`r approved_QAPP_orgs`) organization(s) submitted results with an approved QAPP. duplicates were identified by TADA_FindPotentialDuplicatesMultipleOrgs and TADA_FindPotentialDuplicatesSingleOrg. To priorize results from a particular agency or agencies see documentation for ?TADA_FindDuplicatesMultipleOrgs for information on using the org_hierarchy argument.

The **TADA_FindQAPPDoc** function checks to see if a QAPP Doc is
Available. This function checks data submitted under the "ProjectFileUrl" column to
determine if a QAPP document is available to review. See TADA Module 1 or ?TADA_FindQAPPDoc() for more information on this function.
...
**Question 10: How results were identified as "SUSPECT" based on the MeasureQualifierCode?**
