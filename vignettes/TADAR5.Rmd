...
```{r MonitoringLocations}
FieldValues_MLs_pie <- TADA_FieldValuesPie(R5Profile, field = "MonitoringLocationTypeName")
...
```{r MonitoringLocations River/Stream}
R5Profile <- R5Profile %>%
...
potentially invalid coordinate data. Its functionality will not be showcased in this demo due to time contsraints. If you are interested in incorporating it into your workflow see TADA: Module 1 or enter ?TADA_FlagCoordinates in the R console for mroe information and examples.

## Summarize by Characteristic

**TADA_SummarizeColumn** summarizes the data set based on the user specified column and returns a dataframe displaying the number of sites and number of records for each unique value in the specified column. This example  uses TADA.CharacteristicName. 
...
unique_mqc <- unique(R5ProfileClean5$MeasureQualifierCode)

unique_tada_mqc_def <- unique(R5ProfileClean5$TADA.MeasureQualifierCode.Def)
```
...
**Question: How many nutrient aggregate rows were added? Can users customize aggregation summation?**
...
R5ProfileCleanDO_stats <- TADA_Stats(R5ProfileCleanPH)
...
TP_Boxplot <- TADA::TADA_Boxplot(R5ProfileCleanDO, id_cols = "TADA.ComparableDataIdentifier")
...
**Question: Which figure generating functions can be used to visualize more than one characteristic? Which can be used to visualize only one characteristic?**
...
**Question: How many columns were removed during TADA_RetainRequired?**
...
script. Note: You must have a GitHub account to download TADAModule1.Rmd from GitHub ](images/DownloadModule1.jpg)](https://github.com/USEPA/TADA/blob/develop/vignettes/TADAR5.Rmd)
