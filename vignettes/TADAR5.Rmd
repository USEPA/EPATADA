...
result_na_n <- sum(is.na(TADAProfileClean4$TADA.ResultMeasureValue))
```
...
non_numeric_n <- sum(R5ProfileClean6$TADA.ResultMeasureValueDataTypes[TADA.ResultMeasureValueDataTypes != "Numeric"])
...
TADAProfileClean7 <- TADA_HarmonizeSynonyms(TADAProfileClean6,
...
R5ProfileClean7 <- TADA_CalculateTotalNP(R5ProfileClean6, daily_agg = "max")
```
...
R5_FieldValuesTable(R5ProfileClean6, field = "TADA.CharacteristicName")
```
...
R5_FieldCounts_char <- TADA_FieldCounts(TADAProfileClean6, display = "most", characteristicName = "TOTAL PHOSPHORUS, MIXED FORMS")
```
