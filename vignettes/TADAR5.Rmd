...
organizations using data from EPA Region 5 as an example. For information on additional functions from TADA Module 1, see TADA Module 1: Water Quality Portal Data Discovery and Cleaning.
...
This example includes monitoring data for six??? parameters collected for one month (May 2019) by Region 5 state organizations(???) (Illinois, Indiana, Michigan, Minnesota, Ohio, and Wisconsin).
...
query inputs such as limiting the query to include data from only your state.
...
If you s to download a large amount of data from across a large area,
and the TADA_DataRetrieval function is not working due to WQP timeout
issues, then the **TADA_BigDataRetrieval** function may work better. See ?TADA_BigDataRetrieval for more details or the TADA Module 1 for more details. WARNING, some of the
