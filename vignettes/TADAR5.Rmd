...
Downloads using TADA_DataRetrieval will have the same columns each time. Keep in mind that any columns with a "TADA" prefix were added during data retrieval. TADA_DataRetrieval  automatically
...
*The table above shows the first 500 records from the R5 Water Quality Portal Query.*
...
**Answer:**
*There are `r mlt_n` unique MonitoringLocationTypeNames the R5 data set. The most common is `r mlt_most_common$Value` (n = `r mlt_most_common$Count`).*

Now, let's filter by `r mlt_most_common$Value` and focus the rest of this demo on the River/Stream subset of results.

```{r MonitoringLocationsRiverStream}
R5Profile <- R5Profile %>%
...
potentially invalid coordinate data. Its functionality will not be showcased in this demo due to time constraints. If you are interested in incorporating it into your workflow see TADA: Module 1 or enter ?TADA_FlagCoordinates in the R console for more information and examples.

## Summarize by Characteristic

**TADA_SummarizeColumn** summarizes the data set based on the user specified column and returns a data frame displaying the number of sites and number of records for each unique value in the specified column. This example  uses TADA.CharacteristicName. 
...
**Answer:**
*There are `r n_char_name` unique values of TADA.CharacteristicName in the R5 data set. `r max_sites$Value` was collected at the greatest number of sites (n = `r max_sites$Count`). `r max_results$Value` has the most results (n = `r max_results$Count`).*
...
See function documentation for additional  options by entering ?TADA_FindContinuousData in the console.
...
**Answer:**
*`r orgs_n` unique organizations submitted data. `r org_max$Value` submitted the most results (n = `r org_max$Count`). `r org_min$Value` submitted the fewest results (n = `r org_min$Count`).*
...
**Answer:**
*`r duplicate_removed_n` duplicates were identified by TADA_FindPotentialDuplicatesMultipleOrgs and TADA_FindPotentialDuplicatesSingleOrg. To prioritize results from a particular agency or agencies see documentation for ?TADA_FindDuplicatesMultipleOrgs for information on using the org_hierarchy argument.*
...
activity_n <- length(unique(R5ProfileClean5$ActivityTypeCode))
...
**Answer:**
*There are `r activity_n` unique ActivityTypeCodes present after removing QC samples. `r most_common_activity$Value` is the most common ActivityTypeCode (n = `r most_common_activity$Count`).*
...
mqc <- R5ProfileClean5 %>%
  select(MeasureQualifierCode, TADA.MeasureQualifierCode.Def) %>%
  unique()

```

`r rmarkdown::paged_table(mqc)`
...
**Question 14: How many nutrient aggregate rows were added? Can users customize aggregation summation?**
...
nutrient_aggregate_n <- R5ProfileClean5_n - R5ProfileClean6_n
```

**Answer:**
*`r nutrient_aggregate_n` nutrient aggregate rows were added. Users can review the summation worksheet (**TADA_GetNutrientSummationRef**) and update it to meet their requirements.*
...
TADA_FieldValuesTable(R5ProfileClean8, field = "SampleCollectionMethod.MethodName", characteristicName = "DISSOLVED OXYGEN (DO)")
...
R5ProfileCleanDO_stats

# generate a histogram
DO_Histogram <- TADA_Histogram(R5ProfileCleanDO, id_cols = "TADA.ComparableDataIdentifier")
...
DO_Boxplot <- TADA::TADA_Boxplot(R5ProfileCleanDO, id_cols = "TADA.ComparableDataIdentifier")
...
DO_Scatterplot <- TADA::TADA_Scatterplot(R5ProfileCleanDO_dailymax, id_cols = "TADA.ComparableDataIdentifier")
...
script. Note: You must have a GitHub account to download TADAR5.Rmd from GitHub ](images/DownloadModule1.jpg)](https://github.com/USEPA/TADA/blob/develop/vignettes/TADAR5.Rmd)
