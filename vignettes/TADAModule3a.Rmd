...
crosswalk tables (TADA_ParametersForAnalysis and TADA_UsesForAnalysis) that
...
specific organization in the TADA_ParametersForAnalysis() function, we must
...
TADA_ParametersForAnalysis() can help generate this crosswalk, but if desired,
...
input in TADA_ParametersForAnalysis(). This is useful to go through a review
...
TADA_ParametersForAnalysis() will fill those if there is an exact match, in the
...
MDE_EASP (see ATTAINS.FlagUseName column in NCTC_usesRef). "PH" was
...
TADA_ParametersForAnalysis with the org_id argument input. We will create
another example paramRef from TADA_ParametersForAnalysis that would allow us to
pull in this information for more than one organization. We will also
run TADA_UsesForAnalysis for multiple organizations using autoassign =
TRUE.
