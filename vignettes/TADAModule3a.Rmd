...
TADA.ComparableDataIdentifiers (characteristic, fraction and speciation
...
In this vignette, we will crosswalk the TADA.ComparableDataIdentifiers to ATTAINS parameter names. ATTAINS has multiple domains, including the parameter names domain, that have allowable values. Using a function from rExpertQuery, which provides functions for downloading tidy data from the ATTAINS public web services (<https://github.com/USEPA/rExpertQuery>), we can review all ATTAINS domains (see example below).
...
Parameter Name is one of the ATTAINS domains that has a specific list of
allowable values. Let's review the complete list:
...
In the next section, we will review which parameters a single organization has included in their submission to ATTAINS in previous cycles. In order to select a specific organization in the **`TADA_ParametersForAnalysis()`** function, we can review the ATTAINS organization identifier domain values and choose one.
...
Identifiers. Therefore, we recommend users provide a crosswalk between the two.
**`TADA_ParametersForAnalysis()`** can be used to help generate this crosswalk
or users can create their own crosswalk with the required columns
...
to ATTAINS Parameter Names. Out of the 1180 unique ATTAINS
Parameter Names, 598 have an exact match. \*\*It is important to note
...
**`TADA_ParametersForAnalysis()`** creates a template which includes all ubnique
`TADA.ComparableDataIdentifier` values from the TADA data frame and provides a
...
to create an Excel spreadsheet of the reference table for users to
review and edit. The excel output includes logic to help a user
select ATTAINS parameter names used by the selected organization in previous assessment cycles. If users do not want to work in Excel, they can
use "excel = FALSE" to return a data frame. The data frame can be edited
directly in R although there are no TADA-specific functions
...
When "excel = TRUE", a separate tab in the Excel file which contains the parameter names used in prior assessment cycles by the selected organization(s) is created.
Users then decide whether to use only the same ATTAINS parameter names from prior assessments or to use additional parameter names from the entire ATTAINS domain list. If  a suitable ATTAINS parameter name does not exist users should contact the ATTAINS team (attains\@epa.gov) for assistance or to request the addition of a new parameter name.

For our first example, let's start with a single organization. This example data set contains WQP data from multiple states. We will specify Maryland "MDE_EASP" as
the ATTAINS organization identifier(`org_id`). By default, auto_assign
equals "None" which means the ATTAINS.ParameterName column will not auto-populate, and users will need to manually fill this crosswalk.
...
Now, let's go through an example of auto-populating the crosswalk with exact matches. We will use auto_assign = 'All' to indicate we will use exact matches from all allowable ATTAINS parameter names. This means parameter names matched may not have been included in the selected organization's previous ATTAINS submissions.
...
It is also possible to restrict the assignment of exact matches to only include ATTAINS parameter names that have been used by the selected organization in previous submissions.
...
whether or not the organization specified in the `ATTAINS.OrganizationIdentifier` column has listed this parameter as a cause in prior assessment cycles. It also flags rows where the `TADA.ComparableDataIdentifier` as not been assigned an ATTAINS parameter
name.
...
or regulatory requirements. EPATADA is an optional tool and its use is not required.
