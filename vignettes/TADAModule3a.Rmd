...
organization id's to use for the funciton input.
...
We can now save and reuse NCTC_ParamRef_Manual as the
paramRef argument input in TADA_CreateParamRef(). This is useful to 
go through a review process if new WQX characteristic names are added
as a domain value.
...
(see ATTAINS.FlagUseName column in NCTC_UseParamRef). "PH" was not listed
in ATTAINS for MDE_EASP in the prior assessment cycle, but was included
in MDE_EASP paramRef argument input.
...
If desired, a user can manually assign this parameter to any applicable uses (disclaimer:
...
pull in this information for more than one organization. We will also run
TADA_CreateUseParamRef for multiple organizations using autoassign = TRUE.
