...
```{r help_pages, eval = F}
# Access help page for TADA_CreateParamRef
?TADA_CreateParamRef
```
...
In the next vignette, TADAModule3b, this will allow users to analyze
their data against the 304(a) criteria; and to easily compare results
when when using 304(a) criteria vs. their own organization's criteria.

# Example data from WQP

Let's start with an example dataframe that has gone through data
...
We must first create a crosswalk between the WQP characteristics and
ATTAINS parameters before we can integrate information from these two
data systems. Before creating TADA_CreateParamRef (the crosswalk), let's
review the unique characteristic, fraction and speciation combinations
(TADA.ComparableDataIdentifier) and how many results are available for
each.

```{r}
# create table with counts for each TADA.ComparableDataIdentifier
TADA_FieldValuesTable(Data_NCTC, field = "TADA.ComparableDataIdentifier")
```

rATTAINS provides functions for downloading tidy data from the ATTAINS
public web services (<https://github.com/mps9506/rATTAINS>). Let's use
this package to review a list of all parameter names (across all
organizations) that have been listed as impairment causes in the past in
ATTAINS. Then let's review a list of all the ATTAINS organizations.
...
The ATTAINS Parameter Names are more general than the TADA Comparable
Data Identifiers, therefore it is not easy to automatically generate
this crosswalk (not a 1-to-1 relationship). For now, users are being
asked to fill it out manually. However, the TADA_CreateParamRef()
function is designed to help. It creates a template which includes all
TADA.ComparableDataIdentifiers from the TADA dataframe and a blank
column, "ATTAINS.ParameterName" for users to input the corresponding
ATTAINS parameter name. Additionally, if desired, the function can
automatically pair the EPA 304(a) pollutant name with the
TADA.ComparableDataIdentifier for some priority characteristics.
...
use "excel = FALSE" to return a dataframe. The dataframe can be edited
...
by the specified ATTAINS organization(s) in prior assessment cycles.
...
can be saved and reused for analyses.
...
listed this parameter as a cause in prior assessment cycles (or not). It
also flags rows where the TADA.ComparableDataIdentifier has not been
assigned an ATTAINS parameter name.

```{r}
# # only run the code chunk below if you have manually edited the excel file
# downloads_path <- file.path(Sys.getenv("USERPROFILE"), "Downloads", "myfileRef.xlsx")
# NCTC_ParamRef_2 <- openxlsx::read.xlsx(downloads_path, sheet = "CreateParamRef")
# 
# NCTC_ParamRef2 <- TADA_CreateParamRef(
#   Data_NCTC,
#   org_names = c("MDE_EASP"),
#   paramRef = NCTC_ParamRef_2,
#   excel = FALSE
#   # uncomment excel = TRUE, overwrite = TRUE to run the excel file
#   # excel = TRUE, overwrite = TRUE
#   )

NCTC_ParamRef_2 <- dplyr::mutate(NCTC_ParamRef, ATTAINS.ParameterName = dplyr::case_when(
...
Let's remove intermediate variables (no longer needed) from our R environment:
...
```{r}
NCTC_UseParamRef_OneState <- TADA_CreateUseParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
...
TADA_TableExport(NCTC_UseParamRef_OneState)
```
...
in the example NCTC_UseParamRef2 below, it will not generate rows for
...
```{r}
# this will not work because NCTC_ParamRef_Final is incomplete for all these states
NCTC_UseParamRef_MultipleStates <- TADA_CreateUseParamRef(
  Data_NCTC,
...
TADA_TableExport(NCTC_UseParamRef_MultipleStates)
```
...
We'll move forward with this example in the remainder of this vignette:
...
```{r}
rm(NCTC_UseParamRef_OneState)
rm(NCTC_UseParamRef_MultipleStates)
```

In this example, we designate that the ATTAINS.ParameterName "PH" is
equivalent to both the TADA.ComparableDataIdentifier's 'PH_NA_NA_STD
UNITS' and 'PH_NA_NA_NA', which were not used in prior ATTAINS
assessment cycles for **Maryland**. This results in the 'use_name'
column returning a blank value for ATTAINS.ParameterName "PH".
Therefore, we need to specify the use_name(s) that we would like to
associate with "PH" for this organization. In excel, there will be a
drop down list of organization ("MDE_EASP") specific use_names that were
used in prior ATTAINS assessments. If no excel file is generated, this
is an example workflow of how a user can edit the table in R.
...
NCTC_UseParamRef_2 <- NCTC_UseParamRef %>%
...
TADA_TableExport(NCTC_UseParamRef_2)

# ATTAINS.FlagUseName will now reflect the changes for PH
NCTC_UseParamRef_Final <- TADA_CreateUseParamRef(
  Data_NCTC,
  paramRef = NCTC_ParamRef_Final,
  useParamRef = NCTC_UseParamRef_2, # Edits were made to UseParamRef, updates flag column
  org_id = c("EPA304a", "MDE_EASP"),
  excel = FALSE
  # uncomment excel = TRUE, overwrite = TRUE to run the excel file
  # excel = TRUE, overwrite = TRUE
