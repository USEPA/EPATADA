...
Data Identifiers, we recommend users to manually provide the crosswalk or to 
provide a pre-filled crosswalk with the correct column names 
("TADA.ComparableDataIdentifier" and "ATTAINS.ParameterName"). However, 
TADA_CreateParamRef() includes an argument input 'fillBy' to assist users in doing 
an exact match between TADA Characteristic Names to ATTAINS Parameter Name. 
There are roughly 598/1180 unique ATTAINS Parameter Names that have an exact match. 
**It is important to note that even with these exact matches, there are additional 
complexities related to the fraction, speciation and units that may be unique to 
your organization. For example, some organizations may consider any of Nitrate/Nitrite,
Ammonia as "Nitrogen" as the ATTAINS.ParameterName and may not care for the speciation
in their assessment needs. 
...
For our first example, let's start of with a single organization. In this
example TADA data frame, this spatial area consists of WQP data from several 
different states. We will specify Maryland "MDE_EASP" as our ATTAINS organization
identifier of interest. By default, fillBy = "None" which will not populate
any entries for the ATTAINS.ParameterName column, and users will need to manually fill 
this crosswalk out.
...
Now, let's go through an example in which users auto-populate the crosswalk
with exact matches. We will use fillBy = 'All' to indicate we will use exact
matches regardless of if your organization has used this "ATTAINS.ParameterName"
domain value in the past. i.e. Your organization is fine with using an ATTAINS
Parameter Name that has been submitted to ATTAINS outside of your organization.
...
We can also use fillBy = 'Org' to indicate we will use exact
...
Your 'paramRef' is prioritized over the 'fillBy' argument inputs. However,
fillBy will fill in any remaining blanks that you have not filled in. For
example, this 'paramRef' argument input that was provided matched 'Nitrate' to 
'NITROGEN, TOTAL' rather than 'NITRATE', thus even if you specified 'fillBy = 'All',
this crosswalk will prioritize 'NITROGEN, TOTAL' as the ATTAINS.ParameterName.
However, by specifying fillBy = 'All' for any values that have not been crosswalk,
TADA_CreateParamRef() will fill those if there is an exact match, in the example
below, no crosswalk of pH in your ParamRef2 was provided, so NCTC_ParamRef_Manual2 will
crosswalk the TADA Characteristic name to 'PH' as the exact match for ATTAINS parameter 
name.
...
ATTAINS.UseName associated with it. Later in the
data analysis process, users will be asked to define criteria and
methodologies that is applicable to each ATTAINS use_name and parameter
name labeled as "Include". If a ATTAINS.UseName is not applicable, users
should choose "Exclude" for that ATTAINS parameter name and use name.
...
rows as needed if the parameter applies to multiples uses. Users can also 
choose to 'auto_assign' all unique ATTAINS.UseName by ATTAINS.OrganizatioName 
to any ATTAINS.ParameterName missing a use name associated with it.

In the example below, we can see only "ZINC" and "NITROGEN, TOTAL" were the
ATTAINS.ParameterNames done in prior assessment cycles for MDE_EASP 
(see ATTAINS.FlagUseName column in NCTC_UseParamRef).
...
ParamRef2 <- dplyr::mutate(NCTC_ParamRef2, ATTAINS.ParameterName = dplyr::case_when(
  grepl("PH", TADA.ComparableDataIdentifier) ~ "PH",
  TADA.ComparableDataIdentifier == "ZINC_DISSOLVED_NA_UG/L" ~ "ZINC",
  grepl("NITRATE", TADA.ComparableDataIdentifier) ~ "NITROGEN, TOTAL"
))
