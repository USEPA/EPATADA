...
-   TADA_CreateATTAINSAUMLCrosswalk() automates matching of WQP monitoring locations
...
```{r}
WQP_with_ATTAINSonly <- TADA_CreateATTAINSAUMLCrosswalk(GreenBay_FoxRiver_Subset, fill_catchments = FALSE, return_sf = TRUE)
...
TADA_CreateATTAINSAUMLCrosswalk() fill_catchments function input (leveraging USGS's
nhdplusTools and TADA geospatial functions). This is included for
exploratory purposes only. In theory, states and tribal nations could
use the high res catchments as new assessment unit polygons to assess
additional areas where there is WQP data but no Assessment Unit yet in
ATTAINS, but that process is outside of TADA.
...
```{r}
WQP_withATTAINSandNHDPluscatchments <- TADA_CreateATTAINSAUMLCrosswalk(GreenBay_FoxRiver_Subset, fill_catchments = TRUE, return_sf = TRUE)
