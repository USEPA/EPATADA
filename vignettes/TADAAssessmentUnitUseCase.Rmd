...
This vignette is designed to be consise and provide an example of how
...
Reference](https://usepa.github.io/TADA/reference/index.html) or the
more comprehensive module vignettes:

-   [TADA Module 1 Beginner
    Training](https://usepa.github.io/TADA/articles/TADAModule1_BeginnerTraining.html)

-   [TADA Module 1 Advanced
    Training](https://usepa.github.io/TADA/articles/TADAModule1_AdvancedTraining.html)

-   [TADA Module 1 Water Quality Portal Data Discovery and
    Cleaning](https://usepa.github.io/TADA/articles/TADAModule1.html)

-   [TADA Module
    2](https://usepa.github.io/TADA/articles/TADAModule2.html)
...
Waterbody Report on How's My Water Way:
...
ten year range between 2010 and 2020. We will also select Characteristic
Types to search by. Nutrient contains nitrogen related characteristics
and Physical contains both pH and water temperature, so we use those. It
is possible to query by a specific Characteristic Name, but different
organizations may input the same characteristic in a variety of ways.
There are TADA functions that can be used to harmonize these synonyms,
so it can be useful to start with a broader query to ensure that all
relevant results are downloaded.
...
from ATTAINS relevant to the Monitoring Locations included in the WQP
data set. See [TADA Module
2](https://usepa.github.io/TADA/articles/TADAModule2.html) for a much
...
DT::datatable(MonitoringLocations)
...
to check the validity of characteristic-fraction combinations. In this
...
**TADA_FlagBelowThreshold**, can be used to check results against
national lower and upper thresholds. As in the previous examples, we
will set these functions to remove any results that fall outside the
national thresholds.
...
[TADA_FindPotentialDuplicatesMultipleOrgs](https://usepa.github.io/TADA/reference/TADA_FindPotentialDuplicatesMultipleOrgs.html).
...
the **TADA_SimpleCensoredMethods** function. The next step we take in
this example is to perform simple conversions to the censored data in
the dataset: we keep over-detects as is (no conversion made) and convert
...
After running **TADA_SimpleCensoredMethods**, we will also filter the
data to remove any remaining NAs.
...
= FALSE. In this example, we will modify the harmonization file to group
results with the fraction "NA" and results with the fraction "TOTAL" for
...
DT::datatable(Filtered_data_stats)
```
...
**TADA_Scatterplot**. In this example, horizontal lines have been added
to indicate the upper and lower ends of the water quality standard.
These are not a default option in **TADA_Scatterplot** but can be added
via the **plotly** package.
...
We can use **lubridate** and **dplyr** functions to assign the
appropriate standard to each result by using **lubridate::month** to
identify the month in which each sample was collected and
**dplyr::mutate** to create a new column for the temperature standard.
Then the results can be compared to the standard.
...
**TADA_Histogram** can be useful for identifying the overall shape of
the data.
...
**TADA_ConvertResultUnits** an additional time outside of
**TADA_AutoClean** if we wanted to convert temperature results to deg F
for analysis.
