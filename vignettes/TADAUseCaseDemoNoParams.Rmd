...
remotes::install_github("USEPA/TADA",
  ref = "398-remove-unit-conversions-from-tada-harmonization-template",
  dependencies = TRUE,
  force = TRUE
)
...
water quality standard or other value of our choice. First, l
...
Filtered_data <- Harmonized_data %>%
  dplyr::filter(TADA.ComparableDataIdentifier %in% c("PH_SUSPENDED_NA_None", "PH_TOTAL_NA_std units", "PH_TOTAL_NA_None", "PH_NA_NA_None", "TEMPERATURE, WATER_NA_NA_deg C", "TEMPERATURE, WATER_TOTAL_NA_deg C" ))

# generate stats table
Filtered_data_stats <- TADA::TADA_Stats(Filtered_data)

# choose two and generate scatterplot
TADA_TwoCharacteristicScatterplot(Harmonized_data, id_cols = "TADA.ComparableDataIdentifier", groups = c(params$compid2, params$compid3))

# comparison to standard for compid2

compid2standard <- Filtered_data %>%
  dplyr::filter(TADA.ComparableDataIdentifier == params$compid2) %>%
  dplyr::mutate(MeetsStandard = ifelse(TADA.ResultMeasureValue >= 6.5 & TADA.ResultMeasureValue <= 9, "Yes", "No")) 


compid2standard_table <- compid2standard %>%
  dplyr::select(MonitoringLocationIdentifier, OrganizationFormalName, ActivityStartDate, TADA.ResultMeasureValue,
                MeetsStandard)

compid2scatter <- TADA::TADA_Scatterplot(compid2standard, id_cols = "TADA.ComparableDataIdentifier") %>%
  plotly::add_lines(
    y = params$wqstandard2lower,
    x = c(0, max(compid2standard$ActivityStartDate)),
    inherit = FALSE,
    showlegend = FALSE,
    line = list(color = "red"),
    hoverinfo = "none"
...
compid3standard <- Filtered_data %>%
  dplyr::filter(TADA.ComparableDataIdentifier == params$compid3) %>%
  dplyr::mutate(MonthForAnalysis = lubridate::month(ActivityStartDate),
                TempStandard = ifelse(MonthForAnalysis %in% c(1,2,3,12), params$wqstandard3cool + 1.7, params$wqstandard3warm + 1.7),
                MeetsStandard = ifelse(TADA.ResultMeasureValue < TempStandard,
                                       "Yes", "No"))

  
compid3standard_table <- compid3standard %>%
  dplyr::select(MonitoringLocationIdentifier, OrganizationFormalName, ActivityStartDate, TADA.ResultMeasureValue, TempStandard,
                MeetsStandard)

compid3scatter <- TADA::TADA_Scatterplot(compid3standard, id_cols = "TADA.ComparableDataIdentifier") 
...
Compid1_data <- dplyr::filter(Filtered_data, TADA.ComparableDataIdentifier == params$compid1)

# generate a histogram
Compid1_Histogram <- TADA_Histogram(Compid1_data, id_cols = "TADA.ComparableDataIdentifier")
...
Compid1_Boxplot <- TADA::TADA_Boxplot(Compid1_data, id_cols = "TADA.ComparableDataIdentifier") 
