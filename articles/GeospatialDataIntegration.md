...
- USGS: dataRetrieval, hydrogeofetch, hydroloom
...
We use `StreamCatTools` function `sc_get_comid` (which uses a
`hydrogeofetch` web service client) to get the comid for each location.

``` r

GreenBay_FoxRiver_sf_locs$COMID <- as.integer(strsplit(StreamCatTools::sc_get_comid(GreenBay_FoxRiver_sf_locs), split = ",")[[1]])

nhdplus_data <- hydrogeofetch::subset_nhdplus(GreenBay_FoxRiver_sf_locs$COMID, nhdplus_data = "download")

outlet <- dplyr::filter(nhdplus_data$NHDFlowline_Network, hydroseq == min(hydroseq))

hydrogeofetch::plot_nhdplus(bbox = sf::st_bbox(outlet))
plot(sf::st_transform(sf::st_geometry(GreenBay_FoxRiver_sf_locs), 3857), add = TRUE)
```

### dataRetrieval/NLDI, hydrogeofetch, hydroloom

Do a network navigation and get NHDPlus for our data. Note that the
network navigation only includes flowline geometry. `hydrogeofetch`
...
nhdplus_data <- hydrogeofetch::subset_nhdplus(comids = as.integer(all_network$UT_flowlines$nhdplus_comid), nhdplus_data = "download", flowline_only = FALSE)

# or we could just get everything in the bbox to be sure we get non-network stuff too!
nhdplus_data <- hydrogeofetch::subset_nhdplus(
  bbox = sf::st_bbox(all_network$UT_flowlines),
  nhdplus_data = "download",
  flowline_only = FALSE
)

# see ?hydrogeofetch::subset_nhdplus for lots more options!
...
[hydrogeofetch](https://doi-usgs.github.io/hydrogeofetch/) and
...
*See: hydrogeofetch, TADA::fetchNHD(),
TADA_CreateATTAINSAUMLCrosswalk(), TADA_ViewATTAINS()*
...
(leveraging USGS’s hydrogeofetch and TADA geospatial functions). This is
...
For these, we implemented a solution using hydrogeofetch to pull in
