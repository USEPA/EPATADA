...
crosswalk tables (TADA_CreateParamRef and TADA_CreateUseParamRef) that
are needed before we can start assigning applicable criteria and
...
(TADA_CreateParamRef), let’s review unique
...
NCTC_ParamRef_None <- TADA_CreateParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
  auto_assign = "None",
  excel = FALSE
...
``` r
# create TADA parameter reference table for specified organization
NCTC_ParamRef_All <- TADA_CreateParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
  auto_assign = "All",
  excel = FALSE
...
``` r
# create TADA parameter reference table for specified organization
NCTC_ParamRef_Org <- TADA_CreateParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
  auto_assign = "Org",
  excel = FALSE
...
input in TADA_CreateParamRef(). This is useful to go through a review
process if new WQX characteristic names are added as a domain value.

``` r
NCTC_ParamRef_AutoCreate <- TADA_CreateParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
...
MDE_EASP (see ATTAINS.FlagUseName column in NCTC_UseParamRef). “PH” was
not listed in ATTAINS for MDE_EASP in the prior assessment cycle, but
was included in MDE_EASP paramRef argument input.

``` r
NCTC_UseParamRef_Manual <- TADA_CreateUseParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
...
UseParamRef <- NCTC_UseParamRef_Manual %>%
...
NCTC_UseParamRef_Manual_Update <- TADA_CreateUseParamRef(
  Data_NCTC,
  paramRef = NCTC_ParamRef_Final,
...
TADA_TableExport(NCTC_UseParamRef_Manual_Update)
```
...
``` r
NCTC_UseParamRef_AutoAssign <- TADA_CreateUseParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
  paramRef = NCTC_ParamRef_Final,
  auto_assign = TRUE,
...
TADA_TableExport(NCTC_UseParamRef_AutoAssign)
```

### Multiple Organization TADA_CreateParamRef and TADA_CreateUseParamRef Workflow
...
pull in this information for more than one organization. We will also
run TADA_CreateUseParamRef for multiple organizations using autoassign =
TRUE.

``` r
# create TADA parameter reference table for specified organization
NCTC_ParamRef_MultipleOrgs <- TADA_CreateParamRef(
  Data_NCTC,
...
NCTC_ParamRef_MultipleOrgs_Final <- TADA_CreateParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP"),
...
NCTC_UseParamRef_MultipleOrgs_AutoAssign <- TADA_CreateUseParamRef(
  Data_NCTC,
  org_id = c("MDE_EASP", "21VASWCB", "21PA", "WVDEP"),
...
TADA_TableExport(NCTC_UseParamRef_MultipleOrgs_AutoAssign)
```
