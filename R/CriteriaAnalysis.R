...
TADA_Analysis_Join_WQP_Criteria <- function(
  ...
  if (!is.null(AUMLRef) && is.data.frame(AUMLRef)) AUMLRef <- upperize(AUMLRef)
  if (!is.null(AU_UsesRef) && is.data.frame(AU_UsesRef)) AU_UsesRef <- upperize(AU_UsesRef)
  ...
  if (has_AUMLRef) {
    if ("ATTAINS.AssessmentUnitIdentifier" %in% names(.data)) {
      message("ATTAINS.AssessmentUnitIdentifier already exists in .data; no join of AUMLRef will be done.")
    } else {
      if (all(c("TADA.MonitoringLocationIdentifier", "OrganizationIdentifier") %in% names(.data)) &&
          all(c("TADA.MonitoringLocationIdentifier", "OrganizationIdentifier") %in% names(AUMLRef))) {
        ...
        warning("AUMLRef could not be joined because required columns are missing.", call. = FALSE)
      }
    }
  }
  ...
  if (has_AUUsesRef && has_AUMLRef) {
    if ("ATTAINS.UseName" %in% names(.data)) {
      message("ATTAINS.UseName already exists in .data; no join of AU_UsesRef will be done.")
    } else {
      if (all(c("ATTAINS.OrganizationIdentifier", "ATTAINS.AssessmentUnitIdentifier", "ATTAINS.WaterType") %in% names(.data)) &&
          all(c("ATTAINS.OrganizationIdentifier", "ATTAINS.AssessmentUnitIdentifier", "ATTAINS.WaterType") %in% names(AU_UsesRef))) {
        .data <- dplyr::left_join(
          .data,
          AU_UsesRef,
          by = c(
        ...
        warning("AU_UsesRef could not be joined because required columns are missing.", call. = FALSE)
      }
    }
  }
  ...
  if (has_AUUsesRef) {
    ...
    if(length(allowed_uses) == 0) {
      warning("No ATTAINS.UseName matches were found. No criteria table can be joined, please ensure your uses match those found in your criteria table.")
    }
    criteria <- dplyr::filter(criteria, .data$`ATTAINS.UseName` %in% allowed_uses)
  }
    ...
    for (base in base_names) {
      ...
      if (x_nm %in% names(df) && y_nm %in% names(df)) {
        ...
        df[[flag_nm]] <- dplyr::case_when(
          is.na(df[[x_nm]]) & is.na(df[[y_nm]]) ~ "Pass: Both criteria table and your AUML and/or AU_Uses Ref are NA for this parameter.",
          !is.na(df[[x_nm]]) & is.na(df[[y_nm]]) ~ "Pass: criteria table is NA for this value for this parameter, assume criteria applies to all. Using value populated by your AUML and/or AU_Uses ref.",
          is.na(df[[x_nm]]) & !is.na(df[[y_nm]]) ~ "Suspect: criteria table is populated for this parameter, but your AUML and/or AU_Uses Ref is NA, keeping these values as NA.",
          !is.na(df[[x_nm]]) & !is.na(df[[y_nm]]) & df[[x_nm]] != df[[y_nm]] ~
          ...
          !is.na(df[[x_nm]]) & !is.na(df[[y_nm]]) & df[[x_nm]] == df[[y_nm]] ~ "Pass: Both criteria table and your AUML and/or AU_Uses Ref values match",
          TRUE ~ NA_character_
        )
