...
#' Create A Single Characteristic Profile Over Time (By Depth Category)
#'
#' @param .data TADA data frame containing the data downloaded from the WQP,
#'   where each row represents a unique data record. TADA_DepthCategory.Flag
#'   has been run as data frame must include the columns TADA.DepthCategory.Flag,
...
#'   TADA_ConvertDepthUnits.
#'
#' @param group Oneidentifier from the TADA.ComparableDataIdentifier column.
#' For example 'PH_NA_NA_NA'. TADA_IDDepthProfiles can be used to identify available
#' groups.
#'
#' @param location A single MonitoringLocationIdentifier to plot the depth profile.
#' A MonitoringLocationIdentifier must be entered or an error will be returned and
#' no depth profile will be created.
#'
#' @param depthcat Character argument indicating whether results should be grouped by
#' depth category or for the entire4 depth profile. The default is depthcat = "all"
#' and will show values/descriprtive stats for each category over time. When
#' depthcat = "none", all results will be shown and statistics calculated for the
#' entire depth profile. When depthcat = "surface" only surface values and
#' descriptive statistics will be used. When depthcat = "middle" only middle values
#' and descriptive statistics will be used. When depthcat = "bottom" only botttonm
#' values and descriptive statistics will be used.
#'
#' @param bottomvalue numeric argument. The user enters how many meters from the
#' bottom should be included in the "Bottom" category. Default is
#' bottomvalue = 2.
...
TADA_CharacteristicByDepthCategoryPlot <- function(.data, 
                                  group = NULL,
                                  bycategory = "all",
                                  location = NULL,
                                  surfacevalue = 2,
  ...
  subset.data <- .data %>% dplyr::filter(MonitoringLocationIdentifier %in% location,
                                         TADA.ComparableDataIdentifier %in% group)
  
  min.vals <- subset.data %>%
    TADA_DepthCategory.Flag(bycategory = bycategory, 
                            surfacevalue = surfacevalue,
                            bottomvalue = bottomvalue,
                            dailyagg = "min"
                            ) %>%
    dplyr::filter(TADA.DepthProfileAggregation.Flag == 
                    "Selected as min aggregate value for each depth category")
  
  max.vals <- subset.data %>%
    TADA_DepthCategory.Flag(bycategory = bycategory, 
                            surfacevalue = surfacevalue,
                            bottomvalue = bottomvalue,
                            dailyagg = "max"
    ...
    dplyr::filter(TADA.DepthProfileAggregation.Flag == 
                    "Selected as max aggregate value for each depth category")
  
  avg.vals <- subset.data %>%
    TADA_DepthCategory.Flag(bycategory = bycategory, 
                            surfacevalue = surfacevalue,
                            bottomvalue = bottomvalue,
                            dailyagg = "avg"
    ...
    dplyr::filter(TADA.DepthProfileAggregation.Flag == 
                    "Calculated mean aggregate value for each depth category, with randomly selected metadata from a row in the aggregate group")
  
  plot.data <- min.vals %>%
    dplyr::full_join(max.vals) %>%
    dplyr::full_join(avg.vals)
  
  other.data <- subset.data %>%
    dplyr::filter(!ResultIdentifier %in% plot.data$ResultIdentifier)
...
title <- TADA_InsertBreaks(
      paste0(
        all.agg$TADA.CharacteristicName[1],
        " (",
        all.agg$TADA.ResultMeasure.MeasureUnitCode[1],
        ") for ",
        all.agg$MonitoringLocationName[1],
        " between ",
        format(as.Date(min(all.agg$ActivityStartDate)), "%B %d, %Y"),
        " and ",
        format(as.Date(max(all.agg$ActivityStartDate)), "%B %d, %Y"),
        sep = ""
      ),
      len = 45
    )
...
scatterplot <- plotly::plot_ly(type = "scatter", mode = "lines+markers") %>%
  plotly::layout(
    ...
    yaxis = list(
      title = paste0(plot.data$TADA.CharacteristicName[1], " (", plot.data$TADA.ResultMeasure.MeasureUnitCode[1], ")"),
      titlefont = list(size = 16, family = "Arial"),
      tickfont = list(size = 16, family = "Arial"),
      hoverformat = ",.4r", linecolor = "black", rangemode = "tozero",
      showgrid = FALSE, tickcolor = "black",
  ...
  # add min
  if(dailyagg == "all" | dailyagg == "min") {
    # config options https://plotly.com/r/configuration-options/
    scatterplot <- scatterplot %>%
      plotly::config(displaylogo = FALSE) %>% # , displayModeBar = TRUE) # TRUE makes bar always visible
      plotly::add_trace(
        data = plot.data %>% dplyr::filter(TADA.DepthProfileAggregation.Flag == "Selected as min aggregate value for each depth category",
                                           TADA.DepthCategory.Flag == "Bottom"),
        x = ~as.Date(data$ActivityStartDate),
        y = ~data$TADA.ResultMeasureValue,
        marker = list(
          size = 10,
          color = "red"
        ),
        line = list(color = "red", width = 2, connectgaps = FALSE),
        hoverinfo = "text",
        hovertext = paste(
          "Result:", paste0(data$TADA.ResultMeasureValue, " ", data$TADA.ResultMeasure.MeasureUnitCode), "<br>",
          "Activity Start Date:", data$ActivityStartDate, "<br>",
          "Activity Start Date Time:", data$ActivityStartDateTime, "<br>",
          "Depth:", paste0(
           data$TADA.ConsolidatedDepth, " ",
           data$TADA.ConsolidatedDepth.Unit
          ), "<br>",
          "Activity Relative Depth Name:", data$ActivityRelativeDepthName, "<br>",
          "TADA.DepthCategory.Flag:", paste0(
            data$TADA.DepthCategory.Flag
          ), "<br>"
        )
      )
  }
