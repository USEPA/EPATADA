...
utils::globalVariables(c(
  ...
  "STD_POLLUTANT_NAME.x", "STD_POLLUTANT_NAME.y", "UserRef.AssessmentUnitIdentifier",
  "missing.geo", "percent_match_ATTAINS_CST", "percent_match_ATTAINS_WQX",
  "percent_match_CST"
))
...
#' findInvalidGeometry
#'
#' This function checks the "geometry" column in a df for invalid geometry. A
#' flagging column indicating whether the geometry is valid or not is added. The
#' sf function "st_is_valid" is applied so geometry can be mapped.
#'
#' @param .data A data frame containing columns required for TADA functions which
#' also includes a geometry column and an ATTAINS.AssessmentUnitIdentifier column.
#'
#' @return A TADA data frame with a geometry validity flagging column and geometry
#' ready for mapping.
#'
# find and flag invalid geometry
findInvalidGeometry <- function(.data) {

  # check for geometry column in .data
  if ("geometry" %in% names(.data)) {
    # create df of rows with invalid geometry
    invalid <- .data |> dplyr::filter(!sf::st_is_valid(geometry))

    # add flagging column
    if (dim(invalid)[1] > 0) {
      # col to identify records for flagging
      col.to.flag <- ifelse("ResultIdentifier" %in% names(invalid),
        "ResultIdentifier", "assessmentunitidentifier"
      )
      # assign flag values
      .data <- .data |>
        dplyr::filter(sf::st_is_valid(geometry)) %>%
        dplyr::mutate(TADA.GeometryValidity = ifelse(
        !!rlang::sym(col.to.flag) %in% invalid, "Invalid", "Valid"
      ))

      # remove intermediate object
      rm(col.to.flag, invalid)
    } else {
      .data <- .data %>% dplyr::mutate(TADA.GeometryValidity = "Valid")
    }
  }

  # return .data with flagging column
  return(.data)
}
