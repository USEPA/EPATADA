...
TADA_AutoClean <- function(.data) {
  ...
  print(paste0("TADA_Autoclean: harmonizing dissolved oxygen characterisic name ",
               "to DISSOLVED OXYGEN SATURATION if unit is % or % SATURATN."))
  ...
  # May not be needed if implemented via WQP UI/services.
  # .data$TADA.BiologicalIntentName = toupper(.data$BiologicalIntentName)
  # TADAProfile = dplyr::filter(TADAProfile, 
  #                             TADA.BiologicalIntentName != "TISSUE" | "TOXICITY" | 
  #                             is.na(TADA.BiologicalIntentName) == TRUE)

  # run TADA_ConvertSpecialChars function
  print(paste0("TADA_Autoclean: handling special characters and coverting TADA.ResultMeasureValue ",
               "and TADA.DetectionQuantitationLimitMeasure.MeasureValue value fields to numeric."))
  ...
  print(paste0("TADA_Autoclean: converting TADA.LatitudeMeasure and TADA.LongitudeMeasure fields ",
               "to numeric."))
  .data$TADA.LatitudeMeasure <- as.numeric(.data$LatitudeMeasure)
  .data$TADA.LongitudeMeasure <- as.numeric(.data$LongitudeMeasure)

  # update data types (only needed if TADA_JoinWQPProfiles is used to download data).
  # commenting out for now (these data type conversions should happen on TADA versions of these
  # columns? review what USGS DR does)
  # join2$ActivityStartDate <- as.Date(join2$ActivityStartDate)
  # join2$ActivityEndDate <- as.Date(join2$ActivityEndDate)
  # join2$ActivityDepthHeightMeasure.MeasureValue <- as.double(
  #                                                   join2$ActivityDepthHeightMeasure.MeasureValue)
  # join2$ResultDepthHeightMeasure.MeasureValue <- as.double(
  #                                                   join2$ResultDepthHeightMeasure.MeasureValue)
  ...
  # #conversion to UTC should happen on the TADA versions of these columns, 
  # ActivityStartTime.TimeZoneCode and ActivityEndTime.TimeZoneCode would also need to be edited
  ...
  # join2$ContributingDrainageAreaMeasure.MeasureValue <- as.double(
  #                                             join2$ContributingDrainageAreaMeasure.MeasureValue)
  # join2$VerticalMeasure.MeasureValue <- as.double(join2$VerticalMeasure.MeasureValue)
  # join2$VerticalAccuracyMeasure.MeasureValue <- as.double(
  #                                             join2$VerticalAccuracyMeasure.MeasureValue)
  ...
  .data <- .data %>%
    dplyr::mutate(TADA.ResultMeasure.MeasureUnitCode = 
                    ifelse(TADA.ResultMeasure.MeasureUnitCode == "meters", "m", 
                           TADA.ResultMeasure.MeasureUnitCode),
                  ActivityDepthHeightMeasure.MeasureUnitCode = 
                    ifelse(ActivityDepthHeightMeasure.MeasureUnitCode == "meters", "m", 
                           ActivityDepthHeightMeasure.MeasureUnitCode),
                  ActivityTopDepthHeightMeasure.MeasureUnitCode = 
                    ifelse(ActivityTopDepthHeightMeasure.MeasureUnitCode == "meters", "m", 
                           ActivityTopDepthHeightMeasure.MeasureUnitCode),
                  ActivityBottomDepthHeightMeasure.MeasureUnitCode = 
                    ifelse(ActivityBottomDepthHeightMeasure.MeasureUnitCode == "meters", "m", 
                           ActivityBottomDepthHeightMeasure.MeasureUnitCode),
                  ResultDepthHeightMeasure.MeasureUnitCode = 
                    ifelse(ResultDepthHeightMeasure.MeasureUnitCode == "meters", "m", 
                           ResultDepthHeightMeasure.MeasureUnitCode)
  ...
  print(paste0("TADA_Autoclean: creating TADA.ComparableDataIdentifier field for use when ",
               "generating visualizations and analyses."))
  .data <- TADA_CreateComparableID(.data)

  print(paste0("NOTE: This version of the TADA package is designed to work with numeric data with ",
               "media name: 'WATER'. TADA_AutoClean does not currently remove (filter) data with ",
               "non-water media types. ",
               "If desired, the user must make this specification on their own outside of ",
               "package functions. ", "Example: dplyr::filter(.data, ",
...
TADA_CheckColumns <- function(.data, expected_cols) {
  if (all(expected_cols %in% colnames(.data)) == FALSE) {
    stop(paste0("The dataframe does not contain the required fields. ",
                "Use either the full physical/chemical profile downloaded from WQP or download ",
                "the TADA profile template available on the EPA TADA webpage."))
  }
}
...
#' HandleSpecialChars_ResultMeasureValue <- TADA_ConvertSpecialChars(Data_Nutrients_UT, 
#'                                                                   "ResultMeasureValue")
#' unique(HandleSpecialChars_ResultMeasureValue$TADA.ResultMeasureValueDataTypes.Flag)
#'
#' HandleSpecialChars_DetLimMeasureValue <- TADA_ConvertSpecialChars(Data_Nutrients_UT, 
...
TADA_ConvertSpecialChars <- function(.data, col, percent.ave = TRUE) {
  ...
  if (col == "ResultMeasureValue") {
    ...
    chars.data$masked <- ifelse(chars.data$CharacteristicName == "Dissolved oxygen (DO)" 
    ...
    chars.data$TADA.ResultMeasure.MeasureUnitCode <- 
      toupper(chars.data$TADA.ResultMeasure.MeasureUnitCode)
  }
  ...
  if (is.numeric(chars.data$orig)) {
    ...
    chars.data$masked <- gsub(" ", "", chars.data$masked) # get rid of white space sorting
    ...
    clean.data <- chars.data %>%
      dplyr::mutate(
        flag = dplyr::case_when(
          ...
          (grepl("([1-9]|[1-9][0-9]|100)-([1-9]|[1-9][0-9]|100)%", masked) == TRUE) 
                                                    ~ as.character("Percentage Range - Averaged"),
  ...
  # Result Values that are numeric ranges with the format #-# are converted to an average of the 
  # two numbers expressed in the range.
  if (any(clean.data$flag %in% num.range.filter)) {
    ...
    numrange <- numrange[, !names(numrange) %in% c("num1", "num2")] %>%
      dplyr::mutate(masked = ifelse(flag == "Percentage Range - Average", 
                                    paste(masked, "%", sep = ""), masked))
...
#' df <- TADA_DataRetrieval(startDate = "2022-01-01", endDate = "2022-12-31", 
#'                          characteristicType = "Nutrient", statecode = "MT", 
#'                          applyautoclean = FALSE)
#' df2 <- TADA_SubstituteDeprecatedChars(df)
#' # in this example, "Inorganic nitrogen (nitrate and nitrite)" is a USGS NWIS characteristic 
#' # that is deprecated and "Phosphate-phosphorus***retired***use Total Phosphorus, mixed forms" is 
#' # a deprecated WQX name. Both are are transformed to their new names.
...
#'                           characteristicType = "Nutrient", statecode = "WY", 
#'                           applyautoclean = FALSE)
...
TADA_SubstituteDeprecatedChars <- function(.data) {
  ...
  # read in characteristic reference table with deprecation information, filter to deprecated terms 
  # and for "retired" in CharacteristicName.
  # remove all characters after first "*" in CharacteristicName and remove any leading or trailing 
  # white space to make compatible with deprecated NWIS CharacteristicName.
  nwis.table <- utils::read.csv(system.file("extdata", "WQXCharacteristicRef.csv", 
                                            package = "EPATADA")) %>%
    dplyr::filter(
      Char_Flag == "Deprecated",
      grepl("retired", CharacteristicName)
    ) %>%
    dplyr::mutate(CharacteristicName = trimws(stringr::str_split(CharacteristicName,
                                                                 "\\*", simplify = T)[, 1]))

  # read in characteristic reference table with deprecation information and filter to 
  # deprecated terms.
  # join with deprecated NWIS CharacteristicName data.frame.
  ref.table <- utils::read.csv(system.file("extdata", "WQXCharacteristicRef.csv", 
                                           package = "EPATADA")) %>%
  ...
  # if CharacteristicName is deprecated and comparable name is not blank (NA), use the provided 
  # Comparable.Name. Otherwise, keep TADA.CharacteristicName as-is.
  .data$TADA.CharacteristicName <- ifelse(!is.na(.data$Char_Flag) 
                                          & !.data$Comparable.Name %in% c(""), 
                                          .data$Comparable.Name, .data$TADA.CharacteristicName)
  ...
  if (howmany > 0) {
    ...
    print(paste0(howmany, 
                 " results in your dataset have one of the following deprecated ",
                 "characteristic names: ", chars, ". These names have been substituted with the ",
                 "updated preferred names in the TADA.CharacteristicName field."))
  } else {
...
TADA_CreateComparableID <- function(.data) {
  TADA_CheckColumns(.data, expected_cols = c("TADA.CharacteristicName", 
                                             "TADA.ResultSampleFractionText", 
                                             "TADA.MethodSpeciationName", 
                                             "TADA.ResultMeasure.MeasureUnitCode"))
  .data$TADA.ComparableDataIdentifier <- paste(.data$TADA.CharacteristicName, 
                                               .data$TADA.ResultSampleFractionText, 
                                               .data$TADA.MethodSpeciationName, 
                                               .data$TADA.ResultMeasure.MeasureUnitCode, sep = "_")
  return(.data)
}
...
#' (which defaults to a comma) and returns a new string in the WQX 3.0 format of 
#' c("StringA","StringB")
#'
...
TADA_FindNearbySites <- function(.data, dist_buffer = 100) {
  ...
  data_sf <- unique(.data[, c("MonitoringLocationIdentifier", 
                              "TADA.LongitudeMeasure", 
                              "TADA.LatitudeMeasure")])
  ...
  # loop through distance matrix and extract site groups that are within the buffer distance 
  # from one another
  for (i in 1:dim(dist.mat1)[1]) {
    # focal site
    fsite <- rownames(dist.mat1)[i] 
    # get focal site count row as a column
    dat <- data.frame(Count = dist.mat1[i, ]) 
    # give df site names along with counts
    dat$MonitoringLocationIdentifier <- colnames(dist.mat1)
    # filter to sites within buffer
    sites <- dat$MonitoringLocationIdentifier[dat$Count == 1] 
    # get site list within buffer that does not include focal site
    sites1 <- sites[!sites %in% fsite] 
    # if this list is greater than 0, combine sites within buffer into data frame
    if (length(sites1) > 0) {
      df <- data.frame(MonitoringLocationIdentifier = 
                         sites, TADA.MonitoringLocationIdentifier = paste0(sites, collapse = ","))
      df[c("TADA.MonitoringLocationIdentifier")] <- lapply(df[
        c("TADA.MonitoringLocationIdentifier")], TADA_FormatDelimitedString)
  ...
  if (dim(groups)[1] > 0) { # if there are groups of nearby sites...
    ...
    # grp <- data.frame(TADA.SiteGroup = unique(groups$TADA.SiteGroup), 
    #                TADA.SiteGroupID = paste0("Group_", 1:length(unique(groups$TADA.SiteGroup))))
    ...
    groups_wide <- tidyr::pivot_wider(groups_wide, id_cols = "MonitoringLocationIdentifier", 
                                      names_from = "GroupCount", 
                                      names_prefix = "TADA.MonitoringLocationIdentifier", 
                                      values_from = "TADA.MonitoringLocationIdentifier")
    ...
    .data <- .data %>% tidyr::unite(col = TADA.MonitoringLocationIdentifier, 
                                    dplyr::all_of(grpcols), sep = ", ", na.rm = TRUE)
  }
  ...
  .data <- .data %>% 
    dplyr::mutate(TADA.MonitoringLocationIdentifier = ifelse(
      TADA.MonitoringLocationIdentifier == "", 
      MonitoringLocationIdentifier, 
      TADA.MonitoringLocationIdentifier))   
  
  #if no groups, give a TADA.MonitoringLocationIdentifier column filled with NA
  if (dim(groups)[1] == 0) { 
...
TADA_GetUniqueNearbySites <- function(.data) {
  ...
  .data <- .data[c("MonitoringLocationIdentifier", "MonitoringLocationName", 
                   "MonitoringLocationTypeName", "MonitoringLocationDescriptionText", 
                   "TADA.MonitoringLocationIdentifier", "TADA.LongitudeMeasure", 
                   "TADA.LatitudeMeasure")]
...
TADA_AggregateMeasurements <- function(.data, grouping_cols = c("ActivityStartDate", 
                                                                "MonitoringLocationIdentifier", 
                                                                "TADA.ComparableDataIdentifier", 
                                                                "ResultDetectionConditionText", 
                                                                "ActivityTypeCode"), 
                                       agg_fun = c("max", "min", "mean"), clean = TRUE) {
