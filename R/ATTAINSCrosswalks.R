...
#' ATTAINS.MonitoringLocationIdentifier, ML.OrganizationIdentifier,
...
#' the monitoring location, and ML.OrganizationIdentifier, containing the WQP
...
#' ML.OrganizationIdentifier column is populated with the WQP OrganizationIdentifier.
...
#' ML.OrganizationIdentifier <- c("AKDECWQ", "AKDECWQ", "AKDECWQ", "AKDECWQ", "AKDECWQ")
#'
...
#'   ATTAINS.MonitoringLocationIdentifier, ML.OrganizationIdentifier, ATTAINS.AssessmentUnitIdentifier,
  ...
  if (org_id %in% org.ref$code) {
    ...
    if (attains_replace == FALSE) {
      ...
      attains.crosswalk <- suppressMessages(TADA_GetATTAINSAUSiteCrosswalk(org_id = org_id)) %>%
        dplyr::rename(
          ...
          ML.OrganizationIdentifier = OrganizationIdentifier,
    ...
    updateMonLocIds <- function(.data) {
      provider.ref <- TADA_GetWQPOrgProviderRef() %>%
        ...
        dplyr::distinct() %>%
        dplyr::rename(ML.OrganizationIdentifier = OrganizationIdentifier) %>%
        dplyr::mutate(OrgIDForURL = ML.OrganizationIdentifier)
      ...
      add.orgs <- provider.ref %>%
        dplyr::filter(
          ProviderName == "STORET",
          grepl("_WQX", ML.OrganizationIdentifier)
        ) %>%
        dplyr::mutate(ML.OrganizationIdentifier = stringr::str_remove_all(
          OrgIDForURL,
          "_WQX"
        ))
      ...
      update.crosswalk <- .data %>%
        dplyr::left_join(provider.ref, by = dplyr::join_by(ML.OrganizationIdentifier))
      ...
      update.crosswalk.storet <- update.crosswalk %>%
        ...
        dplyr::mutate(
          ...
          ATTAINS.MonitoringLocationIdentifier = paste0(ML.OrganizationIdentifier, "-",
                                  ATTAINS.MonitoringLocationIdentifier)
        )
