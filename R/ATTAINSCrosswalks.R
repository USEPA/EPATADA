...
TADA_CreateParamRef <- function(.data, org_id = NULL, paramRef = NULL, excel = TRUE,
  ...
  if (is.null(org_id)) {
    ...
    print("Users should provide a list of ATTAINS organization identifiers that are applicable to their analysis.")
...
TADA_CreateParamUseRef <- function(.data, org_id = NULL, paramRef = NULL, paramUseRef = NULL, excel = FALSE, overwrite = FALSE) {
  ...
  CreateParamUseRef <- CreateParamUseRef %>%
    ...
    dplyr::mutate(ATTAINS.FlagUseName = dplyr::case_when(
      IncludeOrExclude == "Exclude" ~ "Use name does not apply for this ATTAINS.ParameterName. Excluding this use name from analysis.",
      is.na(use_name) ~ "No use name is provided. Consider choosing an appropriate use_name.",
