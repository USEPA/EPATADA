  ...
  if (depth.count > 0) {
    ...
    .data <- .data %>%
      ...
      dplyr::mutate(
        ...
        TADA.ConsolidatedDepth.Bottom = ifelse(DepthsPerGroup > 1 & is.na(TADA.ActivityBottomDepthHeightMeasure.MeasureValue), max(TADA.ConsolidatedDepth), TADA.ActivityBottomDepthHeightMeasure.MeasureValue)
      ) %>%
      dplyr::ungroup() %>%
