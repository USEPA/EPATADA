...
#' QAPPIndicator_N_NA <- TADA_FindQAPPApproval(Nutrients_Utah,
#'   clean = FALSE,
#'   cleanNA = FALSE, errorsonly = TRUE
#' )
#'
#' # Show data where the QAPPApprovedIndicator equals "N":
#' QAPPIndicator_N <- TADA_FindQAPPApproval(Nutrients_Utah,
#'   clean = FALSE,
...
TADA_FindPotentialDuplicates <- function(.data, dist_buffer = 100) {
  ...
  dupsprep <- .data %>%
    dplyr::select(OrganizationIdentifier, ResultIdentifier, ActivityStartDate, ActivityStartTime.Time, TADA.CharacteristicName, TADA.ResultMeasureValue) %>%
  ...
  dups_sum <- dupsprep %>%
    dplyr::group_by(ActivityStartDate, ActivityStartTime.Time, TADA.CharacteristicName, roundRV) %>%
  ...
  if (dim(dups_sum)[1] > 0) {
    ...
    dupsdat <- dupsdat %>%
      dplyr::group_by(TADA.DuplicateID) %>%
    ...
    if (dim(dupsites)[1] > 0) { # if results are potentially duplicated temporally and spatially, determine if they share site group membership
      ...
      if (dim(dupsdat)[1] > 0) { # nearby sites exist but do not overlap with potentially duplicated result values
        sitecomp <- dupsites %>%
          dplyr::select(MonitoringLocationIdentifier, dplyr::all_of(dupsitesids)) %>%
          tidyr::pivot_longer(dplyr::all_of(dupsitesids), values_to = "AllGroups") %>%
