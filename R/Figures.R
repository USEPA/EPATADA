...
#' Boxplots_TPbyOrg <- TADA_Boxplot(df, id_cols = c("TADA.ComparableDataIdentifier", 
...
#' Boxplot_output <- TADA_Boxplot(Data_Nutrients_UT, id_cols = c("TADA.ComparableDataIdentifier", 
...
TADA_Boxplot <- function(.data, id_cols = c("TADA.ComparableDataIdentifier")) {
  ...
  if (!"TADA.ComparableDataIdentifier" %in% id_cols) {
    warning(paste0("TADA.ComparableDataIdentifier not found in id_cols argument and is highly ",
                   "recommended: plotting without it may produce errors in the plot."))
  }
  ...
  if (!start == end) {
    net <- start - end
    print(paste0("Plotting function removed ", net, 
...
#' # Load example data frame:
#' data(Data_Nutrients_UT)
#' Histogram_output <- TADA_Histogram(Data_Nutrients_UT, id_cols = 
#'                                c("TADA.ComparableDataIdentifier", "MonitoringLocationTypeName"))
...
TADA_Histogram <- function(.data, id_cols = c("TADA.ComparableDataIdentifier")) {
  ...
  for (i in 1:max(.data$Group)) {
    ...
    no_outliers <- subset(plot.data, plot.data$TADA.ResultMeasureValue >= box_lower 
    ...
    histogram <- histogram %>%
      plotly::layout(
        ...
        yaxis = list(
          title = y_label, titlefont = list(size = 16, family = "Arial"), 
          tickfont = list(size = 16, family = "Arial"),
          hoverformat = ",.4r", linecolor = "black", rangemode = "tozero",
          showgrid = FALSE, tickcolor = "black"
        ),
        ...
        legend = list(title = list(
          text = paste0("<b>Select 'Outliers Removed' \nand Deselect 'All Data' \nto View a ",
                        "Subset of the Data<b>"))),
        margin = mrg
      ) %>%
    ...
    quiet({
      ...
      addLegendCustom <- function(map, colors, labels, sizes, opacity = 0.5) {
        ...
        labelAdditions <- paste0("<div style='display: inline-block;height: ", sizes, 
                                 "px;margin-top: 4px;line-height: ", sizes, "px;'>", labels, 
                                 "</div>")
      ...
      sumdat <- .data %>%
        ...
        dplyr::summarise("Sample_Count" = length(unique(ResultIdentifier)), 
      ...
      # create custom fill color function so that data sets with one value for parameter count are 
      # displayed correctly
      ...
      # Tribal layers will load by default in the overview map, restricted by the bounding box of 
      # the current dataset. They can be toggled on and off using a button (all layers work 
      # together and can't be turned on/off individually). Colors and icons are as discussed 
      #previously (orange/tan colors and open triangle icons for points) but can be changed to 
      # match HMW if desired.
      bbox <- sf::st_bbox(
        c(
      ...
      map <- leaflet::leaflet() %>%
        ...
        leaflet::fitBounds(lng1 = vbbox[1], lat1 = vbbox[2], lng2 = vbbox[3], lat2 = vbbox[4]) %>% 
        # button to reset to initial zoom and lat/long
...
TADA_FlaggedSitesMap <- function(.data) {
  ...
  outsideusa <- invalid[invalid$TADA.SuspectCoordinates.Flag %in% 
  ...
  lowresIcon <- leaflet::makeAwesomeIcon(icon = "circle", library = "fa", iconColor = "#ffffff", 
                                         markerColor = "green")
  outsideIcon <- leaflet::makeAwesomeIcon(icon = "circle", library = "fa", iconColor = "#ffffff", 
                                          markerColor = "darkblue")
  nearbyIcon <- leaflet::makeAwesomeIcon(icon = "circle", library = "fa", iconColor = "#ffffff", 
                                         markerColor = "pink")
...
#' @param characteristicName Optional. Defaults to "null". A vector of TADA-converted (all caps) 
#' WQP characteristics a user may provide to filter the results to one or more characteristics of 
#' interest. "null" will show a summary table for the whole dataset.
#'
...
#' # If there are more than 12 categories to display, any remaining categories, the ones that have 
...
TADA_FieldValuesPie <- function(.data, field = "null", characteristicName = "null") {
  dat <- utils::head(TADA_FieldValuesTable(.data = .data, field = field, 
                                           characteristicName = characteristicName), 12)
  # If data set contains more than 12 categories, dat2 aggregates 'all other' categories into its 
  # own row.
  dat2 <- data.frame(Value = "ALL OTHERS", Count = sum(TADA_FieldValuesTable(
    .data = .data, field = field, characteristicName = characteristicName)[-(1:12), ][2]))
...
#' # Load example dataset:
#' data(Data_Nutrients_UT)
#' Scatterplot_output <- TADA_Scatterplot(Data_Nutrients_UT, id_cols = 
#'                                c("TADA.ComparableDataIdentifier", "MonitoringLocationTypeName"))
...
TADA_Scatterplot <- function(.data, id_cols = c("TADA.ComparableDataIdentifier")) {
  ...
  for (i in 1:max(.data$Group)) {
    ...
    one_scatterplot <- plotly::plot_ly(
      ...
      type = "scatter",
      mode = "markers",
      # currently uses start date only, may want to change to just ActivityStartDateTime in the 
      # future, but for now ActivityStartDateTime includes NAs when time is not available. 
      # Including ActivityStartDateTime in hover feature instead.
      x = plot.data$ActivityStartDate, 
      y = plot.data$TADA.ResultMeasureValue,
      # consider color/shapes to make it easier to see sites or possible relative result values
      ...
      hoverinfo = "text",
      hovertext = paste(
        "Result:", paste0(plot.data$TADA.ResultMeasureValue, " ", 
                          plot.data$TADA.ResultMeasure.MeasureUnitCode), "<br>",
    ...
    one_scatterplot <- one_scatterplot %>%
      plotly::layout(
        ...
        xaxis = list(
          title = y_label, titlefont = list(size = 16, family = "Arial"), tickfont = 
            list(size = 16, family = "Arial"),
          hoverformat = ",.4r", linecolor = "black", rangemode = "tozero",
          showgrid = FALSE, tickcolor = "black"
        ),
        hoverlabel = list(bgcolor = "white"),
...
#'
TADA_TwoCharacteristicScatterplot <- function(.data, id_cols = "TADA.ComparableDataIdentifier", 
                                              groups) {
  # check .data is data.frame
  TADA_CheckType(.data, "data.frame", "Input object")
  ...
  if (!"TADA.ComparableDataIdentifier" %in% id_cols) {
    print(paste0("Note: TADA.ComparableDataIdentifier not found in id_cols argument and is ",
                 "highly recommended."))
  }
  ...
  if (any(!groups %in% id)) {
    stop(paste0("The 'groups' vector contains one or more inputs that are not found within ",
                "your input dataset. Check spelling and try again."))
  }
  ...
  plot.data <- subset(plot.data, plot.data[, id_cols]
                      %in% groups)[, c(id_cols, reqcols, depthcols, "ActivityStartDateTime", 
                                       "MonitoringLocationName", "TADA.ActivityMediaName",
  ...
  scatterplot <- plotly::plot_ly(type = "scatter", mode = "markers") %>%
    plotly::layout(
      ...
      yaxis = list(
        title = stringr::str_remove_all(stringr::str_remove_all(
          stringr::str_remove_all(paste0(param1$TADA.CharacteristicName[1], "  ", 
      ...
      yaxis2 = list(
        ...
        overlaying = "y",
        title = stringr::str_remove_all(stringr::str_remove_all(
          stringr::str_remove_all(paste0(param2$TADA.CharacteristicName[1], "  ", 
    ...
    # config options https://plotly.com/r/configuration-options/
    # , displayModeBar = TRUE) # TRUE makes bar always visible
...
#' Create Scatterplot(s) for Each TADA.ComparableDataIdentifier with Multiple Groups
#'
...
#'    plotted. Defaults to MonitoringLocationName. This input is flexible, and allows for the use 
#'    of other identifiers such as StateCode, CountyCode or user-created groups based on 
#'    concatenation of other variables (e.g. characteristic name, site type, site name, year, 
#'    organization, etc.)
#'
...
#' UT_Nutrients_by_CountyCode <- TADA_GroupedScatterplot(df2, group_col = "CountyCode", 
#'                                                       groups = c("057", "011", "003", "037"))
...
#' df <- dplyr::filter(Data_6Tribes_5y_Harmonized, TADA.ComparableDataIdentifier %in% 
#'                                         c("TOTAL PHOSPHORUS, MIXED FORMS_UNFILTERED_AS P_UG/L"))
...
#'                             groups = c("Upper Red Lake: West", 
#'                             "Upper Red Lake: West-Central", 
#'                             "Upper Red Lake: East Central"))
#'
#' # If no groups are selected, return the 4 groups (by MonitoringLocationName) with the greatest
#' # number of results
#' TADA_GroupedScatterplot(df, group_col = "MonitoringLocationName")
#'
TADA_GroupedScatterplot <- function(.data, group_col = "MonitoringLocationName", groups = NULL) {
  ...
  if (!is.null(groups)) {
    if (length(groups) == 1) {
      stop(paste0("TADA_GroupedScatterplot: requires at least two 'groups'. Use TADA_Scatterplot ",
                  "to plot results without grouping."))
    }
  }
  ...
  if (is.null(groups)) {
    ...
    groups.string <- stringi::stri_replace_last(paste(groups, collapse = "; "), " and ", 
                                                fixed = "; ")
    ...
    if (n.groups.plotted == "one") {
      stop(paste0("TADA_GroupedScatterplot: requires at least two 'groups'. ",
                  "Use TADA_Scatterplot to plot results without grouping."))
    }
  ...
  if (any(!groups %in% id)) {
    ...
    missing.groups.string <- stringi::stri_replace_last(paste(missing.groups, collapse = "; "), 
                                                        " and ", fixed = "; ")

    # stop function if any groups are not found in TADA data frame
    stop("TADA_GroupedScatterplot: The following ", group_col, 
         "s are not found in the TADA data frame: ", missing.groups.string, 
  ...
  # this subset must include all fields included in plot hover below
  plot.data <- subset(plot.data, plot.data[, group_col] %in% groups)[, unique(
                                  c(group_col, reqcols, depthcols, "TADA.ComparableDataIdentifier", 
                                    "ActivityStartDateTime", "MonitoringLocationName", 
                                    "TADA.ActivityMediaName", "ActivityMediaSubdivisionName", 
                                    "ActivityRelativeDepthName", "TADA.CharacteristicName", 
  ...
  for (i in 1:length(unique(groups))) {
    ...
    # assign(paste0("param", as.character(i)), subset(plot.data, plot.data[, group_col] 
    # %in% groups[i]))
  }
  ...
  for (i in 1:length(unique(plot.data$TADA.ComparableDataIdentifier))) {
    ...
    plot.data.y <- subset(plot.data, plot.data[, "TADA.ComparableDataIdentifier"] %in% 
    ...
    scatterplot <-
      ...
      plotly::layout(
        ...
        yaxis = list(
          title = paste(TADA_CharStringRemoveNA(plot.data.y$TADA.CharacteristicName[1]), 
    ...
    for (j in 1:length(groups)) {
      if (length(groups) >= j) {
        param[[j]] <- subset(param.data[[j]], param.data[[j]][, "TADA.ComparableDataIdentifier"] 
                             %in% unique(plot.data$TADA.ComparableDataIdentifier)[i])
        ...
        scatterplot <- scatterplot %>%
          plotly::add_trace(
            ...
            hoverinfo = "text",
            hovertext = paste(
              "Result:", paste0(param[[j]]$TADA.ResultMeasureValue, " ", 
                                param[[j]]$TADA.ResultMeasure.MeasureUnitCode), "<br>",
    ...
    names(all_scatterplots)[i] <- unique(TADA_CharStringRemoveNA(
                                          plot.data$TADA.ComparableDataIdentifier))[i]
  }
