...
TADA_MakeSpatial <- function(.data, crs = 4326) {
  ...
  # Safety check to ensure a valid sf object was created
  if (!inherits(sf, "sf")) {
...
fetchATTAINS <- function(.data, catchments_only = FALSE, org_id = "all") {
  ...
  if (!is.null(org_id) && !identical(org_id, "all")) {
    catchment_features <- dplyr::filter(
      catchment_features,
...
#' fetchNHD
#'
#' Fetches NHD features from either the high resolution or medium resolution
#' version of the National Hydrography Dataset (NHD) that intersect catchments
#' containing TADA Water Quality Portal observations. For HighRes requests,
#' catchments are first fetched from the AOI of unique observation locations,
#' and HighRes flowlines and waterbodies are then fetched from those catchment
#' AOIs rather than from the full bounding box. For Medium resolution requests,
#' features are fetched using `nhdplusTools` from the unique observation
#' locations and derived catchments.
#'
#' @param .data A dataframe created by `TADA_DataRetrieval()` or the geospatial
#'   equivalent made by `TADA_MakeSpatial()`.
#' @param resolution Character. Whether to download the NHDPlus HiRes
#'   resolution ("Hi") or medium NHDPlus V2 resolution ("Med") version of the
#'   National Hydrography Dataset (NHD). Default is "Hi".
#' @param features Character vector indicating which NHD features to return.
#'   Valid options are "catchments", "flowlines", and "waterbodies". Any single
#'   feature or combination of features may be requested. Order does not matter.
#'   Default is "catchments".
#'
#' @return If a single feature type is requested, returns an `sf` object for that
#'   feature. If multiple feature types are requested, returns a named list of
#'   `sf` objects. When no features are found, empty feature tables are returned
#'   for catchments or `NULL`/empty results depending on the requested feature.
#'
#' @seealso [TADA_DataRetrieval()]
#' @seealso [TADA_MakeSpatial()]
#'
...
TADA_GetATTAINSByAUID <- function(
  ...
  # get water types
  water_types <- try(
    fetchWaterType(unique(au_ref$ATTAINS.AssessmentUnitIdentifier)),
    silent = TRUE
  )
...
#'
#' Groups monitoring locations in a TADA dataset when they fall within
#' `dist_buffer` meters of one another. Nearby sites are identified by
#' calculating pairwise distances between unique monitoring locations and
#' grouping sites that are directly or indirectly linked within `dist_buffer`
#' meters of one another. Optional filters can further restrict grouping to
#' sites in the same NHD catchment, ATTAINS assessment unit, or organization.
#'
#' For each nearby-site group, the function creates a bracketed,
#' concatenated `TADA.MonitoringLocationIdentifier` and adds
#' `TADA.NearbySiteGroup` and `TADA.NearbySites.Flag`.
#' Representative metadata for
#' `TADA.MonitoringLocationName`, `TADA.LatitudeMeasure`,
#' `TADA.LongitudeMeasure`, and `TADA.MonitoringLocationTypeName` is taken
#' from one site in each group using the `meta_select` rule.
#'
#' @param .data A TADA data frame or TADA sites data frame.
#' @param dist_buffer Numeric. Maximum distance, in meters, between two sites
#'   for them to be considered nearby. Must be non-negative.
#' @param nhd_res Character. NHD catchment resolution used when `catchment = TRUE`.
#'   Must be `"Hi"` or `"Med"`. Default is `"Hi"`.
#' @param org_hierarchy Character vector of organization identifiers used to
#'   prioritize representative metadata selection. The first organization has
#'   highest priority. Organizations not listed are assigned lowest priority.
#'   Use `NULL` to treat all organizations equally.
#' @param meta_select Character. Rule used to choose representative metadata
#'   from a grouped set of nearby sites. Must be one of:
#'   `"random"`, `"newest"`, or `"count"`.
#'
#'   * `"random"`: choose one grouped site at random.
#'   * `"newest"`: choose the site with the most recent `ActivityStartDate`.
#'   * `"count"`: choose the site with the greatest number of result rows in
#'     the input data.
#'
#' @param catchment Logical. If `TRUE`, nearby sites are only grouped when they
#'   share an NHD catchment. Default is `TRUE`.
#' @param by_AU Logical. If `TRUE`, nearby sites are only grouped when they
#'   share an `ATTAINS.AssessmentUnitIdentifier`, if present. Default is `TRUE`.
#' @param by_org Logical. If `TRUE`, nearby sites are only grouped when they
#'   share an `OrganizationIdentifier`. Default is `FALSE`.
#'
#' @return The input data frame with nearby-site grouping columns added.
#'   Grouped sites receive updated representative metadata and a grouped
#'   `TADA.MonitoringLocationIdentifier`.
#'
#' @details
#' Nearby sites are identified by comparing the distances between unique
#' monitoring locations. Sites are grouped when they are within `dist_buffer`
#' meters of one another, including cases where sites are linked through a
#' chain of nearby sites.
#'
#' For example, if site A is near site B, and site B is near site C, then A, B,
#' and C may all be placed in the same group even if A and C are not directly
#' within `dist_buffer` meters.
#'
#' When `catchment = TRUE`, sites are only grouped if they share at least one
#' NHD catchment. When `by_AU = TRUE` and the ATTAINS assessment unit
#' identifier is available, sites are only grouped if they share the same
#' assessment unit. When `by_org = TRUE`, sites are only grouped if they share
#' the same `OrganizationIdentifier`.
#'
#' If multiple sites are grouped together, one site is chosen to provide the
#' representative metadata values for the grouped record. The choice is
#' controlled by `meta_select`.
#'
#' If no nearby-site groups are found after applying the distance rule and any
#' optional filters, the function returns the input data with nearby-site
#' grouping columns added and a flag indicating that no groups were detected.
#'
#' @export
#'
#' @examples
...
TADA_FindNearbySites <- function(
    ...
    nhd_res = "Hi",
    org_hierarchy = NULL,
    meta_select = c("random", "newest", "count"),
    catchment = TRUE,
    by_AU = TRUE,
    by_org = FALSE
) {
  ...
  # Validate that required columns exist before doing any spatial processing.
  expected_cols <- c(
    "TADA.MonitoringLocationIdentifier",
  ...
  # Validate dist_buffer is a single non-negative numeric value.
  if (!is.numeric(dist_buffer) || length(dist_buffer) != 1 || is.na(dist_buffer) || dist_buffer < 0) {
    stop("TADA_FindNearbySites: dist_buffer must be a single non-negative numeric value.")
  }
  
  # Validate org_hierarchy input.
  if (!is.null(org_hierarchy) && !is.character(org_hierarchy)) {
    stop("TADA_FindNearbySites: Organization hierarchy must be a character vector or NULL.")
  }
  ...
  make_no_nearby <- function(reason, flag_text = "No nearby sites detected.") {
    ...
    sf::st_drop_geometry(.data) |>
      dplyr::mutate(
        ...
        TADA.NearbySites.Flag = flag_text
      ) |>
      TADA_OrderCols() |>
      TADA_CorrectColType()
  }
  
  # Helper: build consistent flag text for grouped records.
  build_flag <- function(meta_mode, org_filtered = FALSE) {
    prefix <- "Grouped with nearby site(s)."
    if (org_filtered) {
      prefix <- paste(prefix, "Filtered by organization.")
    }
    switch(
      meta_mode,
      random = paste(prefix, "Metadata selected randomly."),
      newest = paste(prefix, "Metadata selected from the most recent ActivityStartDate."),
      count = paste(prefix, "Metadata selected from the monitoring location with the most results available across all characteristics.")
    )
  }
  ...
  if (!inherits(site_meta, "sf")) {
    site_meta <- tryCatch(TADA_MakeSpatial(site_meta), error = function(e) {
      stop("TADA_FindNearbySites: Failed to create spatial data from monitoring locations: ", e$message)
    })
  }
  
  if (!inherits(site_meta, "sf")) {
    stop("TADA_FindNearbySites: Spatial conversion failed; site metadata is not an sf object.")
  }
  
  # Validate the sf object has a geometry column.
  geom_col <- attr(site_meta, "sf_column")
  if (is.null(geom_col) || !geom_col %in% names(site_meta)) {
    stop("TADA_FindNearbySites: invalid sf object; geometry column is missing or mis-specified.")
  }
  
  # Compute pairwise distances between unique monitoring locations.
  dist.matrix <- tryCatch(
    as.matrix(sf::st_distance(site_meta)),
    error = function(e) stop("TADA_FindNearbySites: st_distance() failed: ", e$message)
  )
  ...
  # Convert distances to an adjacency matrix.
  ...
  group_xwalk <- tibble::tibble(
    ...
    Group = unname(comp.results$membership)
  ) |>
  ...
  # If no nearby groups are found, return immediately.
  ...
  # Optional catchment filtering: keep nearby groups only if sites share at least one catchment.
  if (isTRUE(catchment)) {
    # Retry wrapper for fetchNHD calls.
    fetchNHD_retry <- function(x, resolution = "Hi", retries = 3, delay = 2) {
      for (i in seq_len(retries)) {
        ...
        if (inherits(out, "sf") && nrow(out) > 0) return(out)
        Sys.sleep(delay * i)
      }
    ...
    # Project geographic data to a planar CRS for more stable spatial joins.
    safe_project <- function(x, crs = 3857) {
      if (!inherits(x, "sf")) stop("safe_project: input must be an sf object.")
      if (sf::st_is_longlat(x)) x <- sf::st_transform(x, crs)
      x
    }
    
    # Resolution-specific identifier column used by fetchNHD().
    id_col <- switch(
      nhd_res,
      Hi = "NHD.nhdplusid",
      Med = "NHD.comid"
    ...
    # Build grouped site sets to pass to fetchNHD().
    near_sites <- site_meta |>
      dplyr::filter(
        TADA.MonitoringLocationIdentifier %in% group_xwalk$TADA.MonitoringLocationIdentifier
      ) |>
      dplyr::left_join(
    ...
    # Fetch catchments for each nearby-site group.
    nhd.catch <- purrr::map(
      near_groups,
      ~ fetchNHD_retry(.x, resolution = nhd_res)
    )
    
    # Keep only sites that intersect a common catchment feature.
    group_xwalk <- purrr::map2_dfr(
      near_groups,
      nhd.catch,
      function(site_grp, catch_grp) {
        # Skip empty fetch results.
        if (is.null(catch_grp) || !inherits(catch_grp, "sf") || nrow(catch_grp) == 0) {
          return(tibble::tibble())
        }
        
        # Standardize projection / validity before joining.
        site_grp <- safe_project(site_grp)
        catch_grp <- safe_project(catch_grp)
        
        site_grp <- tryCatch(sf::st_make_valid(site_grp), error = function(e) site_grp)
        catch_grp <- tryCatch(sf::st_make_valid(catch_grp), error = function(e) catch_grp)
        
        # Ensure expected ID field exists in the fetchNHD output.
        if (!id_col %in% names(catch_grp)) {
          stop(
            "TADA_FindNearbySites: fetchNHD() output for resolution = '",
            nhd_res,
            "' does not contain expected identifier column '",
            id_col,
            "'."
          )
        }
        
        # Spatially join sites to catchment features.
        joined <- tryCatch(
          sf::st_join(site_grp, catch_grp, left = TRUE, join = sf::st_intersects),
        ...
        # Retain only groups where at least two sites share the same catchment ID.
        joined_df |>
          dplyr::filter(!is.na(.data[[id_col]])) |>
          dplyr::group_by(Group, .data[[id_col]]) |>
    ...
    # If nothing remains after catchment filtering, return no-nearby output.
    if (nrow(group_xwalk) == 0) {
      return(make_no_nearby(
        "TADA_FindNearbySites: No nearby sites detected after catchment filtering.",
        "No nearby sites detected."
      ))
    }
  }
  ...
  if (isTRUE(by_org)) {
    org_grouping_applied <- TRUE
    message("TADA_FindNearbySites: Monitoring locations will only be grouped if they are from the same organization.")
    ...
    # Crosswalk site IDs to organization IDs.
    org.ml.cw <- .data |>
      dplyr::select(TADA.MonitoringLocationIdentifier, OrganizationIdentifier) |>
      dplyr::distinct()
    
    # Retain only groups where at least two sites share the same organization.
  ...
  if (au_present && isTRUE(by_AU)) {
    message("TADA_FindNearbySites: ATTAINS.AssessmentUnitIdentifier is present. Monitoring locations will only be grouped if they fall within the same assessment unit.")
    
    # Crosswalk site IDs to assessment units.
    au.ml.cw <- .data |>
      dplyr::select(TADA.MonitoringLocationIdentifier, ATTAINS.AssessmentUnitIdentifier) |>
      dplyr::distinct()
    
    # Retain only groups where at least two sites share the same assessment unit.
    group_xwalk <- group_xwalk |>
  ...
  } else if (au_present && !isTRUE(by_AU)) {
    message("TADA_FindNearbySites: ATTAINS.AssessmentUnitIdentifier is present. Assessment unit was not used for grouping.")
  }
  ...
  new.ids <- group_xwalk |>
    ...
    dplyr::mutate(
      ...
      TADA.MonitoringLocationIdentifier.New = paste0("[", TADA.MonitoringLocationIdentifier.New, "]"),
  ...
  if (is.null(org_hierarchy)) {
    org.ranks <- tibble::tibble(OrganizationIdentifier = all.orgs, OrgRank = 99L)
  } else {
    missing.orgs <- setdiff(all.orgs, org_hierarchy)
    org.ranks <- tibble::tibble(OrganizationIdentifier = org_hierarchy, OrgRank = seq_along(org_hierarchy))
    
    if (length(missing.orgs) > 0) {
      message(paste0(
        "TADA_FindNearbySites: ",
        length(missing.orgs),
        " organization identifier(s) are missing from org_hierarchy (",
        stringi::stri_replace_last(paste(missing.orgs, collapse = ", "), fixed = ", ", " and "),
        "). Using partial hierarchy."
      ))
      org.ranks <- dplyr::bind_rows(
        org.ranks,
        tibble::tibble(OrganizationIdentifier = missing.orgs, OrgRank = length(org_hierarchy) + 1L)
      )
    }
  }
  ...
  select.meta <- switch(
    ...
    newest = {
      group_meta_ranked |>
        dplyr::arrange(OrgRank, dplyr::desc(ActivityStartDate), TADA.MonitoringLocationIdentifier) |>
        dplyr::group_by(TADA.NearbySiteGroup) |>
        dplyr::slice(1) |>
        dplyr::ungroup() |>
        dplyr::transmute(
    ...
    count = {
      ...
      group_meta_ranked |>
        ...
        dplyr::arrange(dplyr::desc(NCount), OrgRank, TADA.MonitoringLocationIdentifier) |>
        dplyr::group_by(TADA.NearbySiteGroup) |>
        dplyr::slice(1) |>
        dplyr::ungroup() |>
        dplyr::transmute(
          ...
          TADA.MonitoringLocationTypeName.New = TADA.MonitoringLocationTypeName,
          TADA.NearbySiteGroup.New = TADA.NearbySiteGroup,
          TADA.NearbySites.Flag = build_flag("count", org_grouping_applied)
        )
    }
  )
  ...
  out <- .data |>
    ...
    dplyr::left_join(select.meta, by = "TADA.MonitoringLocationIdentifier.New") |>
    dplyr::mutate(
      TADA.MonitoringLocationName = dplyr::coalesce(TADA.MonitoringLocationName.New, TADA.MonitoringLocationName),
      TADA.LatitudeMeasure = dplyr::coalesce(TADA.LatitudeMeasure.New, TADA.LatitudeMeasure),
      TADA.LongitudeMeasure = dplyr::coalesce(TADA.LongitudeMeasure.New, TADA.LongitudeMeasure),
      TADA.MonitoringLocationTypeName = dplyr::coalesce(TADA.MonitoringLocationTypeName.New, TADA.MonitoringLocationTypeName),
      TADA.MonitoringLocationIdentifier = dplyr::coalesce(TADA.MonitoringLocationIdentifier.New, TADA.MonitoringLocationIdentifier),
  ...
  if (!is.null(au_ref)) {
    ...
    if (is.data.frame(au_ref)) {
      ...
      if (dim(au.ref.mls)[1] > 0) {
        ...
        # add AUIDs and water type if user ref contained AUs and water type not found in ATTAINS
          ...
          dplyr::mutate(
            ...
            ATTAINS.OrganizationIdentifier = ifelse(
              is.na(ATTAINS.OrganizationIdentifier) &
                !is.na(UserRef.OrganizationIdentifier),
